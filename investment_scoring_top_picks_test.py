...
class InvestmentScoringTopPicksTester:
    ...
    def test_investment_scoring_top_picks_comprehensive(self):
        ...
        if not success:
            print(
                "⚠️  Exact endpoint from review request failed, trying alternative endpoint..."
            )
        ...
        if tradestation_success:
            ...
            print("   ⚠️  WARNING: No TradeStation data sources detected")
        ...
        if criteria_success_rate >= 100:
            ...
            print(
                "   ⚠️  User issue 'nu ai live price in scoring' may be PARTIALLY resolved"
            )
            print("   ✅ Investment scoring top-picks endpoint is mostly working")
            verdict = "GOOD"
        elif criteria_success_rate >= 50:
            print(
                f"⚠️  PARTIAL: Some success criteria met ({criteria_met}/{len(criteria)})"
            )
            print(
                "   ❌ User issue 'nu ai live price in scoring' is NOT fully resolved"
            )
            print("   ⚠️  Investment scoring top-picks endpoint needs improvement")
            verdict = "PARTIAL"
        else:
