...
class InvestmentScoringTopPicksTester:
    ...
    def test_investment_scoring_top_picks_comprehensive(self):
        ...
        print("="*100)
        print("🎯 OBJECTIVE: Test advanced investment scoring top-picks endpoint to verify live prices")
        ...
        print("   2. Live Prices: Confirm `stock_data.price` with real TradeStation prices")
        print("   3. Data Source: Verify `stock_data.data_source` shows 'TradeStation Live API'")
        print("   4. Complete Data: symbol, total_score, rating, component_scores, stock_data")
        ...
        if not success:
            print("⚠️  Exact endpoint from review request failed, trying alternative endpoint...")
            ...
            if not success:
                ...
                self.log_test_result("Endpoint Accessibility", False, "Both /api/investment-scoring/top-picks and /api/investments/top-picks failed")
                return False
            else:
                print("✅ Alternative endpoint works: /api/investments/top-picks")
        
        self.log_test_result("Endpoint Accessibility", success, f"Response time: {response_time:.2f}s")
        ...
        expected_structure_actual = {"recommendations": [], "total_analyzed": 0, "criteria": "", "last_updated": ""}
        
        has_review_structure = "status" in response_data and "top_picks" in response_data
        has_actual_structure = "recommendations" in response_data
        
        if has_review_structure:
            print("✅ Response has expected structure from review request: {status, top_picks}")
            top_picks = response_data.get("top_picks", [])
            structure_type = "review_format"
        elif has_actual_structure:
            print("✅ Response has actual API structure: {recommendations, total_analyzed, criteria}")
            ...
            self.log_test_result("Response Structure", False, "Neither expected structure found")
            return False
        
        self.log_test_result("Response Structure", True, f"Structure type: {structure_type}")
        ...
        self.log_test_result("Top Picks Count", len(top_picks) >= 5, f"{len(top_picks)} picks returned")
        
        # PHASE 3: Live Prices Verification - CRITICAL FOR USER ISSUE
        print(f"\n💰 PHASE 3: Live Prices Verification - CRITICAL FOR USER ISSUE")
        print("-" * 80)
        print("🚨 This phase addresses the user's core issue: 'nu ai live price in scoring'")
        ...
        for i, pick in enumerate(top_picks[:5]):  # Test first 5 picks
            ...
            price_locations = [
                ...
                ('stock_data.current_price', pick.get('stock_data', {}).get('current_price'))
            ]
            ...
            if price_found:
                print(f"     ✅ Price found: ${price_value:.2f} (location: {price_location})")
                live_prices_found += 1
                ...
                if price_value > 1.0:  # Realistic stock price
                    ...
                    print(f"     ⚠️  Price may be mock data: ${price_value:.2f}")
            else:
            ...
            data_source_locations = [
                ('stock_data.data_source', pick.get('stock_data', {}).get('data_source')),
            ...
            if data_source_found:
                ...
                if "TradeStation" in str(data_source_value):
                    ...
                    print(f"     ✅ Using TradeStation data source")
                else:
                    print(f"     ⚠️  Not using TradeStation: {data_source_value}")
            else:
                print(f"     ❌ No data source information found")
        ...
        print(f"   - TradeStation sources: {tradestation_sources}/{min(len(top_picks), 5)}")
        
        live_prices_success = live_prices_found >= 3  # At least 3 out of 5 should have prices
        tradestation_success = tradestation_sources >= 1  # At least 1 should use TradeStation
        ...
        if live_prices_success:
            ...
            print(f"   🚨 This directly relates to user's issue: 'nu ai live price in scoring'")
        
        if tradestation_success:
            print(f"   ✅ GOOD: TradeStation data sources detected")
        else:
            print(f"   ⚠️  WARNING: No TradeStation data sources detected")
        
        self.log_test_result("Live Prices Found", live_prices_success, f"{live_prices_found}/5 picks have prices")
        self.log_test_result("TradeStation Data Source", tradestation_success, f"{tradestation_sources}/5 use TradeStation")
        ...
        optional_fields = ['component_scores', 'stock_data', 'key_strengths', 'key_risks']
        ...
        complete_data_success = complete_picks >= 4  # At least 4 out of 5 should be complete
        self.log_test_result("Complete Data Fields", complete_data_success, f"{complete_picks}/5 picks have all required fields")
        ...
        self.log_test_result("Response Performance", performance_success, f"{response_time:.2f}s ({performance_rating})")
        ...
        self.log_test_result("Different Count Parameters", count_test_success, f"Results: {count_results}")
        ...
        print(f"   - Live prices found: {live_prices_found}/{min(len(top_picks), 5)} picks")
        print(f"   - TradeStation sources: {tradestation_sources}/{min(len(top_picks), 5)} picks")
        print(f"   - Response time: {response_time:.2f} seconds")
        print(f"   - Complete data fields: {complete_picks}/{min(len(top_picks), 5)} picks")
        ...
        if price_details:
            ...
            for detail in price_details:
                ...
                if detail["price_found"]:
                    print(f"   ✅ {symbol}: ${detail['price_value']:.2f} from {detail['price_location']}")
        ...
        criteria = [
            ...
            ("Returns live prices from TradeStation API", live_prices_success and tradestation_success),
        ...
        if criteria_success_rate >= 100:
            print(f"🎉 EXCELLENT: All success criteria met ({criteria_met}/{len(criteria)})")
            print(f"   ✅ User issue 'nu ai live price in scoring' should be RESOLVED")
            print(f"   ✅ Investment scoring top-picks endpoint is working perfectly")
            verdict = "EXCELLENT"
        elif criteria_success_rate >= 75:
            print(f"✅ GOOD: Most success criteria met ({criteria_met}/{len(criteria)})")
            print(f"   ⚠️  User issue 'nu ai live price in scoring' may be PARTIALLY resolved")
            print(f"   ✅ Investment scoring top-picks endpoint is mostly working")
            verdict = "GOOD"
        elif criteria_success_rate >= 50:
            print(f"⚠️  PARTIAL: Some success criteria met ({criteria_met}/{len(criteria)})")
            print(f"   ❌ User issue 'nu ai live price in scoring' is NOT fully resolved")
            print(f"   ⚠️  Investment scoring top-picks endpoint needs improvement")
            verdict = "PARTIAL"
        else:
            print(f"❌ POOR: Few success criteria met ({criteria_met}/{len(criteria)})")
            print(f"   ❌ User issue 'nu ai live price in scoring' is NOT resolved")
            print(f"   ❌ Investment scoring top-picks endpoint has significant issues")
            verdict = "POOR"
        
        # Recommendations
        print(f"\n💡 RECOMMENDATIONS:")
        if not live_prices_success:
            print(f"   🔧 CRITICAL: Ensure stock_data.price field is populated with live prices")
        if not tradestation_success:
            print(f"   🔧 IMPORTANT: Verify TradeStation API integration for live pricing")
        if not performance_success:
            print(f"   🔧 OPTIMIZE: Improve response time (currently {response_time:.2f}s)")
        if not complete_data_success:
            print(f"   🔧 ENHANCE: Ensure all required fields are present in response")
        
        if verdict == "EXCELLENT":
            print(f"   🎉 No immediate action required - endpoint working perfectly!")
