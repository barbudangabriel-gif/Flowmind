    ...
    def test_tradestation_balance_endpoint(self):
        ...
        if not success:
            ...
            if auth_success:
                ...
                print(
                    f"   ⚙️  Credentials Configured: {auth_status.get('credentials_configured', False)}"
                )

                if not auth_status.get("authenticated", False):
                    print(
                        "   ⚠️  Not authenticated - this is expected behavior for secure trading API"
                    )
                    print(
        ...
        if status == "success":
            ...
            if balances is None:
                print("   ⚠️  No balance data found in response")
                print(
        ...
        if "BalanceDetail" in balance_record:
            ...
            print("   ⚠️  No BalanceDetail section found")
            ...
            if value is not None:
                try:
                    ...
                    if min_val <= numeric_value <= max_val:
                        ...
                        print(
                            f"   ⚠️  {field}: ${numeric_value:,.2f} (outside expected range ${min_val:,}-${max_val:,})"
                        )
        ...
        if account_type.lower() == "margin":
            ...
            print(
                f"   ⚠️  Account type unexpected (expected: Margin, got: {account_type})"
            )
        ...
        if str(account_id_in_response) == self.account_id:
            ...
            print(
                f"   ⚠️  Account ID mismatch (expected: {self.account_id}, got: {account_id_in_response})"
            )
        ...
        if not found_timestamp:
            print("   ⚠️  No timestamp found in balance data")
        ...
        if real_time_indicators:
            ...
            print("   ⚠️  No clear real-time data indicators found")
