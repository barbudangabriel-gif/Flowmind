...
class PortfolioManagementTester:
 ...
 print(
 f" Note: Found existing portfolios: {[p.get('name', p.get('id', 'Unknown')) for p in response]}"
 )
 return True
 else:
 ...
 print(
 f" Warning: Module budget (${iv_module['budget']:,.2f}) exceeds portfolio balance (${current_balance:,.2f})"
 )
 ...
 print(
 " NEEDS ATTENTION: Portfolio Management System has significant issues"
 )
