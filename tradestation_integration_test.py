...
class TradeStationIntegrationTester:
    def __init__(self, base_url="https://options-trader-6.preview.emergentagent.com"):
        ...
        print(f"🔧 TradeStation Integration Tester initialized")
                    ...
                    if isinstance(response_data, dict):
                        ...
                        if len(str(response_data)) < 500:
                            ...
                            if 'data' in response_data:
                                data_keys = list(response_data['data'].keys()) if isinstance(response_data['data'], dict) else []
    ...
    def test_tradestation_auth_status(self) -> bool:
        ...
        if success:
            ...
            if authenticated:
                print(f"   ✅ TradeStation is authenticated and ready")
            elif has_refresh_token:
                print(f"   🔄 TradeStation has refresh token but needs refresh")
            else:
                print(f"   ⚠️  TradeStation not authenticated - login required")
        ...
        if success:
            ...
            if auth_url:
                ...
                if 'tradestation.com' in auth_url:
                    print(f"   ✅ URL points to TradeStation domain")
                else:
                    print(f"   ⚠️  URL may not point to TradeStation")
    ...
    def test_tradestation_token_refresh(self) -> bool:
        ...
        if success:
            ...
            print(f"   📊 Token Refresh Analysis:")
            ...
            if status == 'success':
                ...
                print(f"   ⚠️  Token refresh status: {status}")
    ...
    def test_tradestation_accounts(self) -> bool:
        ...
        if success:
            ...
            print(f"   📊 Accounts Analysis:")
            print(f"     - Accounts Found: {len(accounts)}")
            
            if accounts:
                print(f"   ✅ TradeStation accounts retrieved successfully")
                for i, account in enumerate(accounts[:3], 1):
                    account_id = account.get('AccountID', account.get('account_id', 'N/A'))
                ...
                print(f"   ⚠️  No accounts found - may need authentication")
    ...
    def test_tradestation_positions(self, account_id: str = None) -> bool:
        ...
        if not account_id:
            ...
            if accounts_success:
                ...
                if accounts:
                    account_id = accounts[0].get('AccountID', accounts[0].get('account_id'))
                    print(f"   📋 Using account ID: {account_id}")
                else:
                    print(f"   ❌ No accounts available for positions test")
                    return False
            else:
                print(f"   ❌ Could not retrieve accounts for positions test")
                return False
        
        if not account_id:
            print(f"   ❌ No valid account ID available")
            return False
        ...
        if success:
            ...
            if isinstance(positions_data, dict):
                positions = positions_data.get('positions', positions_data.get('data', []))
            ...
            if positions:
                print(f"   ✅ TradeStation positions retrieved successfully")
                
                # Analyze position types
                stocks = [p for p in positions if p.get('asset_type', '').upper() in ['STOCK', 'EQ']]
                options = [p for p in positions if p.get('asset_type', '').upper() in ['OPTION', 'OP']]
                ...
                for i, position in enumerate(positions[:3], 1):
                    ...
                    print(f"     Position {i}: {symbol} - {quantity} shares - ${market_value:.2f}")
            else:
    ...
    def test_tradestation_balances(self, account_id: str = None) -> bool:
        ...
        if not account_id:
            ...
            if accounts_success:
                ...
                if accounts:
                    ...
                    print(f"   ❌ No accounts available for balances test")
                    return False
            else:
                print(f"   ❌ Could not retrieve accounts for balances test")
                return False
        ...
        if success:
            ...
            print(f"   📊 Balances Analysis:")
            
            if isinstance(balances_data, dict):
                # Look for common balance fields
                balance_fields = ['total_value', 'cash_balance', 'buying_power', 'equity', 'market_value']
                found_fields = []
                ...
                for field in balance_fields:
                    if field in balances_data:
                        ...
                        print(f"     - {field.replace('_', ' ').title()}: ${value:,.2f}" if isinstance(value, (int, float)) else f"     - {field.replace('_', ' ').title()}: {value}")
                
                if found_fields:
                    print(f"   ✅ TradeStation balances retrieved successfully")
                    print(f"     - Balance fields found: {len(found_fields)}")
                else:
                    print(f"   ⚠️  Balance data structure may be different than expected")
                    print(f"     - Available keys: {list(balances_data.keys())}")
            else:
                print(f"   ⚠️  Unexpected balances response format")
    ...
    def test_token_auto_refresh_mechanism(self) -> bool:
        ...
        if final_success:
            ...
            if final_authenticated and auto_refresh_working:
                print(f"   ✅ Token auto-refresh mechanism appears to be working")
                return True
            else:
                print(f"   ⚠️  Token auto-refresh mechanism may have issues")
                return False
        else:
    ...
    def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        print("=" * 80)
        print("🎯 OBJECTIVE: Test TradeStation integration and token refresh functionality")
        print("📋 TEST PHASES:")
        ...
        if phase_success_rate >= 85:
            ...
            print("   ⚠️  Some minor issues detected - review failed phases")
        elif phase_success_rate >= 50:
            print("   ⚠️  MODERATE: TradeStation integration has significant issues")
        ...
        if phase_success_rate >= 85 and self.tests_passed >= (self.tests_run * 0.8):
            print("   🎉 EXCELLENT - TradeStation integration and token refresh working perfectly!")
            verdict = "EXCELLENT"
            ...
            print("   ⚠️  MODERATE - TradeStation integration needs attention")
            verdict = "MODERATE"
        else:
