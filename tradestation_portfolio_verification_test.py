...
class TradeStationPortfolioVerificationTester:
    ...
    def test_tradestation_accounts(self) -> Dict[str, Any]:
        ...
        if not success:
            self.log_test("TradeStation Accounts API", "FAIL", 
        ...
        if not isinstance(accounts_data, dict):
            self.log_test("Accounts Response Structure", "FAIL", 
                         f"Expected dict, got {type(accounts_data)}")
            return {"success": False, "accounts": [], "error": "Invalid response structure"}
        ...
        self.log_test("TradeStation Accounts API", "PASS", f"Successfully retrieved {len(accounts)} accounts")
        ...
        for i, account in enumerate(accounts):
            ...
            balance_fields = ['balance', 'total_balance', 'account_value', 'equity', 'cash_balance']
            balance_info = {}
            ...
            if balance_info:
                ...
                self.log_test(f"Account {account_id} Balance Info", "PASS", 
                             f"Found {len(balance_info)} balance fields")
            else:
                self.log_test(f"Account {account_id} Balance Info", "WARNING", 
                             "No balance information found in account data")
            
            # Check for additional account details
            additional_fields = ['status', 'currency', 'margin_enabled', 'day_trading_buying_power']
            additional_info = {}
            ...
            if additional_info:
                print(f"     ✅ Additional account details: {len(additional_info)} fields")
        
        # Verify target account
        if target_account_found:
            self.log_test("Target Account 11775499", "PASS", "Target account found in accounts list")
        else:
            self.log_test("Target Account 11775499", "FAIL", 
                         f"Target account not found. Available accounts: {[acc.get('account_id') for acc in accounts]}")
        
        # Summary
        print(f"\n📊 ACCOUNTS SUMMARY:")
        print(f"   - Total Accounts: {len(accounts)}")
        print(f"   - Target Account Found: {'✅ Yes' if target_account_found else '❌ No'}")
    ...
    def test_tradestation_positions(self, account_id: str = "11775499") -> Dict[str, Any]:
        ...
        print("="*80)
        print(f"🎯 OBJECTIVE: Get exact position values from TradeStation API directly")
        print(f"📊 ENDPOINT: GET /api/tradestation/accounts/{account_id}/positions")
        print(f"🔍 FOCUS: Individual position values, total portfolio calculation, P&L accuracy")
        
        success, response_data = self.make_request('GET', f'tradestation/accounts/{account_id}/positions')
        
        if not success:
            self.log_test("TradeStation Positions API", "FAIL", 
        ...
        if not isinstance(positions_data, dict):
            self.log_test("Positions Response Structure", "FAIL", 
                         f"Expected dict, got {type(positions_data)}")
            return {"success": False, "positions": [], "error": "Invalid response structure"}
        ...
        if not positions:
            ...
            if 'data' in positions_data:
                ...
                self.log_test("Positions List", "FAIL", "No positions found in response")
                return {"success": False, "positions": [], "error": "No positions found"}
        
        self.log_test("TradeStation Positions API", "PASS", 
        ...
        print(f"   📊 Total Market Value (All Positions): ${all_positions_market_value:,.2f}")
        print(f"   📊 Total Unrealized P&L (All Positions): ${all_positions_unrealized_pnl:+,.2f}")
        print(f"   📊 Total P&L Percentage: {(all_positions_unrealized_pnl/all_positions_market_value*100):+.2f}%")
        ...
        if portfolio_summary:
            ...
            if value_difference < 0.01:  # Within 1 cent
                self.log_test("Portfolio Value Calculation", "PASS", 
                             f"Calculated value matches API summary: ${summary_total_value:,.2f}")
            else:
                self.log_test("Portfolio Value Calculation", "WARNING", 
        ...
        for pos_type, count in type_counts.items():
            ...
            percentage = (value / all_positions_market_value * 100) if all_positions_market_value > 0 else 0
            print(f"   📊 {pos_type.title()}: {count} positions, ${value:,.2f} ({percentage:.1f}%)")
    ...
    def test_portfolio_management_positions(self) -> Dict[str, Any]:
        ...
        print("="*80)
        print("🎯 OBJECTIVE: Compare Portfolio Management Service values with direct TradeStation API")
        print("📊 ENDPOINT: GET /api/portfolio-management/portfolios/tradestation-main/positions")
        print("🔍 FOCUS: Value comparison, data consistency, integration accuracy")
        
        success, response_data = self.make_request('GET', 'portfolio-management/portfolios/tradestation-main/positions')
        
        if not success:
            self.log_test("Portfolio Management API", "FAIL", 
        ...
        if not isinstance(pm_data, dict):
            self.log_test("Portfolio Management Response Structure", "FAIL", 
        ...
        if not pm_positions:
            self.log_test("Portfolio Management Positions", "FAIL", "No positions found in response")
            return {"success": False, "positions": [], "error": "No positions found"}
        
        self.log_test("Portfolio Management API", "PASS", 
    ...
    def compare_portfolio_values(self, ts_data: Dict, pm_data: Dict) -> Dict[str, Any]:
        ...
        print("="*80)
        print("🎯 OBJECTIVE: Identify discrepancies between TradeStation API and Portfolio Management Service")
        ...
        if not ts_data.get('success') or not pm_data.get('success'):
            self.log_test("Data Availability", "FAIL", "Cannot compare - missing data from one or both sources")
        ...
        print(f"\n📊 VALUE COMPARISON:")
        print("-" * 60)
        print(f"   TradeStation Direct API:")
        ...
        value_difference_percent = (abs(value_difference) / ts_total_value * 100) if ts_total_value > 0 else 0
        pnl_difference_percent = (abs(pnl_difference) / abs(ts_total_pnl) * 100) if ts_total_pnl != 0 else 0
        ...
        if ts_vs_reported < pm_vs_reported:
            ...
            self.log_test("Accuracy vs Reported Value", "INFO", 
            ...
            self.log_test("Accuracy vs Reported Value", "INFO", "Both sources equally close to reported value")
        ...
        if significant_value_diff:
            issues_found.append(f"Significant value difference: ${value_difference:+,.2f} ({value_difference_percent:.2f}%)")
            self.log_test("Value Consistency", "FAIL", 
                         f"Significant difference: ${value_difference:+,.2f}")
        else:
            self.log_test("Value Consistency", "PASS", 
                         f"Values are consistent (difference: ${value_difference:+,.2f})")
        
        if significant_pnl_diff:
            issues_found.append(f"Significant P&L difference: ${pnl_difference:+,.2f} ({pnl_difference_percent:.2f}%)")
        ...
        if significant_count_diff:
            issues_found.append(f"Position count mismatch: {position_count_difference:+d} positions")
            self.log_test("Position Count Consistency", "FAIL", 
        ...
        if not issues_found:
            ...
            print("   ✅ Both TradeStation API and Portfolio Management Service show consistent values")
            if ts_vs_reported > 1000:  # More than $1000 difference
                print(f"   ⚠️  However, both differ significantly from user reported value (${reported_value:,.2f})")
                print("   🔍 Investigate if user is looking at different account or time period")
        else:
            ...
            if closer_to_reported == "TradeStation API":
                print("   💡 Recommendation: Use TradeStation API as primary source (closer to reported value)")
                print("   🔧 Fix Portfolio Management Service to match TradeStation API values")
        ...
        return {
            ...
            "significant_differences": significant_value_diff or significant_pnl_diff or significant_count_diff,
    ...
    def generate_final_report(self, accounts_result: Dict, ts_result: Dict, pm_result: Dict, comparison_result: Dict):
        ...
        print(f"🔍 USER ISSUE: Values displayed ($790,174) reported as incorrect")
        ...
        if accounts_result.get('success'):
            print(f"   ✅ TradeStation Accounts API: {accounts_result.get('total_accounts', 0)} accounts")
            print(f"     - Target Account (11775499): {'✅ Found' if accounts_result.get('target_account_found') else '❌ Not Found'}")
        else:
            print(f"   ❌ TradeStation Accounts API: Failed")
        
        if ts_result.get('success'):
            print(f"   ✅ TradeStation Positions API: {ts_result.get('total_positions', 0)} positions")
            print(f"     - Total Value: ${ts_result.get('total_market_value', 0):,.2f}")
            print(f"     - Total P&L: ${ts_result.get('total_unrealized_pnl', 0):+,.2f}")
        else:
            print(f"   ❌ TradeStation Positions API: Failed")
        
        if pm_result.get('success'):
            print(f"   ✅ Portfolio Management API: {pm_result.get('total_positions', 0)} positions")
            print(f"     - Total Value: ${pm_result.get('summary_total_value', 0):,.2f}")
            print(f"     - Total P&L: ${pm_result.get('summary_total_pnl', 0):+,.2f}")
        else:
            print(f"   ❌ Portfolio Management API: Failed")
        ...
        if comparison_result.get('success'):
            ...
            print(f"     - TradeStation API: ${comparison_result.get('ts_vs_reported', 0):+,.2f}")
            print(f"     - Portfolio Management: ${comparison_result.get('pm_vs_reported', 0):+,.2f}")
            
            print(f"\n   Inter-Service Difference:")
            print(f"     - Value Difference: ${comparison_result.get('value_difference', 0):+,.2f}")
            print(f"     - Percentage Difference: {comparison_result.get('value_difference_percent', 0):.2f}%")
        ...
        if comparison_result.get('success'):
            ...
            if ts_vs_reported > 1000 and pm_vs_reported > 1000:
                print(f"\n   🎯 ROOT CAUSE ANALYSIS:")
                print(f"     - Both APIs show values significantly different from reported ${reported_value:,.2f}")
                print(f"     - This suggests the user may be looking at:")
                print(f"       • Different account ID")
                print(f"       • Different time period")
                print(f"       • Cached/stale data")
                print(f"       • Different portfolio view (e.g., including cash)")
            elif abs(comparison_result.get('value_difference', 0)) > 1000:
                print(f"\n   🎯 ROOT CAUSE ANALYSIS:")
                print(f"     - Significant difference between TradeStation API and Portfolio Management")
                print(f"     - Portfolio Management Service may have calculation errors")
                print(f"     - Data synchronization issues possible")
        ...
        if comparison_result.get('success'):
            ...
            if closer_source == "TradeStation API":
                ...
                print("   2. 📞 Contact user to verify expected value and account details")
                print("   3. 🕐 Check if timing/market hours affect the values")
            
            print(f"\n   Additional Actions:")
            print(f"   4. 🔄 Implement real-time data refresh")
            print(f"   5. 📝 Add data source indicators in UI")
            print(f"   6. ⚠️  Add discrepancy alerts for large differences")
            print(f"   7. 🧪 Add automated testing for value consistency")
        ...
        if self.tests_passed / self.tests_run >= 0.8:
            if comparison_result.get('significant_differences'):
                print("   ⚠️  PARTIAL SUCCESS: APIs are working but show different values")
                print("   🔧 Action Required: Fix data consistency issues")
            else:
                print("   ✅ SUCCESS: All APIs working and values are consistent")
                print("   🔍 Investigation: User reported value may be from different source")
        else:
            print("   ❌ FAILURE: Critical API issues prevent proper verification")
            print("   🚨 Urgent: Fix API connectivity before addressing value discrepancies")
        ...
        if ts_result.get('success') and pm_result.get('success'):
            if abs(comparison_result.get('value_difference', 0)) < 100:  # Less than $100 difference
                print(f"   💰 Verified Portfolio Value: ${ts_result.get('total_market_value', 0):,.2f}")
                print(f"   📈 Verified P&L: ${ts_result.get('total_unrealized_pnl', 0):+,.2f}")
                print(f"   ✅ Confidence: High (both sources agree)")
            else:
                print(f"   💰 TradeStation API Value: ${ts_result.get('total_market_value', 0):,.2f}")
                print(f"   💰 Portfolio Mgmt Value: ${pm_result.get('summary_total_value', 0):,.2f}")
                print(f"   ⚠️  Confidence: Medium (sources disagree)")
        elif ts_result.get('success'):
            print(f"   💰 TradeStation API Value: ${ts_result.get('total_market_value', 0):,.2f}")
            print(f"   📈 TradeStation API P&L: ${ts_result.get('total_unrealized_pnl', 0):+,.2f}")
            print(f"   ✅ Confidence: Medium (single source)")
        elif pm_result.get('success'):
            print(f"   💰 Portfolio Mgmt Value: ${pm_result.get('summary_total_value', 0):,.2f}")
            print(f"   📈 Portfolio Mgmt P&L: ${pm_result.get('summary_total_pnl', 0):+,.2f}")
            print(f"   ⚠️  Confidence: Low (fallback source only)")
        else:
            print(f"   ❌ Unable to determine true portfolio values")
            print(f"   🚨 Critical: All data sources failed")
    ...
    def run_comprehensive_test(self):
        ...
        if accounts_result.get('success') and accounts_result.get('target_account_found'):
        ...
        if ts_result.get('success') and pm_result.get('success'):
            ...
            comparison_result = {"success": False, "error": "Insufficient data for comparison"}
        
        # Generate Final Report
        self.generate_final_report(accounts_result, ts_result, pm_result, comparison_result)
