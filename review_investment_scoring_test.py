...
class ReviewInvestmentScoringTester:
    ...
    def test_investment_scoring_agent_endpoints(self):
        ...
        for symbol in self.test_symbols:
            ...
            success, data, response_time = self.make_request(
                "POST", "agents/investment-scoring", params={"symbol": symbol}
            )
            ...
            if success:
                ...
                self.log_result(
                    f"Investment Scoring Agent ({symbol})", False, data, response_time
                )
        ...
        success, data, response_time = self.make_request(
            "GET", "agents/investment-scoring/batch", params={"symbols": batch_symbols}
        )
        ...
        if success:
            ...
            self.log_result(
                "Investment Scoring Batch Processing", True, details, response_time
            )
        else:
            self.log_result(
                "Investment Scoring Batch Processing", False, data, response_time
            )

        # Test methodology endpoint
        print("\n📊 Testing Investment Scoring Agent - Methodology")
        success, data, response_time = self.make_request(
            "GET", "agents/investment-scoring/methodology"
        )
        ...
        if success:
            ...
            self.log_result(
                "Investment Scoring Methodology", True, details, response_time
            )
        else:
            self.log_result(
                "Investment Scoring Methodology", False, data, response_time
            )
    ...
    def test_technical_analysis_agent_endpoints(self):
        ...
        for symbol in self.test_symbols:
            ...
            if success:
                ...
                if not missing_fields:
                    ...
                    self.log_result(
                        f"Technical Analysis Agent ({symbol})",
                        True,
                        details,
                        response_time,
                    )
                ...
                self.log_result(
                    f"Technical Analysis Agent ({symbol})", False, data, response_time
                )
        ...
        success, data, response_time = self.make_request(
            "GET", "agents/technical-analysis/methodology"
        )
        ...
        if success:
            ...
            self.log_result(
                "Technical Analysis Methodology", True, details, response_time
            )
        else:
            self.log_result(
                "Technical Analysis Methodology", False, data, response_time
            )
    ...
    def test_pricing_data_sources_3tier(self):
        ...
        for symbol in self.test_symbols:
            ...
            success, data, response_time = self.make_request(
                "GET", f"data-sources/test/{symbol}"
            )
            ...
            if success:
                ...
                self.log_result(
                    f"Data Source Comparison ({symbol})", True, details, response_time
                )
                pricing_results[symbol] = {
                    "success": True,
                ...
                self.log_result(
                    f"Data Source Comparison ({symbol})", False, data, response_time
                )
    ...
    def test_combined_analysis_response_times(self):
        ...
        for symbol in self.test_symbols:
            ...
            if inv_success and tech_success:
                ...
                self.log_result(
                    f"Combined Analysis ({symbol})", True, details, total_time
                )
    ...
    def test_error_handling_invalid_symbols(self):
        ...
        for symbol in invalid_symbols:
            ...
            tech_success, tech_data, tech_time = self.make_request(
                "POST", "agents/technical-analysis", params={"symbol": symbol}
            )

            # Check if errors are handled gracefully
            inv_handled = inv_success and (
                "error" in inv_data or inv_data.get("investment_score", -1) == 0
            )
            tech_handled = tech_success and (
                "error" in tech_data or tech_data.get("technical_score", -1) == 0
            )
            ...
            self.log_result(
                f"Error Handling ({display_symbol})",
                overall_success,
                details,
                inv_time + tech_time,
            )
    ...
    def generate_review_report(self):
        ...
        inv_working = sum(
            1 for r in investment_results.values() if r.get("success", False)
        )
        tech_working = sum(
            1 for r in technical_results.values() if r.get("success", False)
        )

        if inv_working >= 3 and tech_working >= 3:
            print(
                f"   ✅ PASS: Investment Scoring ({inv_working}/4) and Technical Analysis ({tech_working}/4) agents working"
            )
        else:
            print(
                f"   ❌ FAIL: Investment Scoring ({inv_working}/4) or Technical Analysis ({tech_working}/4) agents have issues"
            )

        # Review Requirement 2: 3-tier pricing data sources
        print("\n2️⃣ 3-TIER PRICING DATA SOURCE PRIORITY SYSTEM:")
        pricing_working = sum(
            1 for r in pricing_results.values() if r.get("success", False)
        )

        if pricing_working >= 3:
            print(
                f"   ✅ PASS: 3-tier pricing system verified for {pricing_working}/4 symbols"
            )
        else:
            print(
                f"   ❌ FAIL: 3-tier pricing system issues detected ({pricing_working}/4 symbols working)"
            )

        # Review Requirement 3: Data structure for frontend modal
        print("\n3️⃣ DATA STRUCTURE COMPATIBILITY WITH FRONTEND MODAL:")
        modal_ready = sum(
            1 for r in combined_results.values() if r.get("modal_ready", False)
        )

        if modal_ready >= 3:
            print(
                f"   ✅ PASS: Data structure compatible with frontend modal for {modal_ready}/4 symbols"
            )
        else:
            print(
                f"   ❌ FAIL: Data structure compatibility issues ({modal_ready}/4 symbols ready)"
            )
        ...
        if symbols_tested == 4:
            print("   ✅ PASS: All 4 requested symbols tested (AAPL, MSFT, NVDA, TSLA)")
        else:
        ...
        fast_responses = sum(
            ...
            if r.get("success") and r.get("total_time", 999) < 60
        )
        ...
        if fast_responses >= 3:
            avg_time = sum(
                ...
                for r in combined_results.values()
                if r.get("success")
            ) / max(1, len([r for r in combined_results.values() if r.get("success")]))
            print(
                f"   ✅ PASS: Response times acceptable ({fast_responses}/4 under 60s, avg: {avg_time:.1f}s)"
            )
        else:
        ...
        if complete_responses >= 3:
            print(
                f"   ✅ PASS: All required fields present for {complete_responses}/4 symbols"
            )
        else:
            print(
                f"   ❌ FAIL: Missing required fields for some symbols ({complete_responses}/4 complete)"
            )
        ...
        print(
            f"   - Review Requirements Met: {requirements_passed}/6 ({requirements_success_rate:.1f}%)"
        )

        print("\n📋 DETAILED RESULTS BY SYMBOL:")
        for symbol in self.test_symbols:
            inv_status = (
                "✅" if investment_results.get(symbol, {}).get("success") else "❌"
            )
            tech_status = (
                "✅" if technical_results.get(symbol, {}).get("success") else "❌"
            )
            pricing_status = (
                "✅" if pricing_results.get(symbol, {}).get("success") else "❌"
            )
            combined_status = (
                "✅" if combined_results.get(symbol, {}).get("success") else "❌"
            )

            print(
                f"   {symbol}: Inv{inv_status} Tech{tech_status} Pricing{pricing_status} Combined{combined_status}"
            )

        print("\n🎯 FINAL VERDICT:")
        if requirements_success_rate >= 85:
            print(
                "   🎉 EXCELLENT: Investment Scoring improvements are working perfectly!"
            )
            print("   ✅ All major review requirements satisfied")
            print(
                "   🚀 Backend endpoints ready for production use with new frontend modal"
            )
            verdict = "EXCELLENT"
        elif requirements_success_rate >= 70:
            print("   ✅ GOOD: Investment Scoring improvements mostly working")
            print("   🔧 Minor issues detected but core functionality is solid")
            print(
                "   📝 Some areas may need attention before full production deployment"
            )
            verdict = "GOOD"
        else:
            print(
                "   ❌ NEEDS ATTENTION: Investment Scoring improvements have significant issues"
            )
            print("   🚨 Major problems detected that need to be addressed")
            print("   🔧 Backend endpoints require fixes before production use")
            verdict = "NEEDS_ATTENTION"

        # Summary for main agent
        print("\n💡 RECOMMENDATIONS FOR MAIN AGENT:")
        if verdict == "EXCELLENT":
            print("   ✅ Investment Scoring improvements are working perfectly")
            print("   ✅ All backend endpoints tested successfully")
            print("   ✅ 3-tier pricing system operational")
            print("   ✅ Frontend modal compatibility confirmed")
            print("   ✅ Response times acceptable for production use")
        elif verdict == "GOOD":
            print("   ✅ Core Investment Scoring functionality working")
            print("   ⚠️  Some minor issues detected - review individual test results")
            print("   📝 Consider optimizing response times if needed")
        else:
            print(
                "   ❌ Significant issues detected with Investment Scoring improvements"
            )
            print("   🔧 Review failed tests and fix backend endpoint issues")
            print("   📊 Check data source configuration and API connectivity")
...
def main():
    ...
    print(
        "   1. Backend endpoints for Investment Scoring Agent and Technical Analysis Agent"
    )
    print(
        "   2. 3-tier pricing data sources (TradeStation → Unusual Whales → Yahoo Finance)"
    )
