...
def test_unusual_whales_endpoints():
    ...
    try:
        response = requests.get(
            f"{base_url}/unusual-whales/dark-pool/recent", timeout=30
        )
        print(f"   Status: {response.status_code}")
        if response.status_code == 200:
            data = response.json()
        ...
        if response.status_code == 200:
            ...
            if "analysis" in data:
                ...
                if "summary" in analysis:
                    ...
                    print(
                        f"   👥 Representatives: {summary.get('unique_representatives', 0)}"
                    )
        else:
            print(f"   ❌ Error: {response.text}")
    except Exception as e:
    ...
    try:
        response = requests.get(
            f"{base_url}/unusual-whales/trading-strategies", timeout=30
        )
        print(f"   Status: {response.status_code}")
        if response.status_code == 200:
            data = response.json()
            ...
            if strategies:
                ...
                print(
                    f"   💡 Top Strategy: {first_strategy.get('strategy_name', 'N/A')}"
                )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"   🔮 Market Sentiment: {market_outlook.get('overall_sentiment', 'unknown')}"
            )
