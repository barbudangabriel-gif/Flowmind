...
class InvestmentScoringTester:
 ...
 investment_features = [
 f for f in features if "Investment" in f or "Scoring" in f
 ]
 ...
 print(
 f" #{i+1}: {pick.get('symbol', 'N/A')} - {pick.get('total_score', 'N/A')} - {pick.get('rating', 'N/A')}"
 )
 ...
 print(
 f" Tech #{i+1}: {leader.get('symbol', 'N/A')} - {leader.get('total_score', 'N/A')}"
 )
 ...
 print(
 f" Example: {first_stock.get('symbol', 'N/A')} - Score: {first_stock.get('total_score', 'N/A')}"
 )
...
def main():
 ...
 print(f" {failed_tests} Investment Scoring tests failed")
 return 1
