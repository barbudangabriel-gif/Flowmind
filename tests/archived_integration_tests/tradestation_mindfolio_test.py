...
class TradeStationMindfolioTester:
 ...
 def test_tradestation_mindfolio_dropdown_functionality(self):
 """Test TradeStation Live Mindfolio backend endpoints for dropdown functionality"""
 print("\n🏛️ TESTING TRADESTATION LIVE MINDFOLIO - DROPDOWN FUNCTIONALITY")
 print("=" * 80)
 print(
 " OBJECTIVE: Verify backend data structure supports frontend dropdown grouping"
 )
 print("🔧 CRITICAL FOCUS:")
 print(
 " 1. Test /api/tradestation/accounts/11775499/summary for mindfolio positions"
 )
 print(
 " 2. Verify positions include both stocks and options with correct asset_type field"
 )
 print(
 " 3. Check for position data that should be grouped (stocks with related options)"
 )
 print(
 " 4. Test specific symbols for dropdown functionality (IBM, OSCR, GOOGL)"
 )
 ...
 print(
 f" Authentication Status: {' AUTHENTICATED' if authenticated else ' NOT AUTHENTICATED'}"
 )
 print(f" Environment: {environment}")

 if not authenticated:
 print(
 " WARNING: Not authenticated - mindfolio data may be limited or mock"
 )
 print(
 " NOTE: Testing will continue but may show authentication errors"
 )

 # Test 2: Mindfolio Summary Endpoint - CRITICAL TEST
 print("\n PHASE 2: Mindfolio Summary Endpoint (CRITICAL)")
 print("-" * 60)

 success, mindfolio_data = self.run_test(
 f"Mindfolio Summary (Account {self.account_id})",
 "GET",
 ...
 if not success:
 print(
 " Mindfolio Summary endpoint failed - cannot test dropdown functionality"
 )
 return False
 ...
 if "data" in mindfolio_data:
 print(" Data is nested under 'data' field")
 actual_data = mindfolio_data["data"]
 print(
 " Frontend expects: {mindfolio_metrics, positions, risk_analysis}"
 )
 print(
 " Backend returns: {status, data: {mindfolio_metrics, positions, risk_analysis}}"
 )
 print(
 " 🔧 STRUCTURE MISMATCH CONFIRMED - This explains the loading spinner issue!"
 )
 else:
 print(" Data is at root level (frontend compatible)")
 actual_data = mindfolio_data

 # Extract key components
 mindfolio_metrics = actual_data.get("mindfolio_metrics", {})
 positions = actual_data.get("positions", [])
 risk_analysis = actual_data.get("risk_analysis", {})

 print(
 f" Mindfolio Metrics: {' Present' if mindfolio_metrics else ' Missing'}"
 )
 print(
 f" Positions Array: {' Present' if positions else ' Missing'} ({len(positions)} positions)"
 )
 print(f" Risk Analysis: {' Present' if risk_analysis else ' Missing'}")

 if mindfolio_metrics:
 total_value = mindfolio_metrics.get("total_value", 0)
 total_pnl = mindfolio_metrics.get("total_pnl", 0)
 print(f" Total Mindfolio Value: ${total_value:,.2f}")
 ...
 print(" Groupable Positions Analysis:")
 dropdown_candidates = []
 ...
 print(
 f" {base_symbol}: {len(symbol_positions)} positions (DROPDOWN CANDIDATE)"
 )
 ...
 print(
 f" - User Reported Symbols Found: {[s for s in target_symbols if s in dropdown_candidates]}"
 )
 ...
 option_positions[:10]
 ): # Analyze first 10 options
 symbol = option.get("symbol", "N/A")
 asset_type = option.get(
 "asset_type", option.get("AssetType", "UNKNOWN")
 )
 ...
 print(
 f" - Type: {'Call' if call_put == 'C' else 'Put' if call_put == 'P' else 'Unknown'}"
 )
 ...
 "Data nested under 'data' field - frontend expects root level"
 )
 ...
 if (
 field not in sample_position
 and field.title() not in sample_position
 ):
 ...
 unexpected_types = [
 at for at in asset_types.keys() if at not in expected_asset_types
 ]

 if unexpected_types:
 compatibility_issues.append(
 f"Unexpected asset types: {unexpected_types}"
 )
 ...
 symbol_positions = [
 p for p in positions if p.get("symbol", "").startswith(symbol)
 ]
 ...
 if p.get("asset_type", "").upper() in ["STOCK", "EQUITY"]
 ]
 option_pos = [
 p
 for p in symbol_positions
 if "OPTION" in p.get("asset_type", "").upper()
 ]
 ...
 print(" DROPDOWN SHOULD WORK: Has both stock and options")
 elif len(option_pos) > 1:
 print(" DROPDOWN SHOULD WORK: Multiple option contracts")
 else:
 print(" DROPDOWN NOT NEEDED: Only single position type")
 ...
 quantity = pos.get("quantity", pos.get("Quantity", 0))
 print(
 f" - Position {i+1}: {symbol_detail} ({asset_type}, Qty: {quantity})"
 )
 else:
 ...
 "Mindfolio Summary (Performance Test)",
 "GET",
 f"tradestation/accounts/{self.account_id}/summary",
 200,
 )
 ...
 print(" Slow response time - may affect user experience")

 # Final Assessment
 print("\n FINAL ASSESSMENT: TradeStation Mindfolio Dropdown Functionality")
 print("=" * 80)

 # Calculate success metrics
 test_phases = [
 ("Authentication Check", success),
 ("Mindfolio Data Retrieval", success and len(positions) > 0),
 ...
 "User Symbol Testing",
 any(
 symbol in [p.get("symbol", "") for p in positions]
 for symbol in user_symbols
 ),
 ),
 ...
 print(
 f" - Dropdown Candidates: {len(dropdown_candidates)} ({dropdown_candidates})"
 )
 print(f" - Stock Positions: {len(stock_positions)}")
 print(f" - Option Positions: {len(option_positions)}")
 print(
 ...
 if "data" in mindfolio_data:
 print(" 🚨 CRITICAL ISSUE CONFIRMED: Data structure mismatch")
 print(
 " - Backend returns: {status, data: {mindfolio_metrics, positions, risk_analysis}}"
 )
 print(
 ...
 print(
 " - SOLUTION: Frontend needs to access response.data.* instead of response.*"
 )

 if len(dropdown_candidates) > 0:
 print(
 f" DROPDOWN DATA AVAILABLE: {len(dropdown_candidates)} symbols have multiple positions"
 )
 print(f" - Symbols with dropdowns: {dropdown_candidates}")
 else:
 print(" NO DROPDOWN CANDIDATES: No symbols have multiple positions")

 if not authenticated:
 print(" AUTHENTICATION ISSUE: May be showing mock/limited data")

 # Recommendations
 print("\n RECOMMENDATIONS:")

 if "data" in mindfolio_data:
 print(" 1. 🔧 CRITICAL: Fix frontend data access pattern")
 print(" - Change: response.mindfolio_metrics")
 print(" - To: response.data.mindfolio_metrics")
 print(" - This will resolve the permanent loading spinner issue")

 if len(dropdown_candidates) > 0:
 print(
 " 2. DROPDOWN LOGIC: Backend data supports dropdown functionality"
 )
 print(
 " - Group positions by base symbol (before space or option details)"
 )
 print(
 " - Show dropdown arrow when multiple positions exist for same base symbol"
 )

 if len(option_positions) > 0:
 print(" 3. OPTION PARSING: Option symbols follow standard format")
 print(" - Format: 'BASE_SYMBOL YYMMDDCPPPPPPP'")
 print(" - Extract base symbol by splitting on space")

 # Final verdict
 if success_rate >= 85:
 print("\n VERDICT: BACKEND DATA STRUCTURE IS CORRECT")
 print(
 " The backend provides all necessary data for dropdown functionality."
 )
 print(" The user issue is likely a FRONTEND DATA ACCESS problem.")
 print(
 " Fix: Update frontend to access response.data.* instead of response.*"
 )
 elif success_rate >= 70:
 print("\n VERDICT: MOSTLY WORKING - Minor issues detected")
 print(" Backend data is mostly correct but may need minor adjustments.")
 else:
 print("\n VERDICT: SIGNIFICANT ISSUES - Backend data structure problems")
 print(
 " Backend data structure needs major fixes for dropdown functionality."
 )

 return success_rate >= 70

 def run_all_tests(self):
 """Run all TradeStation mindfolio tests"""
 print("🏛️ TRADESTATION LIVE MINDFOLIO BACKEND TESTING")
 print("=" * 80)
 print(" FOCUS: Verify data structure for dropdown functionality")
 print(
 "🔧 USER ISSUE: Dropdown arrows appear but don't expand to show option contracts"
 )

 # Run the comprehensive dropdown functionality test
 success = self.test_tradestation_mindfolio_dropdown_functionality()
...
if __name__ == "__main__":
 tester = TradeStationMindfolioTester()
 success = tester.run_all_tests()

 if success:
 print(
 "\n TESTING COMPLETE: TradeStation Mindfolio backend data structure verified"
 )
 sys.exit(0)
 else:
 print("\n TESTING FAILED: Issues found with TradeStation Mindfolio backend")
 sys.exit(1)
