...
class EnhancedTradingStrategiesTest:
 ...
 if success:
 self.log_test(
 "Trading Strategies Endpoint Accessible",
 True,
 ...
 self.log_test(
 "Specific Strategy Names Present",
 True,
 ...
 self.log_test(
 "Non-Generic Strategy Names",
 not has_generic,
 "All strategy names are specific options strategies"
 if not has_generic
 ...
 if not strategies_data:
 self.log_test(
 "Strategy Details Structure", False, "No strategies to verify"
 )
 return
 ...
 self.log_test(
 f"Required Fields - {strategy_name}",
 True,
 ...
 self.log_test(
 f"Leg {j+1} Details - {strategy_name}",
 True,
 ...
 self.log_test(
 f"Leg {j+1} Structure - {strategy_name}",
 False,
 ...
 self.log_test(
 f"DTE Calculation - {strategy_name}", True, f"DTE: {dte} days"
 )
 else:
 self.log_test(
 f"DTE Calculation - {strategy_name}",
 False,
 ...
 self.log_test(
 f"Risk/Profit Calculations - {strategy_name}",
 has_risk_calcs,
 f"Max Risk: {max_risk}, Max Profit: {max_profit}, Breakeven: {breakeven}"
 if has_risk_calcs
 ...
 self.log_test(
 f"Confidence Score - {strategy_name}",
 True,
 ...
 self.log_test(
 f"TradeStation Execution - {strategy_name}",
 False,
 ...
 ts_missing = [
 field for field in ts_required_fields if field not in tradestation_exec
 ]

 if not ts_missing:
 self.log_test(
 f"TradeStation Structure - {strategy_name}",
 True,
 ...
 self.log_test(
 f"Underlying Symbol - {strategy_name}",
 True,
 ...
 if all(
 field in leg
 for field in ["action", "strike", "option_type", "quantity"]
 ):
 valid_legs += 1

 if valid_legs == len(legs):
 self.log_test(
 f"Leg Structure - {strategy_name}",
 True,
 ...
 if not strategies_data:
 self.log_test(
 "Strategy Categorization", False, "No strategies to categorize"
 )
 return
 ...
 category_counts[strategy_type] = (
 category_counts.get(strategy_type, 0) + 1
 )

 print("Found strategy categories:")
 for category, count in category_counts.items():
 print(f" • {category}: {count} strategies")

 # Check for expected categories
 matching_categories = found_categories.intersection(
 set(self.expected_categories)
 )
 ...
 short_term = any(
 "day" in tf.lower() or "week" in tf.lower() for tf in timeframes
 )
 if not short_term:
 ...
 long_term = any(
 "month" in tf.lower() or "year" in tf.lower() for tf in timeframes
 )
 if not long_term:
 appropriate_timeframes = False
 ...
 appropriate_timeframes,
 "Strategy timeframes match strategy types"
 ...
 True,
 "Unusual Whales endpoints available, API key likely configured",
 )
 else:
 self.log_test(
 "API Key Configuration",
 ...
 self.log_test(
 "API Key Configuration", False, "Could not verify API key configuration"
 )
 ...
 if (
 isinstance(premium, (int, float)) and premium > 10000
 ): # Real premiums are usually substantial
 ...
 if not strategies:
 print(
 f"\n No strategies returned. Full response: {json.dumps(data, indent=2)}"
 )
 strategies = []
 ...
 print("\n" + "=" * 70)
 print(" Enhanced Trading Strategies Test Results")
 ...
 print(
 " All tests passed! Enhanced AI Trading Strategies are working correctly."
 )
 return True
 else:
 failed_tests = self.tests_run - self.tests_passed
 print(f" {failed_tests} tests failed. Review the issues above.")
 return False
