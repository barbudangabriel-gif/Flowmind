...
class MindfolioManagementTester:
 ...
 def test_mindfolio_management_with_tradestation_integration(self):
 """Test Mindfolio Management Service with real TradeStation integration as requested in review"""
 print(
 "\n MINDFOLIO MANAGEMENT SERVICE WITH TRADESTATION INTEGRATION - COMPREHENSIVE TESTING"
 )
 print("=" * 100)
 print(
 " OBJECTIVE: Test updated Mindfolio Management Service with real TradeStation integration"
 )
 print(" REVIEW REQUIREMENTS:")
 print(
 " 1. GET /api/mindfolio-management/mindfolios - Should load real TradeStation data in TradeStation Main mindfolio"
 )
 print(
 " 2. GET /api/mindfolio-management/mindfolios/tradestation-main/positions - Should show real positions from TradeStation account"
 )
 print(
 " 3. Verify positions data structure includes both stocks and options from TradeStation"
 )
 print(
 " 4. Check if mindfolio totals are calculated correctly from real TradeStation data"
 )
 ...
 print(
 " - Mindfolio totals (total_value, total_pnl, positions_count) reflecting TradeStation data"
 )
 ...
 # Test 1: GET /api/mindfolio-management/mindfolios - Should load real TradeStation data
 print(
 "\n PHASE 1: Get All Mindfolios - TradeStation Main Mindfolio Integration"
 )
 print("-" * 80)

 success, mindfolios_data = self.run_test(
 "Get All Mindfolios with TradeStation Integration",
 "GET",
 "mindfolio-management/mindfolios",
 200,
 )

 if not success:
 print(" CRITICAL: Mindfolio Management mindfolios endpoint failed")
 return False

 # Analyze mindfolios response
 mindfolios = mindfolios_data.get("mindfolios", [])
 print(f" Total Mindfolios Found: {len(mindfolios)}")

 # Find TradeStation Main mindfolio
 tradestation_main = None
 for mindfolio in mindfolios:
 if (
 mindfolio.get("id") == "tradestation-main"
 or "tradestation" in mindfolio.get("name", "").lower()
 ):
 tradestation_main = mindfolio
 break

 if tradestation_main:
 print(" TradeStation Main Mindfolio Found:")
 ...
 print(f" Using Fallback/Mock Data: {data_source}")
 ...
 print(
 f" Position Count Lower Than Expected: {positions_count} (expected ~84)"
 )
 ...
 print(" CRITICAL: TradeStation Main Mindfolio Not Found")
 print(f" Available mindfolios: {[p.get('name') for p in mindfolios]}")
 using_real_data = False
 position_count_correct = False

 # Test 2: GET /api/mindfolio-management/mindfolios/tradestation-main/positions - Real positions
 print(
 "\n PHASE 2: Get TradeStation Main Mindfolio Positions - Real Position Data"
 )
 print("-" * 80)
 ...
 print(
 f" - Mindfolio Total Value: ${mindfolio_summary.get('total_value', 0):,.2f}"
 )
 print(
 f" - Mindfolio Total P&L: ${mindfolio_summary.get('total_pnl', 0):,.2f}"
 )
 print(
 f" - Mindfolio P&L %: {mindfolio_summary.get('total_pnl_percent', 0):.2f}%"
 )
 ...
 print(" Mindfolio contains both stocks and options as expected")
 mixed_positions = True
 elif has_stocks:
 print(
 " Mindfolio contains stocks but no clear options identification"
 )
 mixed_positions = False
 elif has_options:
 print(
 " Mindfolio contains options but no clear stocks identification"
 )
 mixed_positions = False
 else:
 ...
 print(" No positions found in TradeStation Main mindfolio")
 mixed_positions = False
 has_stocks = False
 has_options = False

 # Test 4: Check if mindfolio totals are calculated correctly
 print("\n PHASE 4: Mindfolio Totals Calculation Verification")
 print("-" * 80)

 if positions and mindfolio_summary:
 ...
 reported_total_value = mindfolio_summary.get("total_value", 0)
 reported_total_pnl = mindfolio_summary.get("total_pnl", 0)

 print(" Mindfolio Totals Verification:")
 ...
 * 100
 ...
 print(" Cannot verify totals - missing positions or mindfolio summary")
 ...
 if len(positions) >= 5 and mindfolio_summary.get("total_value", 0) > 0:
 print(
 f" Fallback provides adequate data: {len(positions)} positions, ${mindfolio_summary.get('total_value', 0):,.2f} total value"
 )
 else:
 ...
 fallback_working = False
 fallback_status = "Cannot verify - TradeStation Main mindfolio not found"
 ...
 expected_mindfolio_fields = ["mindfolios"]
 expected_position_fields = ["positions", "mindfolio_summary"]

 mindfolio_structure_ok = all(
 field in mindfolios_data for field in expected_mindfolio_fields
 )
 ...
 print(
 f" - Mindfolio Response Structure: {' Compatible' if mindfolio_structure_ok else ' Issues'}"
 )
 print(
 ...
 # Final Assessment
 print(
 "\n FINAL ASSESSMENT: Mindfolio Management Service with TradeStation Integration"
 )
 print("=" * 100)

 # Calculate success metrics
 test_phases = [
 ("TradeStation Main Mindfolio Found", tradestation_main is not None),
 ...
 ("Mindfolio Totals Calculated Correctly", totals_correct),
 ...
 requirements = [
 (
 "GET /api/mindfolio-management/mindfolios loads real TradeStation data",
 tradestation_main is not None and using_real_data,
 ),
 (
 "GET /api/mindfolio-management/mindfolios/tradestation-main/positions shows real positions",
 ...
 (
 "Mindfolio totals calculated correctly from real TradeStation data",
 totals_correct,
 ),
 ...
 print(" - TradeStation Main Mindfolio: Not Found")
 ...
 print(
 "\n VERDICT: EXCELLENT - Mindfolio Management Service with TradeStation integration working perfectly!"
 )
 print(
 " Real TradeStation account data (84 positions: 19 stocks, 65 options) is being loaded"
 )
 print(" Mindfolio totals reflect TradeStation data accurately")
 print(" Data structure is compatible with existing frontend")
 print(
 " Move to Mindfolio X functionality can work with real TradeStation data"
 )
 ...
 print(" Some TradeStation integration aspects may need attention")
 else:
 print(
 ...
 print(
 " STARTING MINDFOLIO MANAGEMENT SERVICE TESTING WITH TRADESTATION INTEGRATION"
 )
 print("=" * 100)
 ...
 success = self.test_mindfolio_management_with_tradestation_integration()
