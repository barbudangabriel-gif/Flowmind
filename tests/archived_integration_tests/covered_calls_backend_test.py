...
class CoveredCallsBackendTester:
 ...
 def log_result(
 self, test_name: str, success: bool, details: str, is_critical: bool = False
 ):
 ...
 print("=" * 60)
 print(
 "Scenario: shares_owned=300, open_cc_contracts=1, candidate_call={strike: 250, delta: 0.2, dte: 30, premium: 2.5}"
 )
 ...
 sell_call_signals = [
 s for s in cc_signals if s.get("signal") == "SELL CALL"
 ]
 ...
 missing_fields = [
 field for field in required_fields if field not in sell_call
 ]
 ...
 self.log_result(
 "SELL CALL Candidate - Exception", False, f"Error: {str(e)}", True
 )
 return False
 ...
 print("=" * 60)
 print(
 "Scenario: open_cc_state {delta: 0.4, dte: 8, premium_sold: 3.0, premium_mark: 2.2}"
 )
 ...
 take_profit_signals = [
 s for s in cc_signals if s.get("signal") == "TAKE PROFIT"
 ]
 ...
 print(
 f" - Profit ratio (2.2/3.0 = {profit_ratio:.3f}): {profit_ratio:.3f}"
 )
 print(
 f" - Take profit condition ({profit_ratio:.3f} <= 0.30): {profit_condition}"
 )
 ...
 True,
 "ROLL CC signal correctly generated (conditions met)",
 )
 ...
 True,
 "TAKE PROFIT signal correctly generated",
 )
 else:
 ...
 self.log_result(
 "ROLL CC and TAKE PROFIT - Exception", False, f"Error: {str(e)}", True
 )
 return False
 ...
 f"{self.api_url}/options/selling/monitor/status", timeout=15
 )
 ...
 cc_signals_in_current = [
 s
 for s in signals_current
 if s.get("signal") in ["SELL CALL", "ROLL CC", "TAKE PROFIT"]
 ]
 ...
 self.log_result(
 "Monitor Stop CC - Stopped", True, "Monitor stopped successfully"
 )
 ...
 self.log_result(
 "Monitor Start CC - Exception", False, f"Error: {str(e)}", True
 )
 return False
 ...
 True,
 "Monitor started without CC inputs",
 )
 ...
 True,
 "Monitor running without CC inputs",
 )
 else:
 self.log_result(
 "Backward Compatibility Monitor - Status",
 ...
 True,
 "Both compute and monitor work without CC inputs",
 )
 ...
 self.log_result(
 "Backward Compatibility - Exception", False, f"Error: {str(e)}", True
 )
 return False
 ...
 print(
 "Scenarios: SELL CALL candidates, ROLL CC, TAKE PROFIT, Monitor integration, Backward compatibility"
 )
 print("=" * 80)
 ...
 critical_phases = [
 "SELL CALL Candidate",
 "ROLL CC and TAKE PROFIT",
 "Monitor with CC Inputs",
 ]
 critical_failures = [
 name
 ...
 print(
 " SELL CALL candidates processed correctly with proper contract calculations"
 )
