...
class EnhancedMindfolioTester:
 ...
 print(" Failed - Request timeout (15s)")
 return False, {}
 except Exception as e:
 print(f" Failed - Error: {str(e)}")
 return False, {}

 def test_enhanced_mindfolio_features(self):
 """Test enhanced mindfolio features with TradeStation-inspired functionality"""
 print(" Testing Enhanced Mindfolio Management API")

 # Test 1: Get empty mindfolio
 success, mindfolio_data = self.run_test(
 "Get Mindfolio (Empty)", "GET", "mindfolio", 200
 )
 if success:
 print(f" Mindfolio structure: {list(mindfolio_data.keys())}")

 # Check for enhanced mindfolio summary structure
 required_fields = [
 "total_value",
 "total_cost",
 "total_profit_loss",
 ...
 missing_fields = [
 field for field in required_fields if field not in mindfolio_data
 ]
 if missing_fields:
 print(f" Missing mindfolio summary fields: {missing_fields}")
 else:
 print(" All required mindfolio summary fields present")
 ...
 "Add AAPL Position", "POST", "mindfolio", 200, data=aapl_position
 )
 aapl_id = None
 ...
 missing_enhanced = [
 field for field in enhanced_fields if field not in add_response
 ]
 if missing_enhanced:
 print(f" Missing enhanced position fields: {missing_enhanced}")
 else:
 print(" All enhanced position fields present")
 print(f" Current Price: ${add_response.get('current_price', 0):.2f}")
 print(f" Current Value: ${add_response.get('current_value', 0):.2f}")
 print(
 f" P&L: ${add_response.get('profit_loss', 0):.2f} ({add_response.get('profit_loss_percent', 0):.2f}%)"
 )
 ...
 success, msft_response = self.run_test(
 "Add MSFT Position", "POST", "mindfolio", 200, data=msft_position
 )
 msft_id = None
 ...
 # Test 4: Get mindfolio with positions and enhanced calculations
 success, full_mindfolio = self.run_test(
 "Get Mindfolio (With Positions)", "GET", "mindfolio", 200
 )
 if success:
 print(f" Mindfolio Items: {len(full_mindfolio.get('items', []))}")
 print(f" Total Value: ${full_mindfolio.get('total_value', 0):.2f}")
 print(f" Total Cost: ${full_mindfolio.get('total_cost', 0):.2f}")
 print(
 f" Total P&L: ${full_mindfolio.get('total_profit_loss', 0):.2f} ({full_mindfolio.get('total_profit_loss_percent', 0):.2f}%)"
 )

 # Verify each position has real-time data
 for item in full_mindfolio.get("items", []):
 ...
 if aapl_id:
 self.run_test("Delete AAPL Position", "DELETE", f"mindfolio/{aapl_id}", 200)

 if msft_id:
 self.run_test("Delete MSFT Position", "DELETE", f"mindfolio/{msft_id}", 200)

 # Test 6: Verify mindfolio is empty again
 self.run_test("Get Mindfolio (After Cleanup)", "GET", "mindfolio", 200)
 ...
 "Market Overview", "GET", "market/overview", 200
 )
 ...
 # Test top movers
 success, movers_data = self.run_test(
 "Top Movers", "GET", "market/top-movers", 200
 )
 if success:
 ...
 print(
 f" #1 Gainer: {top_gainer.get('symbol', 'N/A')} (+{top_gainer.get('change_percent', 0):.2f}%)"
 )

 if losers:
 top_loser = losers[0]
 print(
 f" #1 Loser: {top_loser.get('symbol', 'N/A')} ({top_loser.get('change_percent', 0):.2f}%)"
 )
 ...
 success, msft_score = self.run_test(
 "Investment Score (MSFT)", "GET", "investments/score/MSFT", 200
 )
 if success:
 ...
 try:
 success, top_picks = self.run_test(
 ...
 print(
 f" #1 Pick: {top_pick.get('symbol', 'N/A')} - Score: {top_pick.get('total_score', 'N/A')}"
 )
 except:
 print(" Top picks endpoint may have timeout issues")

 # Test sector leaders
 success, sector_leaders = self.run_test(
 "Sector Leaders",
 "GET",
 ...
 success, aapl_enhanced = self.run_test(
 ...
 "screener/data",
 200,
 params={"limit": 10, "exchange": "sp500"},
 )
 ...
 print(
 f" Sample: {sample_stock.get('symbol', 'N/A')} - ${sample_stock.get('price', 0):.2f}"
 )

def main():
 print(" Enhanced Mindfolio & Market Data API Tests")
 print("=" * 60)

 tester = EnhancedMindfolioTester()

 # Test enhanced mindfolio features (main focus)
 tester.test_enhanced_mindfolio_features()
