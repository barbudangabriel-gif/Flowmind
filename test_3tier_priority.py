...
try:
 ...
 data = response.json()
 print(
 f" Status endpoint working (200 OK, {response.elapsed.total_seconds():.2f}s)"
 )
 ...
 print(" TradeStation correctly at priority 1")
 ...
 print(
 " Unusual Whales correctly at priority 2 (primary fallback)"
 )
 elif "Yahoo Finance" in name and rank == 3:
 print(" Yahoo Finance correctly at priority 3 (final fallback)")
...
try:
 ...
 data = response.json()
 print(
 f" CRM comparison working (200 OK, {response.elapsed.total_seconds():.2f}s)"
 )
 ...
 print(" Testing results for all 3 sources:")
 ...
 print(" - Not authenticated (expected if not logged in)")
 else:
...
try:
 ...
 data = response.json()
 print(
 f" Basic stock endpoint working (200 OK, {response.elapsed.total_seconds():.2f}s)"
 )

 price = data.get("price", 0)
 data_source = data.get("data_source", "Not specified")

 print(" CRM Basic Stock Data:")
 print(f" - Price: ${price:.2f}")
 print(f" - Data Source: {data_source}")

 # Check data source attribution
 if "TradeStation API (Primary)" in data_source:
 print(" Using TradeStation as primary source")
 elif "Unusual Whales (Fallback)" in data_source:
 print(" Using Unusual Whales as fallback source")
 elif "Yahoo Finance (Final Fallback)" in data_source:
 print(" Using Yahoo Finance as final fallback")
 else:
 print(f" Data source attribution: {data_source}")
 else:
...
try:
 ...
 print(" Enhanced endpoint using TradeStation integration")
 elif "Unusual Whales" in data_source:
 print(" Enhanced endpoint using Unusual Whales fallback")
 elif "Yahoo Finance" in data_source:
 print(" Enhanced endpoint using Yahoo Finance fallback")
 else:
 print(f" Enhanced data source: {data_source}")
 else:
