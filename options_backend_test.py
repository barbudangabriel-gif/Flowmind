...
class OptionsCalculatorTester:
        ...
        for test_case in test_cases:
            try:
                ...
                if response.status_code == 200:
                    ...
                    expected_min, expected_max = test_case.get(
                        "expected_premium_range", (0, 1000)
                    )

                    premium_valid = expected_min <= premium <= expected_max

                    # Verify Greeks calculations
                    strategy_greeks = analysis.get(
                        "greeks", {}
                    )  # Changed from 'strategy_greeks' to 'greeks'
                    required_greeks = ["delta", "gamma", "theta", "vega", "rho"]
                    greeks_present = all(
                        greek in strategy_greeks for greek in required_greeks
                    )

                    # Verify P&L arrays for charting
                    price_array = chart_data.get(
                        "x", []
                    )  # Changed from analysis to chart_data
                    pnl_array = chart_data.get(
                        "y", []
                    )  # Changed from analysis to chart_data
                    ...
                    checks = [
                        ...
                        (
                            "Premium Range",
                            premium_valid,
                            f"Premium ${premium:.2f} in range ${expected_min}-${expected_max}",
                        ),
                        (
                            "Greeks Present",
                            greeks_present,
                            f"Greeks: {list(strategy_greeks.keys())}",
                        ),
                        (
                            "P&L Arrays",
                            len(price_array) > 0 and len(pnl_array) > 0,
                            f"Price points: {len(price_array)}, P&L points: {len(pnl_array)}",
                        ),
                        (
                            "Breakeven Calculation",
                            isinstance(breakeven_points, list),
                            f"Breakeven points: {breakeven_points}",
                        ),
                    ]
                    ...
                    if all_checks_passed:
                        ...
                        self.log_result(
                            f"Options Calculate - {test_case['name']}", True, details
                        )
                        ...
                        greeks_reasonable = (
                            -1 <= delta <= 1  # Delta should be between -1 and 1
                            and gamma >= 0  # Gamma should be positive
                            and theta <= 0  # Theta should be negative (time decay)
                            and vega >= 0  # Vega should be positive
                        )

                        if greeks_reasonable:
                            print("     ✅ Greeks within reasonable ranges")
                        else:
                            print("     ⚠️  Some Greeks outside expected ranges")
    ...
    def test_options_strategies_endpoint(self):
        ...
        try:
            start_time = time.time()
            response = requests.get(f"{self.api_url}/options/strategies", timeout=15)
            response_time = time.time() - start_time
            ...
            if response.status_code == 200:
                ...
                key_strategies_present = [
                    s for s in key_strategies if s in strategies_found
                ]
                ...
                if all_passed:
                    ...
                    failed_checks = [
                        name for name, passed in success_checks if not passed
                    ]
                    self.log_result(
            ...
            self.log_result(
                "Options Strategies Endpoint", False, f"Error: {str(e)}", True
            )
            return False
    ...
    def test_mathematical_accuracy(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                validation_checks = [
                    ...
                    (
                        "Delta ATM Call",
                        0.4 <= delta <= 0.6,
                        f"ATM Call Delta: {delta:.4f} (expected ~0.5)",
                    ),
                    ...
                    (
                        "Rho Positive Call",
                        rho > 0,
                        f"Rho: {rho:.4f} (call should have positive rho)",
                    ),
                ]

                passed_validations = sum(
                    1 for _, passed, _ in validation_checks if passed
                )
                ...
                if passed_validations >= total_validations - 1:  # Allow 1 failure
                    ...
                    failed_validations = [
                        name for name, passed, _ in validation_checks if not passed
                    ]
                    self.log_result(
            ...
            self.log_result(
                "Mathematical Accuracy - Greeks", False, f"Error: {str(e)}", True
            )
            return False
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    validations = [
                        (
                            "Correct Leg Count",
                            len(legs) == strategy_test["expected_legs"],
                        ),
                    ...
                    if strategy_test.get("expected_max_profit_limited"):
                        validations.append(
                            (
                                "Limited Max Profit",
                                max_profit > 0 and max_profit < 100000,
                            )
                        )
                    ...
                    if passed_validations >= len(validations) - 1:  # Allow 1 failure
                        ...
                        failed_validations = [
                            name for name, passed in validations if not passed
                        ]
                        self.log_result(
                            f"Strategy Calculation - {strategy_test['name']}",
                            False,
    ...
    def test_error_scenarios(self):
        ...
        for test_case in error_test_cases:
            try:
                ...
                self.log_result(
                    f"Error Handling - {test_case['name']}", False, f"Error: {str(e)}"
                )
    ...
    def run_comprehensive_options_test(self):
        ...
        print("=" * 80)
        print(
            "Focus: Black-Scholes calculations, Greeks, Strategy Engine, API endpoints"
        )
        print(
            "Test Parameters: CRCL @ $149.53, Strike $95, Target $263.91, 25% vol, 5% rate"
        )
        print("=" * 80)
        ...
        critical_tests_passed = sum(
            1 for _, success in test_results[:4] if success
        )  # First 4 are critical
        overall_success = (
            critical_tests_passed >= 3 and len(self.critical_failures) == 0
        )
    ...
    def print_final_results(self, test_results):
        ...
        critical_failures = [
            ...
            if name in critical_phases and not success
        ]
