...
def final_validation_test():
 ...
 print(
 f" GET /api/unusual-whales/trading-strategies: {response.status_code}"
 )
 ...
 print(
 f" Strategies with chart field: {chart_validation_results['strategies_with_chart_field']}/{len(strategies)}"
 )
 print(
 f" Valid plotly JSON: {chart_validation_results['valid_plotly_json']}/{len(strategies)}"
 )
 print(
 f" Charts with P&L data: {chart_validation_results['charts_with_profit_loss_data']}/{len(strategies)}"
 )
 print(
 f" Charts with breakeven points: {chart_validation_results['charts_with_breakeven_points']}/{len(strategies)}"
 )
 print(
 f" Charts with max calculations: {chart_validation_results['charts_with_max_calculations']}/{len(strategies)}"
 )
 ...
 print(" Chart type mappings found:")
 ...
 if (
 breakeven_value
 and isinstance(breakeven_value, (int, float))
 and 10 < breakeven_value < 1000
 ):
 metrics_analysis["realistic_breakeven"] += 1

 # Check if underlying price is used
 if (
 "entry_logic" in strategy
 and "underlying_price" in strategy["entry_logic"]
 ):
 ...
 print(
 f" Realistic max profit calculations: {metrics_analysis['realistic_max_profit']}/{len(strategies)}"
 )
 print(
 f" Realistic max loss calculations: {metrics_analysis['realistic_max_loss']}/{len(strategies)}"
 )
 print(
 f" Realistic breakeven calculations: {metrics_analysis['realistic_breakeven']}/{len(strategies)}"
 )
 print(
 f" Underlying price used: {metrics_analysis['underlying_price_used']}/{len(strategies)}"
 )
 ...
 print(
 f" 🛡️ Charts with errors: {error_handling_results['charts_with_errors']}"
 )
 print(f" 🛡️ Fallback charts: {error_handling_results['fallback_charts']}")
 print(
 f" 🛡️ charts_included flag: {'' if error_handling_results['charts_included_flag_present'] else ''}"
 )
 ...
 if any(
 ct
 in ["directional", "vertical_spread", "volatility", "iron_condor", "income"]
 ...
 if (
 metrics_analysis["realistic_max_profit"] > 0
 or metrics_analysis["realistic_max_loss"] > 0
 ):
 passed_requirements += 1
 ...
 print("Some chart integration features need attention.")
 return False
