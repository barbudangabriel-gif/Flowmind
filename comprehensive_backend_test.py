...
class ComprehensiveAPITester:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_result(
                    "API Root Endpoint",
                    True,
                    f"Version: {version}, Features: {features}",
                )
                return True
            else:
                self.log_result(
                    "API Root Endpoint", False, f"Status: {response.status_code}", True
                )
                return False
                ...
                if len(indices) >= 4:  # Should have S&P 500, Dow, NASDAQ, Russell
                    ...
                    if len(found_symbols) >= 3 and len(real_prices) >= 3:
                        ...
                        self.log_result(
                            ...
                            False,
                            f"Missing indices or zero prices. Found: {found_symbols}",
                            True,
                        )
                        return False
                else:
                    self.log_result(
                        "Market Overview - Data Count",
                        False,
                        f"Expected 4+ indices, got {len(indices)}",
                        True,
                    )
                    return False
            else:
                self.log_result(
                    "Market Overview API", False, f"HTTP {response.status_code}", True
                )
                return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if len(gainers) >= 3 and len(losers) >= 3:
                    ...
                    real_gainers = [
                        g for g in gainers if abs(g.get("change_percent", 0)) > 0.01
                    ]
                    real_losers = [
                        l for l in losers if abs(l.get("change_percent", 0)) > 0.01
                    ]
                    ...
                    if len(real_gainers) >= 2 and len(real_losers) >= 2:
                        ...
                        self.log_result(
                            f"Top Gainer - {gainer_symbol}",
                            True,
                            f"Change: +{gainer_change:.2f}%",
                        )

                        loser_symbol = top_loser.get("symbol", "Unknown")
                        loser_change = top_loser.get("change_percent", 0)
                        self.log_result(
                            f"Top Loser - {loser_symbol}",
                            True,
                            f"Change: {loser_change:.2f}%",
                        )

                        return True
                    else:
                        self.log_result(
                            "Top Movers - Real Changes",
                            False,
                            "Most stocks showing 0% change",
                            True,
                        )
                        return False
                else:
                    self.log_result(
                        "Top Movers - Data Count",
                        False,
                        f"Insufficient movers: {len(gainers)} gainers, {len(losers)} losers",
                        True,
                    )
                    return False
            else:
                self.log_result(
                    "Top Movers API", False, f"HTTP {response.status_code}", True
                )
                return False
    ...
    def test_investment_scoring_apis(self):
        ...
        for symbol in test_symbols:
            try:
                ...
                response = requests.get(
                    f"{self.api_url}/investments/score/{symbol}", timeout=20
                )
                response_time = time.time() - start_time
                ...
                if response.status_code == 200:
                    ...
                    if score > 0 and rating != "Unknown":
                        ...
                        self.log_result(
                            f"Investment Score - {symbol}", False, "Invalid score data"
                        )
                        scoring_success = False
                else:
                    self.log_result(
                        f"Investment Score - {symbol}",
                        False,
                ...
                self.log_result(
                    f"Investment Score - {symbol}", False, f"Error: {str(e)}"
                )
                scoring_success = False
        ...
        try:
            start_time = time.time()
            response = requests.get(
                f"{self.api_url}/investments/top-picks?limit=5", timeout=30
            )
            response_time = time.time() - start_time
            ...
            if response.status_code == 200:
                ...
                if len(recommendations) >= 3:
                    ...
                    self.log_result(
                        ...
                        False,
                        f"Only {len(recommendations)} recommendations",
                    )
                    scoring_success = False
            else:
                self.log_result(
                    "Investment Top Picks", False, f"HTTP {response.status_code}"
                )
                scoring_success = False
    ...
    def test_enhanced_stock_data_apis(self):
        ...
        for symbol in test_symbols:
            try:
                start_time = time.time()
                response = requests.get(
                    f"{self.api_url}/stocks/{symbol}/enhanced", timeout=15
                )
                response_time = time.time() - start_time
                    ...
                    if price > 0:
                        ...
                        if extended_hours:
                            ...
                            self.log_result(
                                f"Extended Hours - {symbol}",
                                True,
                                "No extended hours data (normal)",
                            )
                    else:
                        self.log_result(
                            f"Enhanced Stock - {symbol}", False, "Zero price detected"
                        )
                        enhanced_success = False
                else:
                    self.log_result(
                        f"Enhanced Stock - {symbol}",
                        False,
                        f"HTTP {response.status_code}",
                    )
                    enhanced_success = False
        ...
        try:
            start_time = time.time()
            response = requests.get(
                f"{self.api_url}/screener/data?limit=10", timeout=20
            )
            response_time = time.time() - start_time
            ...
            if response.status_code == 200:
                ...
                if len(stocks) >= 5:
                    ...
                    self.log_result(
                        "Screener Data API",
                        False,
                        f"Only {len(stocks)} stocks returned",
                    )
                    enhanced_success = False
            else:
                self.log_result(
                    "Screener Data API", False, f"HTTP {response.status_code}"
                )
                enhanced_success = False
    ...
    def test_error_handling(self):
        ...
        try:
            ...
            self.log_result(
                "Error Handling - Invalid Symbol", False, f"Error: {str(e)}"
            )

        # Test invalid investment score
        try:
            response = requests.get(
                f"{self.api_url}/investments/score/INVALID123", timeout=10
            )
            if response.status_code in [404, 500]:
                self.log_result(
                    "Error Handling - Invalid Investment Score",
                    True,
                    f"Proper error response: {response.status_code}",
                )
            else:
            ...
            self.log_result(
                "Error Handling - Invalid Investment Score", False, f"Error: {str(e)}"
            )
    ...
    def test_response_times(self):
        ...
        for endpoint, name, max_time in critical_endpoints:
            try:
                start_time = time.time()
                response = requests.get(
                    f"{self.api_url}{endpoint}", timeout=max_time + 5
                )
                response_time = time.time() - start_time
                ...
                if response.status_code == 200 and response_time <= max_time:
                    self.log_result(
                        ...
                        True,
                        f"{response_time:.2f}s (target: <{max_time}s)",
                    )
                elif response.status_code == 200:
                    self.log_result(
                        f"Response Time - {name}",
                        False,
                        f"{response_time:.2f}s (too slow, target: <{max_time}s)",
                    )
                else:
                    self.log_result(
                        f"Response Time - {name}", False, f"HTTP {response.status_code}"
                    )
    ...
    def print_final_results(self):
        ...
        dashboard_critical = [
            ...
            "Top Movers API",
            "Market Overview - Data Quality",
            "Top Movers - Data Quality",
        ]
        dashboard_failures = [
            f
            for f in self.critical_failures
            if any(dc in f for dc in dashboard_critical)
        ]
