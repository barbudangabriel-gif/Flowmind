...
class InvestmentScannerTester:
    def __init__(self, base_url="https://options-analytics.preview.emergentagent.com"):
        ...
        print("🔍 Investment Scoring Scanner Tester")
    ...
    def test_mongodb_connectivity(self):
        ...
        try:
            ...
            print(f"   Database: {self.db_name}")
            print("   Collection: scanned_stocks")
    ...
    def test_scanner_status_endpoint(self):
        ...
        if "top_5_stocks" in status_data:
            ...
            for i, stock in enumerate(top_5[:3]):
                print(
                    f"   #{i+1}: {stock.get('ticker')} - Score: {stock.get('score')} - Rating: {stock.get('rating')}"
                )
    ...
    def test_scanner_start_endpoint(self):
        ...
        print("⏳ Waiting 10 seconds for scan to start processing...")
        time.sleep(10)
    ...
    def test_scanner_top_stocks_endpoint(self):
        ...
        for limit in [10, 25, 50]:
            ...
            missing_fields = [
                field for field in required_fields if field not in top_stocks_data
            ]
            ...
            if top_stocks:
                ...
                valid_scores = [
                    s
                    for s in top_stocks
                    if isinstance(s.get("score"), (int, float)) and s.get("score") > 0
                ]
                print("\n📊 Data Quality Analysis:")
                print(
                    f"   - Stocks with valid scores: {len(valid_scores)}/{len(top_stocks)}"
                )

                if len(valid_scores) == 0:
                    print("❌ CRITICAL: No stocks with valid scores found!")
                    return False
                else:
                    print(f"✅ Found {len(valid_scores)} stocks with valid scores")
            else:
                print("❌ CRITICAL: No top stocks returned!")
                return False
    ...
    def test_mongodb_data_persistence(self):
        ...
        try:
            ...
            if total_docs == 0:
                print("❌ CRITICAL: No documents found in scanned_stocks collection!")
                print("   This indicates the scanner is not saving results to MongoDB")
                client.close()
                return False
            ...
            for i, doc in enumerate(sample_docs):
                ...
                missing_fields = [
                    field for field in required_fields if field not in doc
                ]
    ...
    def test_scanner_logic_and_scoring(self):
        ...
        for symbol in test_symbols:
            ...
            success, score_data = self.run_test(
                f"Individual Score ({symbol})",
                "GET",
                f"investments/score/{symbol}",
                200,
                timeout=15,
            )
            ...
            if success:
                ...
                print(
                    f"   ✅ {symbol}: Score={total_score}, Rating={rating}, Risk={risk_level}"
                )
                ...
                missing_fields = [
                    field for field in required_fields if field not in score_data
                ]
    ...
    def test_scanner_comprehensive_workflow(self):
        ...
        print("Step 1: Check initial scanner status")
        ...
        if success:
            ...
            if post_scan_count > initial_count:
                ...
                print("❌ No stocks processed by scanner")
                return False
        ...
        if success:
            ...
            if stocks_returned > 0:
                print("✅ Scanner workflow completed successfully")
                return True
            else:
    ...
    def run_comprehensive_scanner_tests(self):
        ...
        print(
            "📋 Testing Romanian requirements: Scanner endpoints, MongoDB persistence, scoring logic"
        )
        print("=" * 80)
        ...
        mongodb_working = (
            test_results[0][1] and test_results[4][1]
        )  # MongoDB connectivity + persistence
        endpoints_working = (
            test_results[1][1] and test_results[2][1] and test_results[3][1]
        )  # All endpoints
        ...
        requirements = [
            (
                "Scanner endpoints working (GET status, POST start-scan, GET top-stocks)",
                endpoints_working,
            ),
            (
                "MongoDB persistence working (saves to 'scanned_stocks' collection)",
                mongodb_working,
            ),
            (
                "Scoring logic working (calculates scores for ~100 tickers)",
                scoring_working,
            ),
        ...
        if not mongodb_working:
            print(
                "   ❌ MongoDB persistence issues - scanner may not be saving results"
            )
        if not endpoints_working:
            print("   ❌ Scanner endpoints issues - API responses may be incomplete")
        if not scoring_working:
            print("   ❌ Scoring logic issues - calculations may be incorrect")
        if not workflow_working:
            print("   ❌ Workflow issues - complete scan process not working")

        if all([mongodb_working, endpoints_working, scoring_working, workflow_working]):
            print("   ✅ No critical issues found - scanner working correctly")
