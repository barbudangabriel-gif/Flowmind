...
class TradeStationDirectTester:
    ...
    def test_tradestation_accounts(self):
        ...
        if isinstance(accounts_data, dict):
            ...
            if 'data' in accounts_data:
                ...
                print(f"   - Response structure: Wrapped in 'data' field")
            else:
                accounts = accounts_data
                print(f"   - Response structure: Direct response")
        elif isinstance(accounts_data, list):
            accounts = accounts_data
            print(f"   - Response structure: Direct list")
        else:
        ...
        for account in accounts:
            ...
            account_type = account.get('AccountType') or account.get('account_type') or account.get('type')
    ...
    def test_tradestation_positions(self):
        ...
        print(f"📋 EXPECTED: Portfolio value should be ~${self.expected_portfolio_value:,.2f}")
        ...
        # Analyze positions
        if len(positions) == 0:
            print(f"⚠️ No positions found in account")
        ...
        for i, position in enumerate(positions[:10]):  # Show first 10
            ...
            market_value = position.get('MarketValue') or position.get('market_value', 0)
            unrealized_pnl = position.get('UnrealizedPnL') or position.get('unrealized_pnl', 0)
            asset_type = position.get('AssetType') or position.get('asset_type', 'Unknown')
            
            print(f"   {i+1:2d}. {symbol}: {quantity} shares, ${market_value:,.2f} value, ${unrealized_pnl:+,.2f} P&L ({asset_type})")
        ...
        value_difference_percent = (value_difference / self.expected_portfolio_value) * 100 if self.expected_portfolio_value > 0 else 0
        
        print(f"   - Difference: ${value_difference:,.2f} ({value_difference_percent:.2f}%)")
        
        if value_difference_percent < 5:
            print(f"   ✅ Portfolio value matches expected value (within 5%)")
            value_correct = True
        elif value_difference_percent < 15:
            print(f"   ⚠️ Portfolio value close to expected (within 15%)")
            value_correct = True
        else:
            print(f"   ❌ Portfolio value significantly different from expected")
            value_correct = False
    ...
    def test_connectivity_and_authentication(self):
        ...
        if success:
            ...
            if auth_status:
                ...
                print(f"   🔧 This may explain API connectivity issues")
                return False
        else:
            print(f"❌ Could not check authentication status")
            return False
    ...
    def run_comprehensive_test(self):
        ...
        test_results = [
            ...
            ("Positions Data Available", positions_data is not None and len(positions_data.get('data', positions_data) if isinstance(positions_data, dict) else positions_data or []) > 0)
        ]
        ...
        print(f"\n🔍 KEY FINDINGS:")
        if accounts_success:
            print(f"   ✅ TradeStation accounts endpoint is accessible")
        else:
            print(f"   ❌ TradeStation accounts endpoint has issues")
        
        if positions_success:
            print(f"   ✅ TradeStation positions endpoint is working")
        else:
            print(f"   ❌ TradeStation positions endpoint failed - explains 'Failed to fetch positions' error")
        ...
        if auth_success:
            ...
            print(f"   ❌ TradeStation authentication issues detected")
        ...
        if positions_success and positions_data:
            if isinstance(positions_data, dict) and 'data' in positions_data:
                print(f"   🔍 POTENTIAL ISSUE IDENTIFIED: Response data is wrapped in 'data' field")
                print(f"   🔧 SOLUTION: Frontend should access response.data.positions instead of response.positions")
                print(f"   📝 This likely explains the 'Failed to fetch positions' error")
            else:
                print(f"   ✅ Response structure appears frontend-compatible")
        
        # Portfolio value verification
        print(f"\n💰 PORTFOLIO VALUE VERIFICATION:")
        if positions_success and positions_data:
            print(f"   ✅ Portfolio data is available from TradeStation API")
            print(f"   📊 Expected value: ${self.expected_portfolio_value:,.2f}")
            print(f"   🔍 Actual values can be calculated from position data")
        else:
            print(f"   ❌ Cannot verify portfolio value - positions endpoint failed")
        
        # Final verdict
        print(f"\n🏆 FINAL VERDICT:")
        if success_rate >= 80:
            print(f"   🎉 EXCELLENT: TradeStation Direct API endpoints are working well")
            print(f"   ✅ API connectivity and authentication are functional")
            if positions_success:
                print(f"   ✅ Portfolio data is accessible - frontend issue likely")
            print(f"   🔧 'Failed to fetch positions' error is likely a frontend data parsing issue")
            ...
            print(f"   🔧 Authentication or connectivity problems detected")
        
        # Recommendations
        print(f"\n💡 RECOMMENDATIONS:")
        if not auth_success:
            print(f"   1. Check TradeStation authentication credentials")
            print(f"   2. Verify API key and environment configuration")
        
        if positions_success and isinstance(positions_data, dict) and 'data' in positions_data:
            print(f"   1. Update frontend to access response.data instead of response")
            print(f"   2. Check frontend error handling for nested response structures")
        
        if not positions_success:
            print(f"   1. Investigate TradeStation API connectivity")
            print(f"   2. Check account permissions and access rights")
        
        print(f"\n📊 SUMMARY FOR MAIN AGENT:")
        print(f"   - Tests Run: {self.tests_run}")
        print(f"   - Tests Passed: {self.tests_passed}")
        print(f"   - Success Rate: {success_rate:.1f}%")
        print(f"   - Primary Issue: {'Frontend data parsing' if positions_success else 'API connectivity'}")
