...
class TradeStationAPITester:
    ...
    def run_test(self, name, method, endpoint, expected_status, data=None, params=None):
        ...
        try:
            ...
            if success:
                ...
                try:
                    ...
                    if (
                        ...
                        print(
                            f"   Response: Dict with keys: {list(response_data.keys())}"
                        )
            ...
            self.test_results.append(
                {"name": name, "status": "ERROR", "response_time": 0, "error": str(e)}
            )
            return False, {}
    ...
    def test_tradestation_authentication(self):
        ...
        if success:
            ...
            print(
                f"   🔧 Credentials Configured: {config_data.get('credentials_configured', False)}"
            )
            print(f"   🔗 Base URL: {config_data.get('base_url', 'unknown')}")

            if auth_data.get("authenticated"):
                print("   ✅ TradeStation authentication is active")
                if "connection_test" in auth_status:
                    conn_test = auth_status["connection_test"]
                    print(f"   🔗 Connection Test: {conn_test}")
            else:
                print("   ⚠️  TradeStation not authenticated (expected for testing)")
        ...
        if success:
            ...
            if auth_url:
                ...
                print(
                    f"   🔧 Contains OAuth params: {'✅ YES' if 'oauth' in auth_url.lower() or 'authorize' in auth_url.lower() else '❌ NO'}"
                )
    ...
    def test_tradestation_portfolio_endpoints(self):
        ...
        if success:
            ...
            if accounts:
                ...
                print("   ⚠️  No accounts found - using test account ID")
        ...
        if success:
            ...
            print(
                f"   📊 Portfolio Summary: {'✅ Present' if portfolio_summary else '❌ Missing'}"
            )
            print(
                f"   📈 Performance Metrics: {'✅ Present' if performance_metrics else '❌ Missing'}"
            )
            print(
                f"   ⚖️  Risk Analysis: {'✅ Present' if risk_analysis else '❌ Missing'}"
            )
    ...
    def test_tradestation_market_data_endpoints(self):
        ...
        success, quote_data = self.run_test(
            "TradeStation Quote (AAPL)", "GET", "tradestation/quotes/AAPL", 200
        )
    ...
    def test_tradestation_risk_management(self):
        ...
        success, risk_limits = self.run_test(
            "TradeStation Risk Limits (GET)", "GET", "tradestation/risk-limits", 200
        )

        if success:
            limits = risk_limits.get("risk_limits", {})

            print("   ⚖️  Risk Limits Configuration:")
    ...
    def test_api_root_with_tradestation(self):
        ...
        print("=" * 80)
        print(
            "🎯 OBJECTIVE: Verify API version 5.0.0 and TradeStation endpoint documentation"
        )

        success, root_data = self.run_test("API Root with TradeStation", "GET", "", 200)
        ...
        if success:
            ...
            print(
                f"   ✅ Expected Version 5.0.0: {'✅ YES' if version == '5.0.0' else '❌ NO'}"
            )
            ...
            missing_endpoints = [
                ep for ep in expected_endpoints if ep not in ts_endpoints
            ]
            if missing_endpoints:
                print(f"   ⚠️  Missing endpoint documentation: {missing_endpoints}")
            else:
                print("   ✅ All TradeStation endpoints documented")
    ...
    def generate_tradestation_test_summary(self, test_results):
        ...
        print(
            f"   - Individual Test Success Rate: {(self.tests_passed/self.tests_run)*100:.1f}%"
        )
        ...
        auth_tests = [
            ...
            if "auth" in t["name"].lower() or "oauth" in t["name"].lower()
        ]
        ...
        portfolio_tests = [
            t
            for t in self.test_results
            if "account" in t["name"].lower()
            or "position" in t["name"].lower()
            or "balance" in t["name"].lower()
        ]
        ...
        trading_tests = [
            t
            for t in self.test_results
            if "order" in t["name"].lower() or "trading" in t["name"].lower()
        ]
        ...
        market_tests = [
            t
            for t in self.test_results
            if "quote" in t["name"].lower() or "historical" in t["name"].lower()
        ]
        ...
        response_times = [
            t["response_time"] for t in self.test_results if "response_time" in t
        ]
        if response_times:
        ...
        # Error analysis
        failed_tests = [
            t for t in self.test_results if t["status"] in ["FAIL", "ERROR", "TIMEOUT"]
        ]
        if failed_tests:
        ...
        print("\n🎯 FINAL VERDICT:")
        if success_rate >= 85:
            print("🎉 EXCELLENT: TradeStation API integration is working excellently!")
            print(
                "   All major components are functional and ready for production use."
            )
            print(
                "   Authentication, portfolio management, trading, and risk controls are operational."
            )
        elif success_rate >= 70:
            print(
                "✅ GOOD: TradeStation API integration is mostly working with minor issues."
            )
            print(
                "   Core functionality is available but some endpoints may need attention."
            )
        elif success_rate >= 50:
            print("⚠️  PARTIAL: TradeStation API integration has significant issues.")
            print(
                "   Some components are working but major functionality may be impaired."
            )
        else:
            print("❌ CRITICAL: TradeStation API integration has major problems.")
            print("   Most endpoints are failing and require immediate attention.")

        # Recommendations
        print("\n💡 RECOMMENDATIONS:")
        if not test_results.get("authentication", False):
            print(
                "   🔐 Authentication: Check TradeStation API credentials and OAuth configuration"
            )

        if not test_results.get("portfolio", False):
            print(
                "   📊 Portfolio: Verify account access and portfolio service initialization"
            )

        if not test_results.get("trading", False):
            print(
                "   ⚡ Trading: Check trading service and order management functionality"
            )

        if not test_results.get("market_data", False):
            print("   💹 Market Data: Verify market data feed and quote service")

        if not test_results.get("risk_management", False):
            print(
                "   ⚖️  Risk Management: Check risk limit configuration and enforcement"
            )

        if success_rate >= 70:
            print("   ✅ TradeStation integration is ready for comprehensive testing")
        else:
            print("   🔧 Focus on fixing failed endpoints before proceeding")
