...
class TradeStationPortfolioValuesTester:
    ...
    def test_all_endpoints(self):
        ...
        try:
            response = requests.get(
                f"{self.api_url}/portfolio-management/portfolios/tradestation-main/positions",
                timeout=30,
            )
            if response.status_code == 200:
                ...
                print("✅ Portfolio Management Service Results:")
                print(f"   - Total Positions: {len(positions)}")
        ...
        if results.get("tradestation_api") and results.get("portfolio_management"):
            ...
            print("\n📊 DIFFERENCES:")
            print(
                f"   TradeStation vs Portfolio Mgmt: ${ts_vs_pm_diff:+,.2f} ({ts_vs_pm_percent:+.1f}%)"
            )
            ...
            if abs(ts_vs_pm_diff) > 1000:  # More than $1000 difference
                ...
                if abs(pm_vs_user_diff) < abs(ts_vs_user_diff):
                    print(
                        f"   - Portfolio Management (${pm_value:,.2f}) is closer to user reported value"
                    )
                    ...
                    results["recommended_action"] = (
                        "Fix Portfolio Management Service calculations"
                    )
            else:
        ...
        if results.get("tradestation_api") and results.get("portfolio_management"):
            ...
            if results["critical_discrepancy_found"]:
                ...
                print(
                    "   - The user is seeing Portfolio Management values, which are INCORRECT"
                )
                print(
                    f"   - The true portfolio value from TradeStation is: ${ts_value:,.2f}"
                )

                print("\n💡 SOLUTION:")
                print(
                    "   1. Fix Portfolio Management Service to use correct TradeStation values"
                )
        ...
        if results["critical_discrepancy_found"]:
            ...
            print(
                f"   3. UPDATE: Frontend to display correct values (${results['true_portfolio_value']:,.2f})"
            )
            print("   4. IMPLEMENT: Data validation and discrepancy alerts")
            print("   5. TEST: Verify all portfolio displays show consistent values")
        else:
        ...
        print(
            f"⚠️  Critical Discrepancy: {'YES' if results['critical_discrepancy_found'] else 'NO'}"
        )
        if results["critical_discrepancy_found"]:
