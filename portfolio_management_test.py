...
class PortfolioManagementTester:
    ...
    def test_portfolio_management_with_tradestation_integration(self):
        ...
        print("=" * 100)
        print("🎯 OBJECTIVE: Test updated Portfolio Management Service with real TradeStation integration")
        print("📋 REVIEW REQUIREMENTS:")
        print("   1. GET /api/portfolio-management/portfolios - Should load real TradeStation data in TradeStation Main portfolio")
        print("   2. GET /api/portfolio-management/portfolios/tradestation-main/positions - Should show real positions from TradeStation account")
        print("   3. Verify positions data structure includes both stocks and options from TradeStation")
        print("   4. Check if portfolio totals are calculated correctly from real TradeStation data")
        ...
        print("   - Portfolio totals (total_value, total_pnl, positions_count) reflecting TradeStation data")
        print("   - Position metadata including TradeStation-specific fields")
        print("   - Data structure compatibility with existing frontend")
        print("   - Live TradeStation account data (84 positions: 19 stocks, 65 options)")
        ...
        for portfolio in portfolios:
            if portfolio.get('id') == 'tradestation-main' or 'tradestation' in portfolio.get('name', '').lower():
                tradestation_main = portfolio
                break
        
        if tradestation_main:
            print(f"✅ TradeStation Main Portfolio Found:")
            ...
            print(f"   - Positions Count: {tradestation_main.get('positions_count', 0)}")
            print(f"   - Data Source: {tradestation_main.get('data_source', 'Unknown')}")
            
            # Check if using real TradeStation data
            data_source = tradestation_main.get('data_source', '')
            if 'tradestation' in data_source.lower() and 'mock' not in data_source.lower():
            ...
            if positions_count >= 80:  # Allow some variance
                print(f"✅ Position Count Matches Expected Range: {positions_count} (expected ~84)")
                position_count_correct = True
            elif positions_count >= 10:
                print(f"⚠️  Position Count Lower Than Expected: {positions_count} (expected ~84)")
            ...
            print(f"❌ CRITICAL: TradeStation Main Portfolio Not Found")
        ...
        print(f"📊 Positions Analysis:")
        print(f"   - Total Positions: {len(positions)}")
        print(f"   - Portfolio Total Value: ${portfolio_summary.get('total_value', 0):,.2f}")
        print(f"   - Portfolio Total P&L: ${portfolio_summary.get('total_pnl', 0):,.2f}")
        print(f"   - Portfolio P&L %: {portfolio_summary.get('total_pnl_percent', 0):.2f}%")
        ...
        if positions:
            ...
            for i, position in enumerate(positions[:20]):  # Analyze first 20 positions to find options
                ...
                required_fields = ['symbol', 'quantity', 'market_value', 'unrealized_pnl', 'type']
            ...
            has_stocks = stocks_count > 0 or any('stock' in str(pos_type).lower() for pos_type in position_types.keys())
            has_options = options_count > 0 or any('option' in str(pos_type).lower() for pos_type in position_types.keys())
            
            if has_stocks and has_options:
                print(f"✅ Portfolio contains both stocks and options as expected")
                mixed_positions = True
            elif has_stocks:
                print(f"⚠️  Portfolio contains stocks but no clear options identification")
                mixed_positions = False
            elif has_options:
                print(f"⚠️  Portfolio contains options but no clear stocks identification")
                mixed_positions = False
            else:
                print(f"❌ Unable to identify stocks and options clearly")
                mixed_positions = False
                
        else:
            print(f"❌ No positions found in TradeStation Main portfolio")
            mixed_positions = False
            has_stocks = False
            has_options = False
        ...
        if positions and portfolio_summary:
            ...
            calculated_total_value = sum(pos.get('market_value', 0) for pos in positions)
            calculated_total_pnl = sum(pos.get('unrealized_pnl', 0) for pos in positions)
            
            reported_total_value = portfolio_summary.get('total_value', 0)
            reported_total_pnl = portfolio_summary.get('total_pnl', 0)
            
            print(f"📊 Portfolio Totals Verification:")
            print(f"   - Calculated Total Value: ${calculated_total_value:,.2f}")
            print(f"   - Reported Total Value: ${reported_total_value:,.2f}")
            print(f"   - Difference: ${abs(calculated_total_value - reported_total_value):,.2f}")
            
            print(f"   - Calculated Total P&L: ${calculated_total_pnl:,.2f}")
            print(f"   - Reported Total P&L: ${reported_total_pnl:,.2f}")
            print(f"   - Difference: ${abs(calculated_total_pnl - reported_total_pnl):,.2f}")
            
            # Check if calculations are reasonably close (allow for rounding)
            value_diff_percent = abs(calculated_total_value - reported_total_value) / max(calculated_total_value, 1) * 100
            pnl_diff_percent = abs(calculated_total_pnl - reported_total_pnl) / max(abs(calculated_total_pnl), 1) * 100
            
            if value_diff_percent <= 1.0:  # Within 1%
                print(f"✅ Total Value calculation accurate (difference: {value_diff_percent:.2f}%)")
                value_calc_correct = True
            else:
                print(f"❌ Total Value calculation may be incorrect (difference: {value_diff_percent:.2f}%)")
                value_calc_correct = False
            
            if pnl_diff_percent <= 1.0:  # Within 1%
                print(f"✅ Total P&L calculation accurate (difference: {pnl_diff_percent:.2f}%)")
                pnl_calc_correct = True
            else:
                print(f"❌ Total P&L calculation may be incorrect (difference: {pnl_diff_percent:.2f}%)")
                pnl_calc_correct = False
                
            totals_correct = value_calc_correct and pnl_calc_correct
        else:
            print(f"❌ Cannot verify totals - missing positions or portfolio summary")
        ...
        if positions:
            print(f"📊 Analyzing Position Metadata:")
            ...
            for i, position in enumerate(positions[:5]):  # Check first 5 positions
                ...
                standard_fields = ['symbol', 'quantity', 'avg_cost', 'current_price', 'market_value', 'unrealized_pnl', 'type']
                ...
                ts_fields = ['account_id', 'position_id', 'trade_date', 'settlement_date', 'commission', 'fees']
                ...
                if position.get('type', '').lower() in ['option', 'options']:
                    option_fields = ['strike_price', 'expiration_date', 'option_type', 'underlying_symbol']
            ...
            metadata_complete = len(standard_fields_found) >= 5  # At least 5 standard fields
        else:
        ...
        if tradestation_main:
            ...
            if 'mock' in data_source.lower() or 'fallback' in data_source.lower():
                if len(positions) >= 5 and portfolio_summary.get('total_value', 0) > 0:
                    print(f"✅ Fallback provides adequate data: {len(positions)} positions, ${portfolio_summary.get('total_value', 0):,.2f} total value")
                else:
        ...
        portfolio_structure_ok = all(field in portfolios_data for field in expected_portfolio_fields)
        position_structure_ok = all(field in positions_data for field in expected_position_fields)
        
        print(f"📊 Frontend Compatibility Check:")
        print(f"   - Portfolio Response Structure: {'✅ Compatible' if portfolio_structure_ok else '❌ Issues'}")
        print(f"   - Position Response Structure: {'✅ Compatible' if position_structure_ok else '❌ Issues'}")
        
        if positions:
            # Check if positions have the fields frontend expects
            sample_position = positions[0]
            frontend_expected_fields = ['symbol', 'quantity', 'market_value', 'unrealized_pnl', 'type']
            frontend_fields_present = sum(1 for field in frontend_expected_fields if field in sample_position)
            
            print(f"   - Position Fields for Frontend: {frontend_fields_present}/{len(frontend_expected_fields)} present")
            
            frontend_compatible = (portfolio_structure_ok and position_structure_ok and 
                                 frontend_fields_present >= len(frontend_expected_fields) - 1)
        else:
        ...
        requirements = [
            ...
            ("Position data structure includes both stocks and options", mixed_positions),
            ("Portfolio totals calculated correctly from real TradeStation data", totals_correct),
        ...
        if tradestation_main:
            ...
            print(f"   - TradeStation Main Portfolio: ❌ Not Found")
        
        if positions:
            print(f"   - Position Data: ✅ {len(positions)} positions retrieved")
            print(f"   - Position Types: {list(position_types.keys()) if 'position_types' in locals() else 'Unknown'}")
        else:
            print(f"   - Position Data: ❌ No positions retrieved")
        
        print(f"   - Real TradeStation Integration: {'✅ Active' if using_real_data else '❌ Not Active'}")
        print(f"   - Fallback Status: {fallback_status}")
        print(f"   - Frontend Compatibility: {'✅ Compatible' if frontend_compatible else '❌ Issues'}")
        ...
        if success_rate >= 85 and requirements_met >= 4:
            ...
            print(f"   ✅ Real TradeStation account data (84 positions: 19 stocks, 65 options) is being loaded")
            print(f"   ✅ Portfolio totals reflect TradeStation data accurately")
            print(f"   ✅ Data structure is compatible with existing frontend")
            print(f"   ✅ Move to Portfolio X functionality can work with real TradeStation data")
        elif success_rate >= 70 and requirements_met >= 3:
            print(f"\n✅ VERDICT: GOOD - Portfolio Management Service mostly working with minor issues")
            print(f"   ✅ Basic functionality operational")
            print(f"   ⚠️  Some TradeStation integration aspects may need attention")
        else:
            print(f"\n❌ VERDICT: NEEDS ATTENTION - Portfolio Management Service has significant issues")
            print(f"   ❌ TradeStation integration not working as expected")
            print(f"   ❌ Review requirements not fully satisfied")
        
        return success_rate >= 70 and requirements_met >= 3

    def run_all_tests(self):
        """Run all portfolio management tests"""
        print("🚀 STARTING PORTFOLIO MANAGEMENT SERVICE TESTING WITH TRADESTATION INTEGRATION")
        print("=" * 100)
