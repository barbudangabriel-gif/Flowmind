#!/usr/bin/env python3
"""
CRITICAL INVESTIGATION: TradeStation Mindfolio Data Discrepancy
==============================================================
...
- TradeStation Direct API: $969,473.90 total mindfolio value
- Mindfolio Management Service: $790,173.50 total mindfolio value
...
3. GET /api/mindfolio-management/mindfolios/tradestation-main/positions - Check if Mindfolio Management Service is using mock data
...
class TradeStationMindfolioInvestigator:
 ...
 severity_icon = {"CRITICAL": "🚨", "WARNING": "", "INFO": "", "SUCCESS": ""}

 print(f"{severity_icon.get(severity, '')} [{category}] {finding}")
 ...
 self.log_finding(
 "ACCOUNTS",
 f"Failed to access TradeStation accounts: {result['error']}",
 "CRITICAL",
 )
 return None
 ...
 self.log_finding(
 "ACCOUNTS",
 f"TradeStation accounts returned status {result['status_code']}",
 "CRITICAL",
 )
 return None
 ...
 self.log_finding(
 "ACCOUNTS", "Successfully retrieved TradeStation accounts data", "SUCCESS"
 )
 ...
 self.log_finding(
 "ACCOUNTS",
 f"Unexpected accounts data structure: {type(accounts_data)}",
 "WARNING",
 )
 accounts = []
 ...
 self.log_finding(
 "ACCOUNTS",
 "Found target account 11775499 (Margin account)",
 "SUCCESS",
 )
 print(" TARGET ACCOUNT IDENTIFIED")

 # Check for any suspicious account details
 if status != "Active":
 self.log_finding(
 "ACCOUNTS",
 f"Account {account_id} has non-active status: {status}",
 "WARNING",
 )
 ...
 self.log_finding(
 "ACCOUNTS",
 f"Account {account_id} uses non-USD currency: {currency}",
 "WARNING",
 )
 ...
 self.log_finding(
 "ACCOUNTS",
 "Target account 11775499 not found in accounts list",
 "CRITICAL",
 )
 ...
 self.log_finding(
 "ACCOUNTS",
 f"Using first available account {target_account_id} for investigation",
 "WARNING",
 )
 ...
 print("=" * 80)
 print(" OBJECTIVE: Check if TradeStation API returns REAL user positions")
 print(f" ACCOUNT: {account_id}")
 print(" LOOKING FOR: Real positions vs mock data indicators")
 ...
 self.log_finding(
 "TS_POSITIONS",
 f"Failed to access TradeStation positions: {result['error']}",
 "CRITICAL",
 )
 return None
 ...
 self.log_finding(
 "TS_POSITIONS",
 f"TradeStation positions returned status {result['status_code']}",
 "CRITICAL",
 )
 return None
 ...
 self.log_finding(
 "TS_POSITIONS",
 "Successfully retrieved TradeStation positions data",
 "SUCCESS",
 )
 ...
 self.log_finding(
 "TS_POSITIONS",
 f"Unexpected positions data structure: {type(positions_data)}",
 "WARNING",
 )
 positions = []
 ...
 if isinstance(market_value, (int, float))
 else f" - Market Value: {market_value}"
 )
 print(
 f" - Unrealized P&L: ${unrealized_pnl:,.2f}"
 if isinstance(unrealized_pnl, (int, float))
 else f" - Unrealized P&L: {unrealized_pnl}"
 )
 ...
 f"{symbol}: {quantity} shares (common mock quantity)"
 )
 ...
 print("\n MINDFOLIO TOTALS (TradeStation Direct):")
 print(f" Total Market Value: ${total_market_value:,.2f}")
 print(f" Total Unrealized P&L: ${total_unrealized_pnl:,.2f}")
 print(
 f" P&L Percentage: {(total_unrealized_pnl/total_market_value*100):+.2f}%"
 if total_market_value > 0
 else " P&L Percentage: N/A"
 )
 ...
 self.log_finding(
 "TS_POSITIONS",
 f"TradeStation Direct API shows ${total_market_value:,.2f} total mindfolio value",
 "INFO",
 )
 self.log_finding(
 "TS_POSITIONS",
 f"TradeStation shows {len(positions)} total positions ({stocks_count} stocks, {options_count} options)",
 "INFO",
 )
 ...
 self.log_finding(
 "TS_POSITIONS",
 f"Suspicious mock data patterns detected: {len(suspicious_patterns)} issues",
 "WARNING",
 )
 ...
 self.log_finding(
 "TS_POSITIONS",
 "No obvious mock data patterns detected in TradeStation positions",
 "SUCCESS",
 )

 # Check for expected user positions (from review request)
 user_reported_symbols = [
 "AMZN",
 "QQQ",
 "GOOGL",
 ] # User said these are NOT their positions
 found_reported_symbols = [
 symbol for symbol in position_symbols if symbol in user_reported_symbols
 ]

 if found_reported_symbols:
 self.log_finding(
 "TS_POSITIONS",
 f"Found user-reported 'fake' symbols in TradeStation: {found_reported_symbols}",
 "CRITICAL",
 )
 else:
 self.log_finding(
 "TS_POSITIONS",
 "User-reported 'fake' symbols not found in TradeStation direct API",
 "INFO",
 )
 ...
 def investigate_mindfolio_management_service(self):
 """PHASE 3: Investigate Mindfolio Management Service"""
 print("\n" + "=" * 80)
 print(" PHASE 3: MINDFOLIO MANAGEMENT SERVICE INVESTIGATION")
 print("=" * 80)
 print(" OBJECTIVE: Check if Mindfolio Management Service is using mock data")
 print(
 ...
 # Test Mindfolio Management Service endpoint
 ...
 self.log_finding(
 "PM_SERVICE",
 f"Failed to access Mindfolio Management Service: {result['error']}",
 "CRITICAL",
 )
 return None
 ...
 self.log_finding(
 "PM_SERVICE",
 f"Mindfolio Management Service returned status {result['status_code']}",
 "CRITICAL",
 )
 return None
 ...
 self.log_finding(
 "PM_SERVICE",
 "Successfully retrieved Mindfolio Management Service data",
 "SUCCESS",
 )

 # Analyze Mindfolio Management Service structure
 positions = pm_data.get("positions", [])
 mindfolio_summary = pm_data.get("mindfolio_summary", {})

 print("\n MINDFOLIO MANAGEMENT SERVICE ANALYSIS:")
 print(f" Total Positions Found: {len(positions)}")

 # Extract mindfolio summary
 ...
 print("\n MINDFOLIO SUMMARY (Mindfolio Management Service):")
 ...
 1000,
 1200,
 ]: # Common mock quantities
 pm_mock_indicators.append(
 f"{symbol}: {quantity} shares (typical mock quantity)"
 )

 # Check for round numbers that suggest mock data
 if isinstance(market_value, (int, float)) and market_value > 0:
 if (
 market_value % 1000 == 0 or market_value % 500 == 0
 ): # Round thousands
 pm_mock_indicators.append(
 f"{symbol}: ${market_value:,.2f} (round market value)"
 )

 # Log key findings
 self.log_finding(
 "PM_SERVICE",
 f"Mindfolio Management Service shows ${pm_total_value:,.2f} total mindfolio value",
 "INFO",
 )
 self.log_finding(
 "PM_SERVICE",
 f"Mindfolio Management shows {len(positions)} total positions",
 "INFO",
 )
 ...
 self.log_finding(
 "PM_SERVICE",
 f"Mock data indicators detected: {len(pm_mock_indicators)} issues",
 "WARNING",
 )
 for indicator in pm_mock_indicators[:5]: # Show first 5
 self.log_finding(
 "PM_SERVICE", f"Mock indicator: {indicator}", "WARNING"
 )
 else:
 self.log_finding(
 "PM_SERVICE",
 "No obvious mock data indicators detected in Mindfolio Management Service",
 "SUCCESS",
 )
 ...
 found_fake_symbols = [
 symbol for symbol in pm_position_symbols if symbol in user_reported_symbols
 ]

 if found_fake_symbols:
 self.log_finding(
 "PM_SERVICE",
 f"Found user-reported 'fake' symbols in Mindfolio Management: {found_fake_symbols}",
 "CRITICAL",
 )
 else:
 self.log_finding(
 "PM_SERVICE",
 "User-reported 'fake' symbols not found in Mindfolio Management Service",
 "INFO",
 )
 ...
 """PHASE 4: Compare TradeStation Direct vs Mindfolio Management Service"""
 print("\n" + "=" * 80)
 print(" PHASE 4: DATA SOURCE COMPARISON")
 print("=" * 80)
 print(" OBJECTIVE: Identify discrepancies between data sources")
 print(" COMPARING: TradeStation Direct API vs Mindfolio Management Service")

 if not ts_data or not pm_data:
 self.log_finding(
 "COMPARISON", "Cannot compare data sources - missing data", "CRITICAL"
 )
 return

 # Compare mindfolio values
 ...
 (value_difference / ts_value * 100) if ts_value > 0 else 0
 )

 print("\n MINDFOLIO VALUE COMPARISON:")
 print(f" TradeStation Direct API: ${ts_value:,.2f}")
 print(f" Mindfolio Management: ${pm_value:,.2f}")
 print(
 f" Difference: ${value_difference:,.2f} ({value_difference_percent:+.1f}%)"
 )

 if abs(value_difference) > 1000: # More than $1000 difference
 self.log_finding(
 "COMPARISON",
 f"CRITICAL VALUE DISCREPANCY: ${value_difference:,.2f} difference ({value_difference_percent:+.1f}%)",
 "CRITICAL",
 )
 elif abs(value_difference) > 100: # More than $100 difference
 self.log_finding(
 "COMPARISON",
 f"Significant value discrepancy: ${value_difference:,.2f} difference",
 "WARNING",
 )
 else:
 self.log_finding(
 "COMPARISON",
 f"Mindfolio values are consistent (difference: ${value_difference:,.2f})",
 "SUCCESS",
 )
 ...
 print(f" Mindfolio Management: {pm_positions} positions")
 print(f" Difference: {position_difference:+d} positions")

 if position_difference != 0:
 self.log_finding(
 "COMPARISON",
 f"Position count mismatch: {position_difference:+d} positions difference",
 "WARNING",
 )
 else:
 self.log_finding(
 "COMPARISON", "Position counts match between data sources", "SUCCESS"
 )
 ...
 print(f" Mindfolio Mgmt only: {len(pm_only_symbols)}")

 if ts_only_symbols:
 print(
 f" TradeStation exclusive: {list(ts_only_symbols)[:10]}"
 ) # Show first 10
 self.log_finding(
 "COMPARISON",
 f"TradeStation has {len(ts_only_symbols)} symbols not in Mindfolio Management",
 "WARNING",
 )

 if pm_only_symbols:
 print(
 f" Mindfolio Mgmt exclusive: {list(pm_only_symbols)[:10]}"
 ) # Show first 10
 self.log_finding(
 "COMPARISON",
 f"Mindfolio Management has {len(pm_only_symbols)} symbols not in TradeStation",
 "WARNING",
 )
 ...
 print(f" Mindfolio Mgmt mock indicators: {pm_suspicious}")

 if pm_suspicious > ts_suspicious:
 self.log_finding(
 "COMPARISON",
 f"Mindfolio Management Service shows more mock data indicators ({pm_suspicious} vs {ts_suspicious})",
 "CRITICAL",
 )
 elif ts_suspicious > pm_suspicious:
 self.log_finding(
 "COMPARISON",
 f"TradeStation Direct API shows more suspicious patterns ({ts_suspicious} vs {pm_suspicious})",
 "WARNING",
 )
 else:
 self.log_finding(
 "COMPARISON",
 "Both data sources show similar levels of data authenticity",
 "INFO",
 )
 ...
 print("=" * 80)
 print(
 " CRITICAL ISSUE: User reports displayed positions are NOT their real TradeStation positions"
 )
 ...
 print(
 " 1. 🚨 URGENT: Fix Mindfolio Management Service to use real TradeStation data"
 )
 print(
 " 2. Investigate why Mindfolio Management Service shows different values"
 )
 print(
 " 3. Implement data validation between TradeStation API and Mindfolio Management"
 )
 ...
 if mock_data_found:
 print(
 " 5. 🎭 Remove or replace mock data with real TradeStation integration"
 )
 print(
 " 6. Ensure Mindfolio Management Service uses authenticated TradeStation data"
 )
 ...
 print(
 " The user's concern about fake/mock positions appears to be VALID."
 )
 print(" Immediate action required to fix data source discrepancies.")
 else:
 print("\n VERDICT: NO CRITICAL ISSUES FOUND")
 print(
 " Data sources appear to be consistent. User issue may be elsewhere."
 )

def main():
 """Run the TradeStation mindfolio investigation"""
 print(" TRADESTATION MINDFOLIO DATA INVESTIGATION")
 print("=" * 80)
 ...
 investigator = TradeStationMindfolioInvestigator()
 ...
 # Phase 3: Investigate Mindfolio Management Service
 pm_data = investigator.investigate_mindfolio_management_service()
 ...
 investigator.log_finding(
 "SYSTEM", f"Investigation failed with error: {str(e)}", "CRITICAL"
 )
