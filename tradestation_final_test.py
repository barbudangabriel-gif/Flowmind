...
class TradeStationFinalTester:
    ...
    def test_tradestation_endpoints(self):
        ...
        print("🎯 FOCUS: API response status codes, data availability, portfolio value verification")
        ...
        try:
            response = requests.get(f"{self.api_url}/auth/tradestation/status", timeout=30)
            if response.status_code == 200:
                ...
                if authenticated and has_access_token:
                    ...
                    print(f"   ✅ TradeStation authentication is WORKING")
                else:
                    results['issues_found'].append("TradeStation not properly authenticated")
                    print(f"   ❌ TradeStation authentication issues")
            else:
                results['issues_found'].append(f"Auth status endpoint returned {response.status_code}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if target_account:
                    ...
                    results['issues_found'].append(f"Target account {self.account_id} not found")
                    print(f"   ❌ Target account {self.account_id} not found")
            else:
                results['issues_found'].append(f"Accounts endpoint returned {response.status_code}")
                print(f"❌ Accounts Endpoint: {response.status_code}")
        except Exception as e:
        ...
        try:
            response = requests.get(f"{self.api_url}/tradestation/accounts/{self.account_id}/positions", timeout=30)
            if response.status_code == 200:
                ...
                if len(positions) > 0:
                    ...
                    for i, position in enumerate(positions[:10]):
                        ...
                        print(f"   {i+1:2d}. {symbol}: {quantity} shares, ${market_value:,.2f}, ${unrealized_pnl:+,.2f} P&L ({asset_type})")
                    ...
                    value_difference_percent = (value_difference / self.expected_portfolio_value) * 100
                    
                    print(f"   - Difference: ${value_difference:,.2f} ({value_difference_percent:.2f}%)")
                    
                    if value_difference_percent < 5:
                        results['portfolio_value_correct'] = True
                        print(f"   ✅ Portfolio value matches expected (within 5%)")
                    elif value_difference_percent < 15:
                        results['portfolio_value_correct'] = True
                        print(f"   ✅ Portfolio value close to expected (within 15%)")
                    else:
                        results['issues_found'].append(f"Portfolio value differs by {value_difference_percent:.1f}%")
                        print(f"   ⚠️ Portfolio value differs significantly ({value_difference_percent:.1f}%)")
                else:
                    results['issues_found'].append("No positions found in account")
                    print(f"   ⚠️ No positions found")
            else:
                results['issues_found'].append(f"Positions endpoint returned {response.status_code}")
                print(f"❌ Positions Endpoint: {response.status_code}")
                print(f"   🚨 This explains the 'Failed to fetch positions' error!")
        except Exception as e:
    ...
    def generate_final_report(self, results):
        ...
        if results['authentication_working']:
            print(f"   ✅ TradeStation authentication is working correctly")
        else:
            print(f"   ❌ TradeStation authentication has issues")
        
        if results['accounts_working']:
            print(f"   ✅ GET /api/tradestation/accounts is working correctly")
        else:
            print(f"   ❌ GET /api/tradestation/accounts has issues")
        
        if results['positions_working']:
            print(f"   ✅ GET /api/tradestation/accounts/11775499/positions is working correctly")
            print(f"   📊 Found {results['total_positions']} positions")
        else:
            print(f"   ❌ GET /api/tradestation/accounts/11775499/positions failed")
            print(f"   🚨 This explains the 'Failed to fetch positions' error")
        ...
        if results['actual_portfolio_value'] > 0:
            ...
            if results['portfolio_value_correct']:
                print(f"   ✅ Portfolio value matches expected ${self.expected_portfolio_value:,.2f}")
            else:
                print(f"   ⚠️ Portfolio value differs from expected ${self.expected_portfolio_value:,.2f}")
        ...
        if results['positions_working']:
            print(f"   ✅ TradeStation positions endpoint is working correctly")
            print(f"   📊 Returns {results['total_positions']} positions with proper data structure")
            print(f"   🔍 'Failed to fetch positions' error is likely a frontend issue:")
            print(f"      - Check frontend error handling")
            print(f"      - Verify API URL configuration")
            print(f"      - Check for CORS or network issues")
            print(f"      - Verify frontend data parsing logic")
        else:
            print(f"   ❌ TradeStation positions endpoint is failing")
            print(f"   🚨 This directly explains the 'Failed to fetch positions' error")
        
        # Final verdict
        print(f"\n🏆 FINAL VERDICT:")
        if success_rate >= 80:
            print(f"   🎉 EXCELLENT: TradeStation Direct API endpoints are working correctly")
            print(f"   ✅ Both requested endpoints are functional:")
            print(f"      - GET /api/tradestation/accounts ✅")
            print(f"      - GET /api/tradestation/accounts/11775499/positions ✅")
            print(f"   ✅ Authentication and connectivity are working")
            print(f"   ✅ Portfolio value ${results['actual_portfolio_value']:,.2f} is accessible")
            if not results['portfolio_value_correct']:
                print(f"   ⚠️ Portfolio value differs from expected, but API is working")
            print(f"   🔧 'Failed to fetch positions' error is a frontend issue, not API")
        elif success_rate >= 60:
            print(f"   ✅ GOOD: TradeStation API mostly working with some issues")
            print(f"   🔧 Some endpoints may need attention")
        else:
            print(f"   ❌ CRITICAL: TradeStation API has significant issues")
            print(f"   🚨 This explains the 'Failed to fetch positions' error")
        
        # Recommendations
        print(f"\n💡 RECOMMENDATIONS:")
        if results['positions_working']:
            print(f"   1. ✅ TradeStation API is working - focus on frontend debugging")
            print(f"   2. Check frontend network requests and error handling")
            print(f"   3. Verify frontend API URL configuration")
            print(f"   4. Test frontend with browser developer tools")
        else:
            print(f"   1. ❌ Fix TradeStation API connectivity issues")
            print(f"   2. Check authentication and credentials")
            print(f"   3. Verify account permissions")
