...
class ReviewBackendTester:
 ...
 print(
 "\n Skipping positions and balances tests - no account ID available"
 )
 ...
 schema_notes.append(
 " TradeStation API returns data nested under 'data' field - frontend may need to access response.data.* instead of response.*"
 )
 ...
 schema_notes.append(
 " Some expected fields may be missing or named differently than expected"
 )
 ...
 schema_notes.append(
 " Portfolio positions may use 'type' field instead of 'position_type'"
 )
 ...
 print(" MODERATE - Some backend APIs have issues that need attention")
 else:
