...
class AdvancedScoringEngine:
    ...
    async def calculate_comprehensive_score(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            scores = {
                ...
                "options_score": await self._calculate_options_score(
                    options_data, symbol
                ),
            ...
            result = {
                ...
                "data_quality": self._assess_data_quality(
                    technical_data, fundamental_data, options_data
                ),
                "recommendation": self._get_recommendation(total_score, scores),
                "risk_level": self._assess_risk_level(
                    risk_data, scores.get("risk_score", 50)
                ),
    ...
    async def _get_technical_data(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.ts_client:
                ...
                if hasattr(self.ts_client, "auth") and hasattr(
                    self.ts_client.auth, "is_authenticated"
                ):
                    ...
                    if is_auth and hasattr(self.ts_client, "get_historical_bars"):
                        try:
                            logger.info(
                                f" Fetching historical bars for {symbol} from TradeStation..."
                            )
                            ...
                            logger.info(
                                f" Received {len(bars) if bars else 0} bars for {symbol}"
                            )

                            if bars and len(bars) > 50:
                                logger.info(
                                    f" Using TradeStation live data for {symbol}"
                                )
                                df = pd.DataFrame(bars)
                                result = self._calculate_advanced_technical_indicators(
                                    df, symbol
                                )
                                ...
                                logger.warning(
                                    f" Insufficient TradeStation data for {symbol}: {len(bars) if bars else 0} bars"
                                )
                        except Exception as e:
                            logger.error(
                                f" TradeStation technical data failed for {symbol}: {e}"
                            )
                    else:
                        logger.warning(
    ...
    def _calculate_advanced_technical_indicators(
        ...
        try:
            ...
            indicators["distance_to_resistance"] = (
                (resistance_level - current_price) / current_price
            ) * 100
            indicators["distance_to_support"] = (
    ...
    async def _calculate_technical_score(
        ...
        technical_score = (
            rsi_score * 0.3 + trend_score * 0.4 + volume_score * 0.2 + bb_score * 0.1
        )
    ...
    async def _calculate_fundamental_score(
        ...
        fundamental_score = (
            pe_score * 0.3 + roe_score * 0.25 + growth_score * 0.25 + debt_score * 0.2
        )

        return max(0, min(100, fundamental_score))

    async def _calculate_options_score(
        self, data: Dict[str, Any], symbol: str
    ) -> float:
        ...
        options_score = pcr_score * 0.4 + uv_score * 0.4 + trades_score * 0.2
    ...
    async def _calculate_sentiment_score(
        ...
        sentiment_score = news_score * 0.4 + social_score * 0.3 + analyst_score * 0.3
    ...
    async def _calculate_risk_score(self, data: Dict[str, Any], symbol: str) -> float:
        ...
        risk_score = beta_score * 0.3 + sharpe_score * 0.4 + vol_score * 0.3
    ...
    def _assess_data_quality(self, tech_data, fund_data, options_data) -> str:
        ...
        if (
            tech_data
            and tech_data.get("is_live_data")
            and tech_data.get("data_points", 0) >= 100
        ):
        ...
        try:
            ...
            return {
                ...
                "change_percent": round(
                    -5 + secrets.randbelow(1000) / 100, 2
                ),  # -5 to 5
