...
class EnhancedTickerDataManager:
    ...
    async def get_real_time_quote(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            current_live_price = info.get("currentPrice") or info.get(
                "regularMarketPrice"
            )
        ...
        try:
            ...
            if not premarket_data.empty:
                ...
                result["premarket"] = {
                    ...
                    "change": (
                        premarket_price - float(today_data["Close"].iloc[0])
                        if len(today_data) > 0
                        else 0
                    ),
            ...
            if not postmarket_data.empty:
                ...
                regular_close = (
                    float(today_data.between_time("09:30", "16:00")["Close"].iloc[-1])
                    if len(today_data.between_time("09:30", "16:00")) > 0
                    else postmarket_price
                )
                result["postmarket"] = {
    ...
    async def get_bulk_real_time_data(
        self, symbols: List[str], max_batch_size: int = 10
    ) -> List[Dict[str, Any]]:
    ...
    async def screen_stocks_enhanced(
    ...
    def _meets_criteria_enhanced(
        self, stock: Dict[str, Any], criteria: Dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error in criteria matching for {stock.get('symbol', 'unknown')}: {str(e)}"
            )
            return False
