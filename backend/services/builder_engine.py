...
def leg_payoff_expiry(
 ...
 kind = kind.upper()
 side = side.upper()
 sign = 1 if side == "BUY" else -1 # BUY = long position (positive payoff when ITM)
...
def price_structure(payload: Dict[str, Any], provider=None) -> Dict[str, Any]:
 ...
 # pdf lognormal integrată pe [0,∞) = 1; discretizarea pe [x_min,x_max] e ok pentru range 10–30%
 ...
 # greeks (aggregate) – per 1 contract, scalate la qty și 100 multiplier unde e cazul
 ...
 # delta e pe unitate subiect; vega e pe 1.0 (nu %); theta e per an → zilnic ca /365
 ...
 # Mock chain data for quality calc (in real implementation, get from provider)
