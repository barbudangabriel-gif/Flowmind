...
async def scan_earnings_calendar(
    ...
    min_fwd_vol_factor: float = Query(
        1.3, ge=1.0, le=3.0, description="Minimum forward vol factor"
    ),
):
    """
    ...
    try:
        logger.info(
            f"Scanning earnings calendar: {days_ahead} days ahead, min factor {min_fwd_vol_factor}"
        )

        # Scan for opportunities
        opportunities = await term_structure_agent.scan_earnings_calendar(
        ...
        logger.info(
            f"Found {len(filtered)} opportunities (filtered from {len(opportunities)})"
        )
...
async def get_top_opportunities(
    limit: int = Query(
        20, ge=1, le=100, description="Number of opportunities to return"
    ),
    min_backtest_win_rate: float = Query(
        0.65, ge=0.0, le=1.0, description="Minimum backtest win rate"
    ),
):
    """
...
async def backtest_symbol(
    symbol: str,
    lookback_quarters: int = Query(
        8, ge=1, le=20, description="Number of quarters to backtest"
    ),
    ...
    try:
        ...
        if mindfolio_id not in active_supervisors:
            ...
            active_supervisors[mindfolio_id] = TermStructureSupervisor(
                mindfolio_id=mindfolio_id, module_config=module_config
            )
        ...
        opportunity = {
            ...
            "trade_recommendation": {"position_size": position_size},
            "backtest": {"win_rate": 0.75},  # Demo value
        }
        ...
        if auto_execute:
            ...
            return {
                ...
                "strike": strike,
                "position_size": position_size,
                "validation": {"approved": True, "warnings": validation.warnings},
                "timestamp": datetime.now().isoformat(),
            }
        else:
...
async def get_module_stats(mindfolio_id: str):
    ...
    try:
        ...
        return {"status": "success", "data": {**stats, "module_active": True}}
...
async def resume_module(mindfolio_id: str):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=400, detail="Cannot resume from EMERGENCY_STOP"
            )
