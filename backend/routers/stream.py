...
async def initialize_websocket():
    ...
    if not uw_api_token:
        logger.warning(
            "⚠️  UW_API_TOKEN not set - WebSocket streaming DISABLED"
        )
        logger.warning(
...
async def stream_gex_strike_expiry(websocket: WebSocket, ticker: str):
    """
    Stream real-time gamma exposure (GEX) updates per strike AND expiration.
    
    **🆕 NEW CHANNEL** - Most granular GEX data available (added UW 2025-01-22)
    
    **Connection:** wss://your-backend.com/api/stream/ws/gex-strike-expiry/SPY
    ...
    **Frontend Integration:**
    - Use for heatmap visualization (expiry × strike)
    - Track gamma walls by expiration
    - Monitor zero-DTE positioning
    """
    ticker = ticker.upper()
    ...
    async def gex_strike_expiry_handler(ch: str, payload: dict):
        """Broadcast GEX strike/expiry updates to all subscribed frontend clients"""
        message = {
            "channel": ch,
            "timestamp": datetime.now().isoformat(),
            "ticker": ticker,
    ...
    try:
        ...
        logger.error(f"Error in GEX strike/expiry stream for {ticker}: {e}")
