...
class TradeStationAuthService:
    ...
    def __init__(self):
        ...
        self.redirect_uri = os.getenv(
            ...
            "http://localhost:8001/api/auth/tradestation/callback",
        )
        ...
        if not self.client_id or not self.client_secret:
            raise ValueError(
                "TradeStation API credentials not configured in environment variables"
            )
        ...
        logger.info(
            f"TradeStation Auth Service initialized - Environment: {self.environment}"
        )
    ...
    async def exchange_code_for_tokens(self, authorization_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                logger.info(
                    f" TradeStation authentication successful! Token expires in {expires_in}s"
                )
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f" TradeStation tokens refreshed successfully - expires in {expires_in}s"
                    )
                    return True
                else:
                    logger.error(
    ...
    async def ensure_valid_token(self) -> bool:
        ...
        if self.needs_refresh():
            if self.refresh_token:
                logger.info(
                    "🔄 TradeStation token needs refresh, attempting automatic refresh..."
                )
