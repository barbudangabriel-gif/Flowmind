...
class BlackScholesCalculator:
    ...
    def calculate_greeks(
        ...
        if time_to_expiry <= 0:
            ...
            delta = (
                1.0
                if (option_type == OptionType.CALL and stock_price > strike)
                else 0.0
            )
        ...
        if option_type == OptionType.CALL:
            delta = norm.cdf(d1)
            theta = (
                (-stock_price * pdf_d1 * volatility) / (2 * math.sqrt(time_to_expiry))
                - risk_free_rate
                * strike
...
class OptionsStrategyEngine:
    ...
    def analyze_strategy(self, strategy: StrategyConfig) -> StrategyAnalysis:
        ...
        for price in price_array:
            ...
            for leg in strategy.legs:
                ...
                if leg.action == ActionType.BUY:
                    leg_pnl = (
                        (option_value - leg.premium) * leg.quantity * 100
                    )  # $100 per contract
                else:  # SELL
        ...
        for i in range(len(pnl_array) - 1):
            if (pnl_array[i] <= 0 <= pnl_array[i + 1]) or (
                pnl_array[i] >= 0 >= pnl_array[i + 1]
            ):
                # Linear interpolation pentru exact breakeven
                if pnl_array[i + 1] != pnl_array[i]:
                    breakeven = price_array[i] + (
                        price_array[i + 1] - price_array[i]
    ...
    def create_strategy_by_name(
        ...
        if strategy_name == "Long Call":
            return self.create_long_call_strategy(
                symbol,
                stock_price,
                parameters.get("strike", stock_price),
                days_to_expiry,
                volatility,
                risk_free_rate,
            )
            ...
            return self.create_covered_call_strategy(
                symbol,
                stock_price,
                parameters.get("call_strike", stock_price + 5),
                days_to_expiry,
                volatility,
                risk_free_rate,
            )
