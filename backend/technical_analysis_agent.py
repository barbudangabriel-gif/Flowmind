...
class TechnicalAnalysisAgent:
    ...
    async def generate_technical_analysis(
        self, symbol: str, include_smc: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if (
                not price_data
                or not price_data.get("daily")
                or len(price_data["daily"]) < 20
            ):
                logger.warning(
                    f"Insufficient data for {symbol}, using fallback analysis"
                )
                return self._generate_fallback_analysis(symbol)

            # 2. Smart Money Concepts Analysis
            if include_smc:
                smc_analysis = await self._analyze_smart_money_concepts(
                    price_data, symbol
                )
            else:
            ...
            technical_scores = await self._analyze_technical_indicators(
                price_data, symbol
            )
            ...
            recommendation = self._generate_technical_recommendation(
                composite_score, technical_scores, smc_analysis
            )

            # 8. Calculate confidence level
            confidence = self._calculate_technical_confidence(
                technical_scores, timeframe_analysis
            )

            # 9. Extract key technical signals
            key_signals = self._extract_key_technical_signals(
                technical_scores, smc_analysis, sr_analysis
            )

            # 10. Risk/Entry analysis
            risk_entry_analysis = self._analyze_risk_entry_levels(
                price_data, sr_analysis
            )
        ...
        try:
            ...
            if ts_auth.is_authenticated():
                ...
                if quotes and len(quotes) > 0:
                    ...
                    logger.info(
                        f" Live data for {symbol}: ${quote.last} ({quote.change_percent:.2f}%) Volume: {quote.volume}"
                    )
                    return live_data
                else:
    ...
    def _generate_fallback_analysis(self, symbol: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "entry_timing": {"entry_signal": "WAIT", "timing_confidence": "low"},
            "timestamp": datetime.now().isoformat(),
    ...
    async def _fetch_multi_timeframe_data(self, symbol: str) -> Dict[str, List[Dict]]:
        ...
        try:
            logger.info(
                f" Fetching multi-timeframe data for {symbol} from TradeStation"
            )
            ...
            if not ts_auth.is_authenticated():
                logger.warning(
                    f" TradeStation not authenticated, using mock data for {symbol}"
                )
            ...
            for timeframe, params in timeframes.items():
                try:
                    ...
                    if bars and len(bars) > 0:
                        ...
                        logger.info(
                            f" Fetched {len(converted_bars)} {timeframe} bars for {symbol}"
                        )
                    else:
            ...
            if "daily" not in timeframe_data or len(timeframe_data["daily"]) < 20:
                logger.warning(
                    f" Insufficient data from TradeStation for {symbol}, using mock"
                )
                return self._get_enhanced_mock_price_data(symbol)
            ...
            logger.info(
                f" Successfully fetched multi-timeframe data for {symbol}: {list(final_data.keys())} timeframes"
            )
            return final_data
    ...
    async def _analyze_smart_money_concepts(
        self, price_data: Dict[str, List[Dict]], symbol: str
    ) -> Dict[str, Any]:
        """
            ...
            smc_signals.append(
                {
                    ...
                    "strength": (
                        "strong" if len(order_blocks["bullish_ob"]) > 2 else "moderate"
                    ),
            ...
            smc_signals.append(
                {
                    ...
                    "strength": (
                        "strong" if len(order_blocks["bearish_ob"]) > 2 else "moderate"
                    ),
        ...
        logger.info(
            f"Smart Money Concepts for {symbol}: Score={smc_score:.1f}, Signals={len(smc_signals)}"
        )
    ...
    async def _analyze_technical_indicators(
        ...
        try:
            ...
            indicator_scores["stochastic"] = self._calculate_stochastic_score(
                daily_data
            )
            indicator_scores["williams_r"] = self._calculate_williams_r_score(
                daily_data
            )

            # 2. Trend Indicators
            indicator_scores["macd"] = self._calculate_macd_score(daily_data)
            indicator_scores["ema_crossover"] = self._calculate_ema_crossover_score(
                daily_data
            )
            ...
            indicator_scores["volume_trend"] = self._calculate_volume_trend_score(
                daily_data
            )
            indicator_scores["vwap"] = self._calculate_vwap_score(daily_data)

            # 4. Volatility Indicators
            indicator_scores["bollinger_bands"] = self._calculate_bollinger_score(
                daily_data
            )
    ...
    def _detect_order_blocks(self, price_data: List[Dict]) -> Dict[str, List[Dict]]:
        ...
        return {
            "bullish_ob": bullish_ob[-3:],
            "bearish_ob": bearish_ob[-3:],
        }  # Last 3 OBs
    ...
    def _detect_fair_value_gaps(self, price_data: List[Dict]) -> Dict[str, List[float]]:
        ...
        for i in range(1, len(price_data) - 1):
            ...
            if (
                prev["high"] < next_candle["low"]
                and (next_candle["low"] - prev["high"]) / prev["high"]
                > self.smc_params["fvg_min_size"]
            ):
            ...
            if (
                prev["low"] > next_candle["high"]
                and (prev["low"] - next_candle["high"]) / next_candle["high"]
                > self.smc_params["fvg_min_size"]
            ):
    ...
    def _analyze_market_structure(
        self, daily_data: List[Dict], weekly_data: List[Dict]
    ) -> Dict[str, Any]:
        ...
        higher_highs = sum(
            1 for i in range(1, len(daily_highs)) if daily_highs[i] > daily_highs[i - 1]
        )
        higher_lows = sum(
            1 for i in range(1, len(daily_lows)) if daily_lows[i] > daily_lows[i - 1]
        )
    ...
    def _analyze_liquidity_zones(self, price_data: List[Dict]) -> Dict[str, bool]:
        ...
        if swing_lows:
            ...
            if recent_low < min(swing_lows) * (
                1 - self.smc_params["liquidity_sweep_threshold"]
            ):
                swept_lows = True

        if swing_highs:
            recent_high = max(candle["high"] for candle in latest_candles)
            if recent_high > max(swing_highs) * (
                1 + self.smc_params["liquidity_sweep_threshold"]
            ):
                swept_highs = True
    ...
    def _analyze_premium_discount_zones(
        self, daily_data: List[Dict], weekly_data: List[Dict]
    ) -> Dict[str, Any]:
    ...
    def _get_enhanced_mock_price_data(self, symbol: str) -> Dict[str, List[Dict]]:
        ...
        for i in range(12):
            ...
            monthly_data.append(
                {
                    "date": (datetime.now() - timedelta(days=(11 - i) * 30)).strftime(
                        "%Y-%m-%d"
                    ),
        ...
        for i in range(20):
            ...
            weekly_data.append(
                {
                    "date": (datetime.now() - timedelta(days=(19 - i) * 7)).strftime(
                        "%Y-%m-%d"
                    ),
                    "open": round(open_price, 2),
                    "high": round(high, 2),
        ...
        for i in range(50):
            ...
            daily_data.append(
                {
                    "date": (datetime.now() - timedelta(days=49 - i)).strftime(
                        "%Y-%m-%d"
                    ),
                    "open": round(open_price, 2),
                    "high": round(high, 2),
        ...
        for i in range(100):
            ...
            h4_data.append(
                {
                    "date": (datetime.now() - timedelta(hours=(99 - i) * 4)).strftime(
                        "%Y-%m-%d %H:00"
                    ),
                    "open": round(open_price, 2),
                    "high": round(high, 2),
        ...
        for i in range(200):
            ...
            h1_data.append(
                {
                    "date": (datetime.now() - timedelta(hours=199 - i)).strftime(
                        "%Y-%m-%d %H:00"
                    ),
                    "open": round(open_price, 2),
                    "high": round(high, 2),
        ...
        for i in range(400):
            ...
            m15_data.append(
                {
                    "date": (
                        datetime.now() - timedelta(minutes=(399 - i) * 15)
                    ).strftime("%Y-%m-%d %H:%M"),
                    "open": round(open_price, 2),
                    "high": round(high, 2),
                    "low": round(low, 2),
        ...
        for i in range(500):
            ...
            m1_data.append(
                {
                    "date": (datetime.now() - timedelta(minutes=499 - i)).strftime(
                        "%Y-%m-%d %H:%M"
                    ),
                    "open": round(open_price, 2),
                    "high": round(high, 2),
    ...
    def _calculate_rsi_score(self, price_data: List[Dict]) -> float:
        ...
        closes = [
            float(candle["close"]) for candle in price_data[-14:]
        ]  # 14-period RSI
        ...
        avg_loss = (
            sum(losses) / len(losses) if losses else 0.01
    ...
    def _calculate_adx_score(self, price_data: List[Dict]) -> float:
        ...
        for i in range(1, len(recent_data)):
            ...
            dm_plus.append(
                max(0, high - prev_high) if high - prev_high > prev_low - low else 0
            )
            dm_minus.append(
                max(0, prev_low - low) if prev_low - low > high - prev_high else 0
            )
    ...
    def _calculate_ichimoku_score(self, price_data: List[Dict]) -> float:
        ...
        # TK Cross
        if len(price_data) > 26:
            prev_tenkan_high = max(
                float(candle["high"]) for candle in price_data[-10:-1]
            )
    ...
    def _calculate_bollinger_score(self, price_data: List[Dict]) -> float:
        ...
        variance = sum((close - sma) ** 2 for close in recent_closes) / len(
            recent_closes
        )
    ...
    def _analyze_multi_timeframe_confluence(
        ...
        return {
            ...
            "timeframe_alignment": self._check_enhanced_timeframe_alignment(
                primary_analysis, secondary_analysis
            ),
        }

    def _analyze_primary_timeframes(
        self, price_data: Dict[str, List[Dict]]
    ) -> Dict[str, Any]:
        ...
        if "monthly" in price_data and len(price_data["monthly"]) >= 3:
            ...
            monthly_closes = [
                float(candle["close"]) for candle in monthly_data[-6:]
            ]  # Last 6 months
            monthly_trend = (
                "bullish" if monthly_closes[-1] > monthly_closes[0] else "bearish"
            )
            monthly_strength = (
                abs(monthly_closes[-1] - monthly_closes[0]) / monthly_closes[0]
            )
        ...
        if "weekly" in price_data and len(price_data["weekly"]) >= 5:
            weekly_data = price_data["weekly"]
            weekly_closes = [
                float(candle["close"]) for candle in weekly_data[-10:]
            ]  # Last 10 weeks
            weekly_trend = (
                "bullish" if weekly_closes[-1] > weekly_closes[0] else "bearish"
            )
            weekly_strength = (
                abs(weekly_closes[-1] - weekly_closes[0]) / weekly_closes[0]
            )
        ...
        if "daily" in price_data and len(price_data["daily"]) >= 10:
            daily_data = price_data["daily"]
            daily_closes = [
                float(candle["close"]) for candle in daily_data[-20:]
            ]  # Last 20 days
        ...
        if primary_scores:
            weighted_score = sum(
                score["score"] * score["weight"] for score in primary_scores.values()
            )
        else:
            weighted_score = 50.0
    ...
    def _analyze_secondary_timeframes(
        self, price_data: Dict[str, List[Dict]]
    ) -> Dict[str, Any]:
        ...
        if "h4" in price_data and len(price_data["h4"]) >= 10:
            ...
            h4_closes = [
                float(candle["close"]) for candle in h4_data[-24:]
            ]  # Last 24 periods (4 days)
        ...
        if "h1" in price_data and len(price_data["h1"]) >= 20:
            ...
            h1_closes = [
                float(candle["close"]) for candle in h1_data[-48:]
        ...
        if secondary_scores:
            weighted_score = sum(
                score["score"] * score["weight"] for score in secondary_scores.values()
            )
        else:
            weighted_score = 50.0
    ...
    def _calculate_vwap_analysis(
        self, price_data: List[Dict], timeframe: str
    ) -> Dict[str, Any]:
        ...
        # VWAP trend analysis
        vwap_slope = (
            (vwap_values[-1] - vwap_values[-10]) / 10 if len(vwap_values) >= 10 else 0
        )
    ...
    def _analyze_market_sessions(
        ...
        async def _analyze_session_analysis(
            self, symbol: str, price_data: Dict[str, List[Dict]]
        ) -> Dict[str, Any]:
            ...
            try:
                ...
                if hasattr(self, "ts_client") and self.ts_client:
                    try:
                        from tradestation_auth_service import (
                            tradestation_auth_service as ts_auth,
                        )
                        ...
                        if ts_auth.is_authenticated():
                            ...
                            if quotes and len(quotes) > 0:
                                ...
                                logger.info(
                                    f" Live price for {symbol}: ${current_price}"
                                )
                    except Exception as e:
                ...
                if daily_data and len(daily_data) > 0:
                    ...
                    recent_days = (
                        daily_data[-5:] if len(daily_data) >= 5 else daily_data
                    )
                    ...
                    premarket_range = base_price * 0.02
                    regular_range = base_price * 0.04
                    postmarket_range = base_price * 0.015
                    ...
                    logger.info(
                        f" Live session levels for {symbol} (base: ${base_price}): {session_levels}"
                    )
    ...
    def _check_enhanced_timeframe_alignment(
        self, primary_analysis: Dict, secondary_analysis: Dict
    ) -> str:
                    ...
                    vwap_signals.append(
                        {
                            ...
                            "strength": (
                                "strong"
                                if abs(vwap_analysis["score"] - 50) > 25
                                else "moderate"
                            ),
                            "price_vs_vwap_pct": vwap_analysis.get(
                                "price_vs_vwap_pct", 0
                            ),
    ...
    def _analyze_support_resistance_levels(
        self, price_data: Dict[str, List[Dict]], symbol: str
    ) -> Dict[str, Any]:
        ...
        resistance_levels = [
            level
            for level in pivots["resistance"] + weekly_levels["resistance"]
            if level > current_price
        ]
        support_levels = [
            level
            for level in pivots["support"] + weekly_levels["support"]
            if level < current_price
        ]
    ...
    def _find_pivot_points(self, price_data: List[Dict]) -> Dict[str, List[float]]:
        ...
        for i in range(2, len(price_data) - 2):
            ...
            if (
                high > float(price_data[i - 1]["high"])
                and high > float(price_data[i - 2]["high"])
                and high > float(price_data[i + 1]["high"])
                and high > float(price_data[i + 2]["high"])
            ):
            ...
            if (
                low < float(price_data[i - 1]["low"])
                and low < float(price_data[i - 2]["low"])
                and low < float(price_data[i + 1]["low"])
                and low < float(price_data[i + 2]["low"])
            ):
    ...
    def _find_weekly_levels(self, weekly_data: List[Dict]) -> Dict[str, List[float]]:
        ...
        return {"support": major_support, "resistance": major_resistance}

    def _analyze_risk_entry_levels(
        self, price_data: Dict[str, List[Dict]], sr_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        risk_reward_ratio = (
            reward_to_resistance / risk_to_support if risk_to_support > 0 else 0
        )
        ...
        if position_risk_pct > 0:
            recommended_position_size = min(
                100, (max_risk_percentage / position_risk_pct) * 100
            )
        else:
        ...
        return {
            ...
            "entry_recommendation": self._get_entry_recommendation(
                entry_score, risk_reward_ratio
            ),
        }

    def _get_entry_recommendation(
        self, entry_score: float, risk_reward_ratio: float
    ) -> str:
    ...
    def _calculate_technical_composite_score(
        self, technical_scores: Dict[str, float]
    ) -> float:
        ...
        for category, indicators in categories.items():
            ...
            category_scores = [
                technical_scores.get(indicator, 50.0)
                for indicator in indicators
                if indicator in technical_scores
            ]
    ...
    def _calculate_technical_confidence(
        self, technical_scores: Dict[str, float], timeframe_analysis: Dict[str, Any]
    ) -> str:
        ...
        if (
            score_std < 12
            and alignment_bonus > 0
            and (avg_score > 70 or avg_score < 30)
        ):
            return "high"
        elif score_std < 20 and (
            alignment_bonus > 0 or (avg_score > 65 or avg_score < 35)
        ):
            return "medium"
        else:
            return "low"

    def _extract_key_technical_signals(
        self,
        technical_scores: Dict[str, float],
        smc_analysis: Dict[str, Any],
        ...
        sorted_indicators = sorted(
            technical_scores.items(), key=lambda x: abs(x[1] - 50), reverse=True
        )
            ...
            key_signals.append(
                {
                    ...
                    "direction": (
                        "bullish" if signal.get("impact", 0) > 0 else "bearish"
                    ),
