                ...
                try:
                    ...
                    if isinstance(data, list) and len(data) >= 2:
                        ...
                        if len(self.message_handlers) <= 3:
                            logger.debug(f"📬 Received on {channel}: {str(payload)[:100]}...")
                    ...
                    logger.debug(f"No messages for {TIMEOUT_LENGTH}s, checking connection...")
...
async def test_connection(api_token: str, channels: List[str] = None, duration: int = 30):
    """
    ...
    logger.info(f"📊 Test summary: {message_count} messages received from {len(channels)} channels")
