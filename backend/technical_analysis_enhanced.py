...
class TechnicalAnalyzer:
    ...
    async def analyze_stock_technical(
        ...
        try:
            ...
            technical_data["support_resistance"] = self._find_support_resistance_levels(
                df
            )

            # 6. Calculate overall technical score
            technical_data["technical_score"] = self._calculate_technical_score(
                technical_data
            )
    ...
    def _analyze_overall_trend(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            short_trend = self._calculate_trend_strength(
                close_prices, 20
            )  # 20-day trend
            medium_trend = self._calculate_trend_strength(
                close_prices, 50
            )  # 50-day trend
            long_trend = self._calculate_trend_strength(
                close_prices, 100
            )  # 100-day trend if available
            ...
            sma20 = (
                df["SMA20"].iloc[-1]
                if not pd.isna(df["SMA20"].iloc[-1])
                else current_price
            )
            sma50 = (
                df["SMA50"].iloc[-1]
                if not pd.isna(df["SMA50"].iloc[-1])
                else current_price
            )
    ...
    def _calculate_technical_indicators(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            indicators["macd"] = {
                ...
                "crossover": (
                    "BULLISH" if macd.iloc[-1] > macd_signal.iloc[-1] else "BEARISH"
                ),
                "momentum": (
                    "INCREASING"
            ...
            indicators["stochastic"] = {
                ...
                "signal": self._interpret_stochastic(
                    stoch_k.iloc[-1] if not pd.isna(stoch_k.iloc[-1]) else 50
                ),
            }
            ...
            if not pd.isna(bb_high.iloc[-1]) and not pd.isna(bb_low.iloc[-1]):
                bb_position = (current_price - bb_low.iloc[-1]) / (
                    bb_high.iloc[-1] - bb_low.iloc[-1]
                )
            ...
            indicators["bollinger_bands"] = {
                ...
                "squeeze": (
                    abs(bb_high.iloc[-1] - bb_low.iloc[-1])
                    < abs(bb_high.iloc[-10] - bb_low.iloc[-10])
                    if len(df) > 10
                    else False
                ),
            }
            ...
            indicators["williams_r"] = {
                "current": (
                    williams_r.iloc[-1] if not pd.isna(williams_r.iloc[-1]) else -50
                ),
                "signal": self._interpret_williams_r(
                    williams_r.iloc[-1] if not pd.isna(williams_r.iloc[-1]) else -50
                ),
            }
    ...
    def _analyze_price_action(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            # Price momentum
            momentum_5d = (
                (close.iloc[-1] / close.iloc[-6] - 1) * 100 if len(close) > 5 else 0
            )
            momentum_20d = (
                (close.iloc[-1] / close.iloc[-21] - 1) * 100 if len(close) > 20 else 0
            )
    ...
    def _find_support_resistance_levels(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            nearest_support = (
                max(support_levels) if support_levels else current_price * 0.9
            )
            ...
            return {
                ...
                "resistance_strength": len(
                    [
                        ...
                        if abs(h - nearest_resistance) / nearest_resistance < 0.02
                    ]
                ),
                "distance_to_support": (
    ...
    def _calculate_technical_score(
        ...
        try:
            ...
            if trend.get("direction") == "BULLISH":
                ...
                trend_score = 50 + (
                    10 if "BULLISH" in trend.get("direction", "") else -10
                )
            ...
            total_score = (
                ...
                * self.indicators_weights["momentum_indicators"]
    ...
    def _generate_technical_signals(
        ...
        try:
            ...
            if (
                ...
                and macd_data.get("momentum") == "INCREASING"
            ):
                signals.append(
                    {
                        "type": "MOMENTUM",
