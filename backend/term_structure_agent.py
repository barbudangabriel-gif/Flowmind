...
class TermStructureAgent:
        ...
        for stock in earnings_calendar:
            try:
                ...
                front_exp, back_exp = self._find_optimal_expirations(
                    chain_data["expirations"], stock["earnings_date"]
                )
                ...
                if fwd_vol_factor < self.min_fwd_vol_factor:
                    logger.debug(
                        f"{stock['symbol']} factor {fwd_vol_factor:.2f} too low"
                    )
                    continue
                ...
                expected_profit = (
                    front_call - expected_front_value_post_er - (spread_cost * 0.1)
                )  # 10% slippage
                ...
                backtest_results = await self._backtest_calendar_spread(
                    symbol=stock["symbol"], lookback_quarters=8
                )
                ...
                opportunity = {
                    ...
                    "risk_rating": self._calculate_risk_rating(
                        fwd_vol_factor, backtest_results
                    ),
    ...
    def _find_optimal_expirations(
        self, expirations: List[str], earnings_date: str
    ) -> tuple:
        """
            ...
            if exp_dt < earnings_dt:
                ...
                if 2 <= days_before_er <= 7:
                    if not front_month or abs(days_before_er - 3) < abs(
                        (earnings_dt - datetime.strptime(front_month, "%Y-%m-%d")).days
                        - 3
                    ):
                        front_month = exp_str
            ...
            # Back month: expires 20-40 days after earnings
                ...
                if 20 <= days_after_er <= 40:
                    if not back_month or abs(days_after_er - 30) < abs(
                        (datetime.strptime(back_month, "%Y-%m-%d") - earnings_dt).days
                        - 30
                    ):
                        back_month = exp_str
    ...
    def _get_iv_for_strike(
        self, chain_data: Dict, expiration: str, strike: float, option_type: str
    ) -> Optional[float]:
    ...
    def _calculate_risk_rating(
        self, fwd_vol_factor: float, backtest_results: Dict
    ) -> str:
    ...
    def _recommend_position_size(
        self, opportunity_score: float, backtest_results: Dict
    ) -> int:
        """
