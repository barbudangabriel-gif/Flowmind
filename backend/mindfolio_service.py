...
class MindfolioMetrics:
    """Calculate mindfolio performance metrics"""

    @staticmethod
    def calculate_total_return(positions: List[Position]) -> Dict[str, float]:
        """Calculate total mindfolio return metrics"""
        total_market_value = sum(pos.market_value for pos in positions)
        total_cost_basis = sum(
            pos.average_price * abs(pos.quantity) for pos in positions
        )
    ...
    def analyze_sector_allocation(
        ...
        """Analyze mindfolio allocation by asset type and sector"""
    ...
    def calculate_position_metrics(positions: List[Position]) -> List[Dict[str, Any]]:
        ...
        for position in positions:
            ...
            position_dict["risk_score"] = (
                abs(position.unrealized_pnl_percent)
                * abs(position.market_value)
                / 10000
            )
            position_dict["position_type"] = (
...
class RiskAnalyzer:
    """Mindfolio risk analysis and management"""
    ...
    def calculate_concentration_risk(positions: List[Position]) -> Dict[str, Any]:
        ...
        # Calculate metrics
        max_position_weight = (
            position_weights[0]["weight_percent"] if position_weights else 0
        )
    ...
    def calculate_risk_metrics(
        positions: List[Position], account_balance: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if positions:
            ...
            unrealized_pnl_volatility = sum(
                (pnl - avg_pnl) ** 2 for pnl in pnl_values
            ) / len(pnl_values)
            unrealized_pnl_volatility = (
                unrealized_pnl_volatility**0.5
            )  # Standard deviation
        ...
        risk_flags = {
            ...
            "over_leveraged": (
                total_market_value > total_equity * 2 if total_equity > 0 else False
            ),
            "significant_losses": any(
                pos.unrealized_pnl_percent < -20 for pos in positions
            ),
        }
        ...
        return {
            ...
            "volatility_metrics": {
                ...
                "total_market_value": total_market_value,
                "mindfolio_leverage": (
                    total_market_value / total_equity if total_equity > 0 else 0
                ),
            },
            "risk_flags": risk_flags,
            "recommendations": RiskAnalyzer._generate_risk_recommendations(
                risk_flags, concentration_analysis
            ),
        }

    @staticmethod
    def _generate_risk_recommendations(
        risk_flags: Dict[str, bool], concentration: Dict[str, Any]
    ) -> List[str]:
        ...
        if risk_flags["high_concentration"]:
            recommendations.append(
                f"Consider reducing position in largest holding ({concentration['max_position_weight']:.1f}% of mindfolio)"
            )

        if risk_flags["low_diversification"]:
            recommendations.append(
                "Increase diversification by adding positions in different sectors/asset classes"
            )

        if risk_flags["high_volatility"]:
            recommendations.append(
                "Consider reducing position sizes to lower mindfolio volatility"
            )

        if risk_flags["over_leveraged"]:
            recommendations.append(
                "Reduce leverage by closing some positions or adding capital"
            )

        if risk_flags["significant_losses"]:
            recommendations.append(
                "Review positions with >20% losses and consider stop-loss strategies"
            )

        if not any(risk_flags.values()):
            recommendations.append("Mindfolio risk profile appears well-balanced")

        return recommendations


class PerformanceAnalyzer:
    """Mindfolio performance analysis"""

    @staticmethod
    async def calculate_performance_metrics(
        positions: List[Position], orders: List[Any], days_back: int = 30
    ) -> Dict[str, Any]:
        """Calculate comprehensive performance metrics"""

        # Current mindfolio metrics
        current_metrics = MindfolioMetrics.calculate_total_return(positions)
        ...
        recent_orders = [
            ...
            and (datetime.now() - order.timestamp).days <= days_back
        ]

        filled_orders = [
            order for order in recent_orders if order.status in ["FLL", "PFL"]
        ]
        ...
        most_active_symbols = sorted(
            symbol_trades.keys(), key=lambda x: len(symbol_trades[x]), reverse=True
        )[:10]

        # Performance summary
        performance_data = {
            "analysis_period_days": days_back,
            "current_mindfolio": current_metrics,
            "trading_activity": {
...
class MindfolioService:
    """Main mindfolio service orchestrating all mindfolio operations"""

    def __init__(self, ts_client: TradeStationClient):
        self.ts_client = ts_client
        self.metrics = MindfolioMetrics()
        self.risk_analyzer = RiskAnalyzer()
        self.performance_analyzer = PerformanceAnalyzer()

    async def get_comprehensive_mindfolio_analysis(
        self, account_id: str
    ) -> Dict[str, Any]:
        """Get comprehensive mindfolio analysis combining all metrics"""
        try:
            ...
            for pos_data in account_summary.get("positions", []):
                if isinstance(pos_data, dict):
                    position = Position(
                        ...
                        unrealized_pnl_percent=pos_data.get(
            ...
            mindfolio_metrics = self.metrics.calculate_total_return(positions)
            ...
            risk_analysis = self.risk_analyzer.calculate_risk_metrics(
                positions, account_summary.get("balances", {})
            )

            # Performance analysis
            performance_metrics = (
                await self.performance_analyzer.calculate_performance_metrics(
                    positions, account_summary.get("recent_orders", [])
                )
            )
            ...
            comprehensive_analysis = {
                ...
                "mindfolio_metrics": mindfolio_metrics,
                ...
                "summary": {
                    ...
                    "total_value": mindfolio_metrics["total_market_value"],
                    "total_pnl": mindfolio_metrics["total_unrealized_pnl"],
                    "overall_return": mindfolio_metrics["total_return_percent"],
                    "risk_level": risk_analysis["risk_level"],
                    "risk_score": risk_analysis["risk_score"],
                },
            }

            logger.info(f"Generated comprehensive mindfolio analysis for {account_id}")
            return comprehensive_analysis

        except Exception as e:
            logger.error(f"Error in comprehensive mindfolio analysis: {str(e)}")
            raise
