...
class SmartMoneyAnalyzer:
    ...
    async def analyze_smart_money_concepts(self, symbol: str, period: str = "3mo") -> Dict[str, Any]:
        """Comprehensive Smart Money analysis for a symbol"""
        try:
            # Get detailed price data
            ticker = yf.Ticker(symbol)
            df = ticker.history(period=period, interval="1h")  # Hourly data for better precision
            ...
            result = {
                ...
                "order_blocks": [self._order_block_to_dict(ob) for ob in order_blocks[-10:]],  # Last 10
                "fair_value_gaps": [self._fvg_to_dict(fvg) for fvg in fair_value_gaps if not fvg.filled],
                "liquidity_sweeps": [self._liquidity_sweep_to_dict(ls) for ls in liquidity_sweeps[-5:]],
                "market_structure": {
                    "current_trend": market_structure["trend"],
                    "structure_points": [self._structure_point_to_dict(sp) for sp in market_structure["points"][-10:]],
                ...
                "price_action": {
                    ...
                    "candlestick_patterns": candlestick_patterns[-10:],  # Recent patterns
                ...
                "trading_signals": self._generate_trading_signals(
                    order_blocks, fair_value_gaps, support_resistance, 
    ...
    def _identify_order_blocks(self, df: pd.DataFrame) -> List[OrderBlock]:
        ...
        for i in range(10, len(df) - 5):
            ...
            if (current_candle['price_change'] > 0.02 and  # 2%+ move
                current_candle['high_volume'] and
            ...
            elif (current_candle['price_change'] < -0.02 and  # 2%+ drop
                  current_candle['high_volume'] and
                  current_candle['Close'] < current_candle['Open']):
        ...
        for i in range(2, len(df)):
            ...
            if prev_candle['High'] < next_candle['Low']:
                ...
                if gap_size > 0:  # Valid gap
                    ...
                    for j in range(i+1, min(len(df), i+50)):  # Check next 50 candles
                        if df.iloc[j]['Low'] <= prev_candle['High']:
                            filled = True
                            fill_percentage = 100.0
                            break
                        elif df.iloc[j]['Low'] < next_candle['Low']:
                            fill_percentage = ((next_candle['Low'] - df.iloc[j]['Low']) / gap_size) * 100
                ...
                if gap_size > 0:  # Valid gap
                    ...
                    for j in range(i+1, min(len(df), i+50)):  # Check next 50 candles
                        if df.iloc[j]['High'] >= prev_candle['Low']:
                            ...
                            fill_percentage = ((df.iloc[j]['High'] - next_candle['High']) / gap_size) * 100
    ...
    def _identify_liquidity_sweeps(self, df: pd.DataFrame) -> List[LiquiditySweep]:
        ...
        for i in range(20, len(df)):
            ...
            if (current['High'] > prev_recent_high and 
                current['Close'] < current['High'] * 0.995): # Price retreated after sweep
                
                # Calculate significance based on volume and price action
                volume_ratio = current['Volume'] / df['Volume'].rolling(20).mean().iloc[i]
            ...
            # Low liquidity sweep
            elif (current['Low'] < prev_recent_low and 
                current['Close'] > current['Low'] * 1.005): # Price recovered after sweep
    ...
    def _calculate_premium_discount_zones(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if len(df) < 50:
            return {'premium': 0, 'discount': 0, 'equilibrium': 0, 'current_zone': 'equilibrium'}
        ...
        return {
            ...
            'premium_percentage': ((current_price - recent_low) / range_size) * 100 if range_size > 0 else 50
        }
    ...
    def _identify_support_resistance(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for i in range(10, len(df) - 10):
            ...
            if all(df.iloc[j]['High'] <= current_high for j in range(i-10, i+11) if j != i):
                levels.append({'price': float(current_high), 'type': 'resistance', 'strength': 1})
            
            # Check for swing low
            if all(df.iloc[j]['Low'] >= current_low for j in range(i-10, i+11) if j != i):
        ...
        resistance_levels = [l for l in levels if l['price'] > current_price and l['type'] == 'resistance']
        support_levels = [l for l in levels if l['price'] < current_price and l['type'] == 'support']
    ...
    def _identify_candlestick_patterns(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for i in range(2, len(df)):
            ...
            # Hammer pattern
            elif (lower_shadow > body_size * 2 and upper_shadow < body_size * 0.5 and 
            ...
            elif (upper_shadow > body_size * 2 and lower_shadow < body_size * 0.5 and 
                current['Close'] < current['Open']):
                pattern_found = {
                    'name': 'Shooting Star',
                    'type': 'reversal',
    ...
    def _identify_supply_demand_zones(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for i in range(20, len(df) - 5):
            ...
            if (df.iloc[i+1]['Close'] > df.iloc[i]['Close'] * 1.02 and # 2% move
                df.iloc[i+1]['Volume'] > df.iloc[i-20:i]['Volume'].mean() * 1.5):
            ...
            elif (df.iloc[i+1]['Close'] < df.iloc[i]['Close'] * 0.98 and # 2% drop
                df.iloc[i+1]['Volume'] > df.iloc[i-20:i]['Volume'].mean() * 1.5):
    ...
    def _analyze_volume_profile(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'high_volume_bars': recent_high_volume_bars[-10:], # Last 10 high volume bars
            'average_volume': float(df['Volume'].mean()),
            'current_volume_ratio': float(df_temp['volume_ratio'].iloc[-1])
        }
 
    def _generate_smart_money_verdict(self, order_blocks, fair_value_gaps, liquidity_sweeps, 
        market_structure, bos_choch, current_price) -> Dict[str, Any]:
        """Generate overall Smart Money verdict"""
        signals = []
        
        # Analyze Order Blocks
        recent_bullish_obs = [ob for ob in order_blocks if ob.type == 'bullish' and not ob.tested]
        recent_bearish_obs = [ob for ob in order_blocks if ob.type == 'bearish' and not ob.tested]
        
        if len(recent_bullish_obs) > len(recent_bearish_obs):
            signals.append(('bullish', 'order_blocks', 'More untested bullish order blocks'))
        elif len(recent_bearish_obs) > len(recent_bullish_obs):
            signals.append(('bearish', 'order_blocks', 'More untested bearish order blocks'))
        
        # Analyze Fair Value Gaps
        unfilled_bullish_fvgs = [fvg for fvg in fair_value_gaps if fvg.type == 'bullish' and not fvg.filled]
        unfilled_bearish_fvgs = [fvg for fvg in fair_value_gaps if fvg.type == 'bearish' and not fvg.filled]
        ...
        if market_structure['trend'] == 'bullish':
            signals.append(('bullish', 'market_structure', 'Bullish market structure (HH, HL)'))
        elif market_structure['trend'] == 'bearish':
            signals.append(('bearish', 'market_structure', 'Bearish market structure (LH, LL)'))
    ...
    def _generate_trading_signals(self, order_blocks, fair_value_gaps, support_resistance, 
        market_structure, current_price) -> List[Dict[str, Any]]:
    ...
    def _calculate_ob_strength(self, df: pd.DataFrame, current_idx: int, origin_idx: int) -> str:
        """Calculate Order Block strength based on various factors"""
        volume_ratio = df.iloc[current_idx]['Volume'] / df['Volume'].rolling(20).mean().iloc[current_idx]
        price_move = abs(df.iloc[current_idx]['Close'] - df.iloc[origin_idx]['Close']) / df.iloc[origin_idx]['Close']
