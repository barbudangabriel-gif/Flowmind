...
class LegSchema(BaseModel):
    ...
    expiry: Optional[str] = Field(None, pattern=r"^\d{4}-\d{2}-\d{2}$", description="Expiration date (YYYY-MM-DD)")
...
class BuilderPriceRequest(BaseModel):
    ...
    symbol: str = Field(..., pattern=r"^[A-Z]{1,5}$", description="Stock ticker (1-5 uppercase letters)")
    expiry: str = Field(..., pattern=r"^\d{4}-\d{2}-\d{2}$", description="Expiration date (YYYY-MM-DD)")
    legs: List[LegSchema] = Field(..., min_items=1, max_items=4, description="Strategy legs (1-4)")
    ...
    range_pct: float = Field(0.15, ge=0.05, le=0.5, description="Price range % (0.05-0.5)")
    dte: Optional[int] = Field(None, ge=1, le=730, description="Days to expiration (1-730)")
...
class OptionsChainRequest(BaseModel):
    ...
    expiry: Optional[str] = Field(None, pattern=r"^\d{4}-\d{2}-\d{2}$", description="Specific expiration")
...
class FlowSummaryRequest(BaseModel):
    ...
    symbol: Optional[str] = Field(None, pattern=r"^[A-Z]{1,5}$", description="Filter by symbol")
...
class OptimizerSuggestRequest(BaseModel):
    ...
    budget: Optional[float] = Field(None, gt=0, le=100000, description="Max capital (USD)")
    dte: int = Field(30, ge=1, le=180, description="Days to expiration")
    risk_bias: float = Field(0.0, ge=-1.0, le=1.0, description="Risk preference (-1 to 1)")
