...
class TermStructureSupervisor:
    ...
    async def validate_trade(
        self, opportunity: Dict, ml_prediction: Optional[Dict] = None
        ...
        if len(self.positions) >= self.max_positions:
            return TradeValidationResult(
                approved=False, reason=f"Max positions ({self.max_positions}) reached"
            )
        ...
        # All checks passed
        approval_msg = (
            f"Trade validated: {symbol} calendar spread, ${total_cost:,.2f} cost"
        )
        ...
        return TradeValidationResult(
            approved=True, reason=approval_msg, warnings=warnings
        )
    ...
    async def _calculate_position_pnl(self, position: Dict) -> float:
        ...
        front_decay = (
            position["front_premium"] * (days_held / position["dte_front"]) * 0.7
        )
    ...
    def _check_risk_alerts(self, position: Dict) -> List[Dict]:
        ...
        if (
            days_to_earnings <= 5
            and position.get("current_iv_front", 0) < position["entry_iv_front"] * 0.90
        ):
            alerts.append(
                {
                    "type": "LOW_IV_WARNING",
    ...
    def get_module_stats(self) -> Dict:
        ...
        return {
            ...
            "budget_utilization_pct": (
                (self.budget_used / self.budget * 100) if self.budget > 0 else 0
            ),
            "daily_pnl": self.daily_pnl,
            "total_pnl": self.total_pnl,
            "total_pnl_pct": (
                (self.total_pnl / self.budget * 100) if self.budget > 0 else 0
            ),
            "positions_count": len(self.positions),
