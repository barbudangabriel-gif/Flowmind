...
async def get_stock_quote(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "change": (
                float(latest["Close"] - history.iloc[-2]["Close"])
                if len(history) > 1
                else 0.0
            ),
            "change_percent": (
                float(
                    (
                        (latest["Close"] - history.iloc[-2]["Close"])
...
async def get_technical_indicators(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get MACD
        macd_data, macd_meta = ti.get_macd(
        ...
        latest_macd = {
            ...
            "histogram": (
                macd_data.iloc[0]["MACD_Hist"] if not macd_data.empty else None
            ),
        }

        # Get SMA
        sma20_data, sma20_meta = ti.get_sma(
            symbol=symbol, interval="daily", time_period=20, series_type="close"
        )
        sma50_data, sma50_meta = ti.get_sma(
            symbol=symbol, interval="daily", time_period=50, series_type="close"
        )
        ...
        return {
            ...
            "sma": {
                "sma20": (
                    float(sma20_data.iloc[0]["SMA"]) if not sma20_data.empty else None
                ),
                "sma50": (
                    float(sma50_data.iloc[0]["SMA"]) if not sma50_data.empty else None
                ),
            },
        }
    except Exception as e:
...
async def get_stock_history(
    symbol: str,
    period: str = Query(
        "1y", description="Period: 1d, 5d, 1mo, 3mo, 6mo, 1y, 2y, 5y, 10y, ytd, max"
    ),
):
    """Get historical stock data"""
...
async def add_mindfolio_item(item: MindfolioItemCreate):
    ...
    mindfolio_item = MindfolioItem(
        **item.dict(),
        ...
        profit_loss_percent=(
            (current_data["price"] - item.purchase_price) / item.purchase_price
        )
        * 100,
    )
...
async def get_mindfolio():
    ...
    for item_data in items:
        ...
        try:
            ...
            item_data["profit_loss"] = (
                current_data["price"] - item_data["purchase_price"]
            ) * item_data["shares"]
            item_data["profit_loss_percent"] = (
                (current_data["price"] - item_data["purchase_price"])
            ...
            item_data["current_price"] = item_data.get(
                "current_price", item_data["purchase_price"]
            )
            item_data["current_value"] = item_data.get(
...
async def get_market_overview():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching market overview: {str(e)}"
        )
