...
class TradingRiskManager:
    ...
    async def validate_order(
        ...
        try:
            ...
            if not quotes:
                validation_result["errors"].append(f"Unable to get quote for {order.symbol}")
            ...
            if order.price:
                price_deviation = abs(order.price - current_quote.last) / current_quote.last
            ...
            current_position = next((pos for pos in positions if pos.symbol == order.symbol), None)
...
class TradingService:
    ...
    async def validate_order(self, account_id: str, order: OrderRequest) -> Dict[str, Any]:
    ...
    async def place_order(
        ...
        try:
            # Validate order
            validation = await self.risk_manager.validate_order(order, account_id, self.client)
            ...
            response = await self.client.place_order(
                ...
                symbol=order.symbol,
                quantity=order.quantity if order.side in [OrderSide.BUY, OrderSide.BUY_TO_COVER] else -order.quantity,
            ...
            logger.info(f"Order placed: Account {account_id}, Symbol {order.symbol}, Quantity {order.quantity}, Type {order.order_type}")
    ...
    async def get_trading_summary(self, account_id: str, days_back: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            filled_orders = [order for order in orders if order.status in ["FLL", "PFL"]]
            ...
            for order in filled_orders:
                estimated_value = abs(order.quantity) * (order.price or 100)  # Rough estimate
            ...
            return {
                ...
                "order_summary": {
                    ...
                    "success_rate": (len(filled_orders) / total_orders * 100) if total_orders > 0 else 0
                },
                "volume_analysis": {
