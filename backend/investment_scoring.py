...
class StockScanner:
 ...
 # TODO[OPS-008]: Integrare TradeStation API pentru lista completă de tickere
 ...
 if not stock_data:
 logger.warning(f" {ticker}: Nu s-au putut obține datele stock")
 continue
 ...
 result["scan_id"] = (
 f"scan_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
 )

 scanned_results.append(result)
 logger.info(f" {ticker}: Score {result['total_score']:.1f}")
 else:
 logger.warning(f" {ticker}: Nu s-a putut calcula scorul")
 ...
 logger.info(
 f" Scanare completă: {len(top_stocks)} acțiuni top din {processed} procesate"
 )
 return summary
 ...
 try:
 cursor = (
 scanned_stocks_collection.find({}).sort("total_score", -1).limit(limit)
 )
...
class InvestmentScorer:
 ...
 scores["profitability_score"] = self._calculate_profitability_score(
 stock_data
 )
 scores["dividend_score"] = self._calculate_dividend_score(stock_data)
 scores["financial_health"] = self._calculate_financial_health_score(
 stock_data
 )
 ...
 scores["price_action_score"] = technical_scores.get(
 "price_action_score", 50
 )
 scores["support_resistance_score"] = technical_scores.get(
 ...
 rating = self._get_enhanced_investment_rating(
 total_score, technical_data, sentiment_data
 )

 # Create enhanced recommendation explanation
 explanation = self._generate_enhanced_explanation(
 stock_data, scores, total_score, technical_data, sentiment_data
 )
 ...
 fundamental_weighted_sum = sum(
 scores[k] * self.weights[k] for k in fundamental_keys if k in scores
 )
 fundamental_weights_sum = sum(
 self.weights[k] for k in fundamental_keys if k in self.weights
 )
 fundamental_score = (
 fundamental_weighted_sum / fundamental_weights_sum
 if fundamental_weights_sum > 0
 else 50
 )

 # Calculate technical score
 technical_weighted_sum = sum(
 scores[k] * self.weights[k] for k in technical_keys if k in scores
 )
 technical_weights_sum = sum(
 self.weights[k] for k in technical_keys if k in self.weights
 )
 technical_score = (
 technical_weighted_sum / technical_weights_sum
 if technical_weights_sum > 0
 else 50
 )

 # Calculate sentiment score
 sentiment_weighted_sum = sum(
 scores[k] * self.weights[k] for k in sentiment_keys if k in scores
 )
 sentiment_weights_sum = sum(
 self.weights[k] for k in sentiment_keys if k in self.weights
 )
 sentiment_score = (
 sentiment_weighted_sum / sentiment_weights_sum
 if sentiment_weights_sum > 0
 else 50
 )
 ...
 "investment_horizon": self._recommend_enhanced_investment_horizon(
 scores, technical_data, sentiment_data
 ),
 "key_strengths": self._identify_enhanced_key_strengths(
 scores, technical_data, sentiment_data
 ),
 ...
 "trend_strength": technical_data.get("trend_analysis", {}).get(
 "strength", 50
 ),
 "key_indicators": self._get_key_technical_indicators(
 technical_data
 ),
 ...
 "sentiment_analysis": {
 "direction": sentiment_data.get("insights", {}).get(
 "direction", "UNKNOWN"
 ),
 "strength": sentiment_data.get("insights", {}).get(
 "strength", "Unknown"
 ),
 "market_mood": sentiment_data.get("market_mood", "UNCERTAIN"),
 "confidence": sentiment_data.get("confidence_level", 0.0),
 "dominant_source": sentiment_data.get("insights", {}).get(
 "dominant_source", "None"
 ),
 "key_themes": sentiment_data.get("insights", {}).get(
 "key_themes", []
 ),
 "total_mentions": sentiment_data.get("total_mentions", 0),
 "summary": sentiment_data.get("insights", {}).get(
 ...
 logger.error(
 f"Error calculating enhanced investment score for {stock_data.get('symbol', 'unknown')}: {str(e)}"
 )
 ...
 logger.error(
 f"Stock data keys: {list(stock_data.keys()) if stock_data else 'None'}"
 )
 ...
 # Get sentiment data
 sentiment_direction = sentiment_data.get("insights", {}).get(
 "direction", "UNKNOWN"
 )
 ...
 and sentiment_direction == "BULLISH"
 ...
 and sentiment_direction == "BEARISH"
 and sentiment_confidence > 0.5
 )
 ...
 sentiment_strength = sentiment_data.get("insights", {}).get(
 "strength", "Unknown"
 )
 market_mood = sentiment_data.get("market_mood", "UNCERTAIN")
 ...
 f"bullish technical trend (strength: {trend_strength:.0f}%)"
 )
 elif trend_direction == "BEARISH":
 context_parts.append(
 f"bearish technical trend (strength: {trend_strength:.0f}%)"
 )
 else:
 ...
 context_parts.append(
 f"{sentiment_strength.lower()} positive market sentiment ({market_mood.lower()})"
 )
 elif sentiment_direction == "BEARISH":
 context_parts.append(
 f"{sentiment_strength.lower()} negative market sentiment ({market_mood.lower()})"
 )
 else:
 ...
 sentiment_score = scores.get("sentiment_score", 50) * self.weights.get(
 "sentiment_score", 0.2
 )
 ...
 explanation += (
 f"strong fundamentals, {context_parts[0]}, and {context_parts[1]}."
 )
 elif strongest[0] == "technicals":
 explanation += (
 f"{context_parts[0]}, decent fundamentals, and {context_parts[1]}."
 )
 else:
 explanation += (
 f"{context_parts[1]}, {context_parts[0]}, and balanced fundamentals."
 )
 ...
 trend_reliability = (
 technical_data.get("trend_analysis", {})
 .get("short_term", {})
 .get("reliability", "LOW")
 )
 ...
 def _recommend_enhanced_investment_horizon(
 self,
 scores: Dict[str, float],
 technical_data: Dict[str, Any],
 ...
 and sentiment_direction != "NEGATIVE"
 ):
 return "LONG-TERM"
 else:
 return "MEDIUM-TERM"

 def _identify_enhanced_key_strengths(
 self,
 scores: Dict[str, float],
 technical_data: Dict[str, Any],
 sentiment_data: Dict[str, Any],
 ...
 self, stock_data: Dict[str, Any], scores: Dict[str, float], total_score: float
 ) -> str:
 ...
 self, stock_data: Dict[str, Any], scores: Dict[str, float]
 ) -> str:
 ...
 "rating": "HOLD",
 "individual_scores": {
 k.replace("_score", ""): 50.0 for k in self.weights.keys()
 },
