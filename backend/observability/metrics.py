...
cache_hits_total = Counter(
    "flowmind_cache_hits_total", "Number of cache hits", ["cache_type", "key_prefix"]
)
...
cache_size_bytes = Gauge(
    "flowmind_cache_size_bytes", "Approximate cache size in bytes", ["cache_type"]
)

cache_entries = Gauge(
    "flowmind_cache_entries", "Number of entries in cache", ["cache_type"]
)
...
db_queries_total = Counter(
    "flowmind_db_queries_total", "Total database queries", ["operation", "collection"]
)
...
system_info = Info("flowmind_system", "FlowMind system information")

uptime_seconds = Gauge("flowmind_uptime_seconds", "Application uptime in seconds")
        ...
        if is_async:
            ...
            async def async_wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    api_request_duration_seconds.labels(
                        endpoint=endpoint_name, method="GET"
                    ).observe(duration)
            ...
            def sync_wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    api_requests_total.labels(
                        endpoint=endpoint_name, method="GET", status=status
                    ).inc()
            ...
            async def async_wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    external_api_errors_total.labels(
                        provider=provider, error_type=type(e).__name__
                    ).inc()
                    raise
                finally:
                    duration = time.time() - start_time
...
if __name__ == "__main__":
    ...
    api_requests_total.labels(
        endpoint="/api/builder/price", method="POST", status="success"
    ).inc()

    api_request_duration_seconds.labels(
        endpoint="/api/builder/price", method="POST"
    ).observe(0.254)

    strategies_priced_total.labels(strategy_type="iron_condor", symbol="TSLA").inc()
