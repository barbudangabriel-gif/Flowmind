...
class TradeStationClient:
    ...
    def __init__(self, auth_handler: TradeStationAuth):
        ...
        logger.info(
            f"TradeStation client initialized for {auth_handler.environment} environment"
        )
        ...
        for pos_data in response.get("Positions", []):
            try:
                position = Position(
                    ...
                    unrealized_pnl_percent=float(
                        pos_data.get("UnrealizedProfitLossPercent", 0.0)
                    ),
                )
    ...
    async def get_orders(
        self, account_id: str, since_date: Optional[datetime] = None
    ) -> List[Order]:
        ...
        response = await self._make_request(
            "GET", f"/brokerage/accounts/{account_id}/orders", params=params
        )
        orders = []
        ...
        for order_data in response.get("Orders", []):
            try:
                ...
                order = Order(
                    ...
                    price=(
                        float(order_data.get("LimitPrice"))
                ...
                logger.warning(
                    f"Error parsing order data for {order_data.get('OrderID', 'unknown')}: {e}"
                )
                continue
    ...
    async def get_quote(self, symbols: Union[str, List[str]]) -> List[Quote]:
        ...
        for quote_data in response.get("Quotes", []):
            try:
                ...
                logger.warning(
                    f"Error parsing quote data for {quote_data.get('Symbol', 'unknown')}: {e}"
                )
                continue
    ...
    async def place_order(
        ...
        response = await self._make_request(
            "POST", f"/brokerage/accounts/{account_id}/orders", json=order_data
        )
    ...
    async def cancel_order(self, account_id: str, order_id: str) -> Dict[str, Any]:
        ...
        response = await self._make_request(
            "DELETE", f"/brokerage/accounts/{account_id}/orders/{order_id}"
        )
    ...
    async def get_account_summary(self, account_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_orders_task = self.get_orders(
                account_id, datetime.now() - timedelta(days=7)
            )
            ...
            if isinstance(positions, Exception):
                ...
                logger.info(
                    f"Positions retrieved successfully: {len(positions)} positions"
                )
            ...
            if isinstance(recent_orders, Exception):
                ...
                logger.info(
                    f"Orders retrieved successfully: {len(recent_orders)} orders"
                )

            # Check for authentication issues
            if not positions and not balances:
                logger.warning(
                    "No positions or balances retrieved - possible authentication issue"
                )
            ...
            summary = {
                ...
                "totals": {
                    ...
                    "total_market_value": sum(
                        pos.get("market_value", 0) for pos in positions_data
                    ),
                    "total_unrealized_pnl": sum(
                        pos.get("unrealized_pnl", 0) for pos in positions_data
                    ),
                },
            }

            logger.info(
                f"Generated account summary for {account_id}: {len(positions_data)} positions, {len(orders_data)} recent orders"
            )
            return summary
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to retrieve account summary: {str(e)}"
            )
