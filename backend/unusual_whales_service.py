...
class UnusualWhalesService:
    def __init__(self):
        self.api_token = os.getenv("UNUSUAL_WHALES_API_KEY") or os.getenv(
            "UW_API_TOKEN"
        )
        ...
        if not self.api_token:
            logger.warning(
                "UNUSUAL_WHALES_API_KEY or UW_API_TOKEN not set in environment variables"
            )
    ...
    async def get_options_flow_alerts(
        ...
        """
        try:
            params = {"limit": limit, "min_premium": minimum_premium}
            ...
            if not response.get("data"):
                logger.warning(
                    "No options flow data from API, using mock data as fallback"
                )
    ...
    def _process_real_flow_alert(self, alert: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "strike_type": (
                    f"{int(strike)}{option_type[0].upper()}" if strike else "N/A"
                ),
                ...
                "sentiment": sentiment,
                "unusual_activity": premium > 200000
                or vol_oi > 3.0
                or alert.get("volume", 0) > 1000,
                "action": action,
    ...
    async def get_institutional_holdings(
        ...
        try:
            ...
            response = await self._make_request(
                f"/api/stock/{ticker}/institutional", params
            )

            if not response:
                logger.warning(
                    f"No institutional holdings data for {ticker}, using fallback"
                )
            ...
            logger.error(
                f"Error fetching institutional holdings for {ticker}: {str(e)}"
            )
            return self._get_mock_institutional(ticker)
    ...
    async def get_recent_dark_pool_activity(
        self, limit: Optional[int] = 100, minimum_volume: Optional[int] = 100000
    ) -> List[Dict[str, Any]]:
        """Fetch recent dark pool trading activity"""
        try:
            params = {"limit": limit, "volume_gte": minimum_volume}
            ...
            logger.info(
                f"Successfully processed {len(processed_trades)} dark pool trades"
            )
    ...
    def _process_dark_pool_trade(self, trade: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            significance = self._determine_dark_pool_significance(
                dark_volume, dark_percentage
            )

            # Parse timestamp
            timestamp = trade.get("executed_at") or trade.get(
            ...
            return {
                ...
                "institutional_signal": dark_percentage > 15
                and dark_volume > 50000,  # Adjusted thresholds
            }
        except Exception as e:
    ...
    def _determine_dark_pool_significance(
        self, volume: int, dark_percentage: float
    ) -> str:
            ...
            for index, row in df.iterrows():
                if isinstance(row, pd.Series):
                    result[str(index)] = {
                        str(k): (
                            float(v)
                            if pd.api.types.is_numeric_dtype(type(v))
                            else str(v)
                        )
                    ...
                    result[str(index)] = (
                        float(row)
                        if pd.api.types.is_numeric_dtype(type(row))
                        else str(row)
                    )
            return result
        except Exception as e:
    ...
    async def _get_mock_options_flow(self) -> List[Dict[str, Any]]:
        ...
        for i in range(80):
            ...
            strike_multiplier = self._secure_choices_weighted(
                strike_multipliers, strike_weights
            )
            ...
            # Expiration date
            expiration_date = (datetime.now() + timedelta(days=dte)).strftime(
                "%Y-%m-%d"
            )
            ...
            mock_trades.append(
                {
                    ...
                    "disclosure_date": (datetime.now() - timedelta(days=i)).strftime(
                        "%Y-%m-%d"
                    ),
    ...
    # ANALYSIS METHODS
    async def analyze_options_flow_patterns(
        ...
        return {
            ...
            "top_tickers": {
                str(k): float(v)
                for k, v in df.groupby("symbol")["premium"]
                .sum()
                .nlargest(5)
                .to_dict()
                .items()
    ...
    async def analyze_dark_pool_patterns(
        ...
        return {
            ...
            "top_tickers_by_volume": {
                str(k): int(v)
                for k, v in df.groupby("ticker")["dark_volume"]
                .sum()
                .nlargest(5)
                .to_dict()
                .items()
    ...
    async def analyze_congressional_patterns(
        self, trades: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        party_analysis = (
            df.groupby("party")
            .agg({"transaction_amount": ["sum", "count", "mean"]})
            .round(2)
        )
        ...
        sector_concentration = (
            sector_analysis.iloc[0][("transaction_amount", "sum")]
        ...
        return {
            ...
            "party_breakdown": (
                self._convert_df_to_dict(party_analysis)
                if len(party_analysis) > 0
                else {}
            ),
            "transaction_breakdown": (
                self._convert_df_to_dict(transaction_analysis)
                if len(transaction_analysis) > 0
                else {}
            ),
            "sector_breakdown": (
                self._convert_df_to_dict(sector_analysis.head())
                if len(sector_analysis) > 0
                else {}
            ),
            "insights": insights,
    ...
    async def get_etf_data_for_futures(
        ...
        try:
            ...
            logger.info(
                "Unusual Whales API doesn't provide direct stock/ETF data - skipping"
            )
            return {}
    ...
    async def get_stock_screener_data(
        self, limit: Optional[int] = 100, exchange: Optional[str] = "all"
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                "Using mock stock screener data - UW API doesn't provide screener endpoint"
            )
    ...
    async def filter_stocks_by_criteria(
        self, criteria: Dict[str, Any], exchange: Optional[str] = "all"
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not response.get("data"):
                logger.warning(
                    "No filtered stock data from Unusual Whales, using mock data"
                )
    ...
    def _process_stock_data(self, stock_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "pe_ratio": (
                float(stock_data.get("pe_ratio", 0))
            ...
            "dividend_yield": (
                float(stock_data.get("dividend_yield", 0))
    ...
    async def _get_mock_stock_screener_data(
        self, limit: int = 100, exchange: str = "all"
    ) -> List[Dict[str, Any]]:
        ...
        for i, stock in enumerate(stock_list[:limit]):
            ...
            mock_stock = {
                ...
                "market_cap": self._secure_randint(
                    10000000000, 3000000000000
                ),  # 10B to 3T
                "pe_ratio": (
                    round(self._secure_uniform(10, 50), 2)
                ...
                "exchange": (
                    ...
                    if exchange == "sp500"
                        ...
                        "NASDAQ"
                        if exchange == "nasdaq"
                        else self._secure_choice(["NYSE", "NASDAQ"])
                    )
                ),
                "dividend_yield": (
                    round(self._secure_uniform(0, 5), 2)
                    if secrets.randbelow(100) > 40
                    else None
                ),
                "unusual_activity": secrets.randbelow(100)
                > 80,  # 20% chance of unusual activity
                "options_flow_signal": self._secure_choice(
                    ["bullish", "bearish", "neutral", "neutral", "neutral"]
                ),  # Mostly neutral
            }
    ...
    async def _get_mock_filtered_stocks(
        self, criteria: Dict[str, Any], exchange: str = "all"
    ) -> List[Dict[str, Any]]:
        ...
        for stock in base_stocks:
            ...
            if (
                ...
                and stock["market_cap"] < criteria["min_market_cap"] * 1000000
            ):
                include = False
            if (
                criteria.get("max_market_cap")
                and stock["market_cap"] > criteria["max_market_cap"] * 1000000
            ):
                include = False

            # P/E filters
            if criteria.get("min_pe") and (
                not stock["pe_ratio"] or stock["pe_ratio"] < criteria["min_pe"]
            ):
                include = False
            if criteria.get("max_pe") and (
                not stock["pe_ratio"] or stock["pe_ratio"] > criteria["max_pe"]
            ):
                include = False
            ...
            if (
                ...
                and stock["sector"] != criteria["sector"]
            ):
                include = False

            # Change percent filters
            if (
                criteria.get("min_change")
                and stock["change_percent"] < criteria["min_change"]
            ):
                include = False
            if (
                criteria.get("max_change")
                and stock["change_percent"] > criteria["max_change"]
            ):
                include = False
