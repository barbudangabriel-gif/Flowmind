"""
Mindfolio Charts Service
Provides data and analysis for mindfolio performance charts and allocation visualizations
"""
...
class MindfolioChartsService:
    def __init__(self):
        self.logger = logger

    async def get_mindfolio_performance_data(
        self,
        mindfolio_id: str,
        ...
        """
        Get mindfolio performance data for charting
        """
        try:
            self.logger.info(f"Fetching performance data for mindfolio {mindfolio_id}")

            # Generate performance data based on filters
            performance_data = await self._generate_performance_data(
                mindfolio_id, filter_type, asset_type, timeframe, start_date, end_date
            )

            # Calculate mindfolio summary
            mindfolio_summary = await self._calculate_mindfolio_summary(
                mindfolio_id, performance_data
            )
            ...
            self.logger.error(f"Error getting mindfolio performance data: {str(e)}")
            return {
                "status": "error",
                "message": f"Failed to fetch performance data: {str(e)}",
            }

    async def get_mindfolio_allocation_data(
        self,
        mindfolio_id: str,
        ...
        """
        Get mindfolio allocation data for pie charts and distribution analysis
        """
        try:
            self.logger.info(f"Fetching allocation data for mindfolio {mindfolio_id}")

            # Generate allocation data
            allocation_data = await self._generate_allocation_data(
                mindfolio_id, filter_type, asset_type
            )
            ...
            self.logger.error(f"Error getting mindfolio allocation data: {str(e)}")
            return {
                "status": "error",
    ...
    async def _generate_allocation_data(
        self, mindfolio_id: str, filter_type: str, asset_type: str
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def _calculate_mindfolio_summary(
        self, mindfolio_id: str, performance_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        Calculate mindfolio summary statistics
        """
        ...
        # Mindfolio values based on mindfolio_id
        mindfolio_values = {"htech-15t": {"total_value": 139902.60, "cash": 100000.00}}

        mindfolio_info = mindfolio_values.get(
            mindfolio_id, mindfolio_values["htech-15t"]
        )

        total_pnl = latest_data["combined_pnl"] - first_data["combined_pnl"]

        return {
            "total_value": mindfolio_info["total_value"],
            "total_pnl": round(total_pnl, 2),
            "cash_balance": mindfolio_info["cash"],
            "pnl_percentage": (
                round((total_pnl / mindfolio_info["total_value"]) * 100, 2)
                if mindfolio_info["total_value"] > 0
                else 0
            ),
        }
