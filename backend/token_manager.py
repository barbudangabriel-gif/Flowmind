        ...
        while self.running:
            try:
                ...
                # Check if tokens are expired
                    ...
                    logger.warning(
                        "Tokens have expired. Manual re-authentication required."
                    )
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "auth_status": {
                ...
                "needs_refresh": (
                    self.auth.needs_refresh() if self.auth.is_authenticated() else False
                ),
                "token_expires": (
                    self.auth.token_expires.isoformat()
                    if self.auth.token_expires
                    else None
                ),
                "expires_in_minutes": (
                    int(
                        (self.auth.token_expires - datetime.utcnow()).total_seconds()
                        / 60
                    )
                    if self.auth.token_expires
                    and self.auth.token_expires > datetime.utcnow()
                    else 0
                ),
            },
        }
