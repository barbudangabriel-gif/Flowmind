...
class InvestmentScoringAgent:
    ...
    async def generate_investment_score(self, symbol: str, user_context: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'agent_version': '1.0',
                'data_sources': ['unusual_whales_options_flow', 'dark_pool', 'congressional_trades', 'discount_premium_analysis', 'risk_reward_optimization']
            }
    ...
    async def _analyze_signal_components(self, uw_data: Dict[str, Any], symbol: str) -> Dict[str, float]:
        ...
        signal_scores['options_flow_bullish'] = self._analyze_options_flow(uw_data['options_flow'])
        
        # 3. Dark Pool Activity
        signal_scores['dark_pool_strength'] = self._analyze_dark_pool(uw_data['dark_pool'])
        
        # 4. Congressional Activity
        signal_scores['congressional_interest'] = self._analyze_congressional(uw_data['congressional'])
        
        # 5. Risk/Reward Optimization (new component)
        signal_scores['risk_reward_favorability'] = await self._analyze_risk_reward(uw_data, symbol)
        
        # 6. Strategy Viability (new component)
        signal_scores['strategy_quality'] = self._analyze_strategy_quality(uw_data.get('strategies', []))
        
        # 7. Multi-Timeframe Alignment (trend confirmation)
        signal_scores['timeframe_alignment'] = await self._analyze_timeframe_alignment(symbol)
        
        # 8. Volume Profile Analysis
        signal_scores['volume_profile'] = await self._analyze_volume_profile(uw_data, symbol)
    ...
    async def _analyze_discount_opportunity(self, uw_data: Dict[str, Any], symbol: str) -> float:
        ...
        try:
            ...
            call_premium = sum(opt.get('premium', 0) for opt in options_flow if opt.get('option_type') == 'call')
            put_premium = sum(opt.get('premium', 0) for opt in options_flow if opt.get('option_type') == 'put')
            ...
            recent_trades = sorted(options_flow, key=lambda x: x.get('timestamp', ''), reverse=True)[:20]
            if recent_trades:
                avg_price = sum(t.get('stock_price', 0) for t in recent_trades if t.get('stock_price')) / len(recent_trades)
    ...
    def _analyze_options_flow(self, options_flow: List[Dict]) -> float:
        ...
        try:
            ...
            for flow in options_flow:
                ...
                if (opt_type == 'call' and sentiment == 'bullish') or (opt_type == 'put' and sentiment == 'bearish'):
                    bullish_premium += premium
                # Bearish: Calls sold, Puts bought  
                elif (opt_type == 'call' and sentiment == 'bearish') or (opt_type == 'put' and sentiment == 'bullish'):
    ...
    def _analyze_congressional(self, congressional: List[Dict]) -> float:
        ...
        try:
            # Count buys vs sells
            buys = sum(1 for trade in congressional if trade.get('type', '').lower() == 'buy')
            sells = sum(1 for trade in congressional if trade.get('type', '').lower() == 'sell')
        ...
        try:
            ...
            for strategy in strategies:
                ...
                if strategy.get('max_loss') and strategy.get('max_profit'):
                    risk_reward_ratio = abs(strategy.get('max_profit', 0) / (strategy.get('max_loss', 1) + 1))
                    
                    if risk_reward_ratio >= self.risk_reward_params['min_risk_reward_ratio']:
                        strategy_score += 30
    ...
    async def _analyze_volume_profile(self, uw_data: Dict[str, Any], symbol: str) -> float:
    ...
    def _extract_key_signals(self, uw_data: Dict[str, Any], signal_scores: Dict[str, float]) -> List[str]:
        """Extract the most important signals driving the investment score."""
        try:
            key_signals = []
            
            # Find top 3 signals by score
            sorted_signals = sorted(signal_scores.items(), key=lambda x: abs(x[1] - 50), reverse=True)[:3]
            
            for signal_name, score in sorted_signals:
                if score >= 70:
                    key_signals.append(f"Strong {signal_name.replace('_', ' ')}: {score:.1f}/100")
                elif score >= 60:
                    key_signals.append(f"Moderate {signal_name.replace('_', ' ')}: {score:.1f}/100")
                elif score <= 30:
                    key_signals.append(f"Weak {signal_name.replace('_', ' ')}: {score:.1f}/100")
            ...
            dark_pool = uw_data.get('dark_pool', [])
            if dark_pool:
                total_dark_volume = sum(dp.get('dark_pool_volume', 0) for dp in dark_pool)
        ...
        try:
            ...
            if options_flow:
                ...
                if iv_values:
                    ...
                    if avg_iv > 0.60:
                        risk_score += 20
                        risk_analysis['risk_factors'].append('High implied volatility (IV > 60%)')
            ...
            if congressional:
                ...
                if sells > len(congressional) * 0.6:
                    risk_score += 10
                    risk_analysis['risk_factors'].append('Majority of congressional trades are sells')
            
            # Mitigating factors
            if signal_scores.get('dark_pool_strength', 0) > 70:
                risk_score -= 10
                risk_analysis['mitigating_factors'].append('Strong institutional interest (dark pool)')
            
            if signal_scores.get('discount_opportunity', 0) > 70:
                risk_score -= 15
                risk_analysis['mitigating_factors'].append('Stock at significant discount')
    ...
    async def _analyze_discount_opportunity_old(self, uw_data: Dict[str, Any], symbol: str) -> float:
        """
    ...
    def _generate_recommendation_old(self, composite_score: float, signal_scores: Dict[str, float]) -> str:
    ...
    def _extract_key_signals_old(self, uw_data: Dict[str, Any], signal_scores: Dict[str, float]) -> List[Dict[str, Any]]:
    ...
    def _assess_risk_factors_old(self, uw_data: Dict[str, Any], signal_scores: Dict[str, float]) -> Dict[str, Any]:
