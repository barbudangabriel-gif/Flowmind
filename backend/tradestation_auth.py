...
class TradeStationAuth:
    ...
    def _load_tokens(self) -> bool:
        ...
        try:
            ...
            self.token_expires = (
                datetime.fromisoformat(token_data["token_expires"])
                if token_data.get("token_expires")
                else None
            )
            ...
            logger.info(
                f"TradeStation tokens loaded successfully. Expires: {self.token_expires}"
            )
            return True
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"Tokens refreshed successfully. New expiry: {self.token_expires}"
                    )
                    return True
                else:
                    logger.error(
    ...
    async def exchange_code_for_tokens(self, authorization_code: str) -> Dict[str, Any]:
        ...
        if not self.client_id or not self.client_secret:
            raise HTTPException(
                status_code=500, detail="TradeStation API credentials not configured"
            )
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            try:
                ...
                logger.info(
                    f"Successfully exchanged authorization code for tokens. Expires in {expires_in}s"
                )
