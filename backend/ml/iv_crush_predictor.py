...
class IVCrushPredictor:
 ...
 predicted_crush = base_crush + iv_adjustment + regime_adjustment + cap_adjustment
 ...
 if confidence < 0.60:
 logger.warning(f"Low confidence ({confidence:.2%}), not returning prediction")
 return None
 ...
 **kwargs
 ...
 "historical_range": {
 "min": symbol_data["last_8_quarters"][0] if symbol_data else sector_data.get("min", 0.20),
 "max": symbol_data["last_8_quarters"][-1] if symbol_data else sector_data.get("max", 0.70),
