...
class PortfolioManagementService:
    ...
    async def initialize(self):
        ...
        if not self._initialized:
            try:
                ...
                logger.info(
                    "Portfolio management service initialized with TradeStation data"
                )
            except Exception as e:
    ...
    async def _load_tradestation_positions(self, account_id: str = None):
        ...
        try:
            ...
            logger.info(
                f" Portfolio totals: ${total_value:,.2f} value, ${total_pnl:,.2f} P&L"
            )
    ...
    async def _update_portfolio_totals(self, portfolio_id: str):
        ...
        portfolio_positions = [
            pos for pos in self.positions.values() if pos.portfolio_id == portfolio_id
        ]
        ...
        logger.info(
            f"Updated portfolio {portfolio_id}: ${total_value:.2f} value, {positions_count} positions"
        )
    ...
    async def get_aggregate_view(self) -> Dict[str, Any]:
        ...
        for portfolio in self.portfolios.values():
            portfolio_positions = [
                pos for pos in all_positions if pos.portfolio_id == portfolio.id
            ]
            ...
            portfolio_breakdown[portfolio.id] = {
                ...
                "positions_count": len(portfolio_positions),
                "percentage": (
                    (portfolio_value / total_value * 100) if total_value > 0 else 0
                ),
            }
    ...
    async def get_move_history(
        ...
        if portfolio_id:
            moves = [
                move
                for move in moves
                if move.from_portfolio_id == portfolio_id
                or move.to_portfolio_id == portfolio_id
            ]

        return sorted(moves, key=lambda x: x.move_date, reverse=True)

    async def get_available_portfolios_for_move(
        self, current_portfolio_id: str
    ) -> List[Portfolio]:
