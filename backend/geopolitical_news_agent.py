...
class GeopoliticalNewsAgent:
        ...
        try:
            ...
            for position in positions:
                ...
                if ticker_intel["aggregate_sentiment"] < -0.5:
                    risk_alerts.append({
                        "symbol": symbol,
                        "severity": "high" if ticker_intel["aggregate_sentiment"] < -0.7 else "medium",
            ...
            return {
                ...
                "total_news_items": sum(len(t.get("news_items", [])) for t in ticker_news.values()),
                "last_updated": datetime.now().isoformat()
            }
    ...
    def _calculate_sentiment(self, news_items: List[Dict]) -> float:
        ...
        positive_keywords = ['beat', 'surge', 'gain', 'up', 'profit', 'growth', 'strong', 'upgrade']
        negative_keywords = ['miss', 'fall', 'down', 'loss', 'weak', 'concern', 'probe', 'downgrade']
    ...
    async def _get_options_suggestions(self, symbol: str, sentiment: float) -> List[Dict]:
    ...
    def _generate_recommendation(
        ...
        if sentiment < -0.5 and impact_level > 70:
            return "⚠️ High risk - Consider protective measures or reduce exposure"
    ...
    def _generate_portfolio_summary(
        ...
        if macro_events.get("severity") == "high":
            summary_parts.append(f"⚠️ High-impact macro events detected.")
        
        # Risk alerts
        if len(risk_alerts) > 0:
            summary_parts.append(f"🚨 {len(risk_alerts)} risk alert(s) require attention.")
        
        # Opportunities
        if len(opportunities) > 0:
            summary_parts.append(f"✅ {len(opportunities)} bullish opportunity(ies) identified.")
        ...
        return " ".join(summary_parts) if summary_parts else "No significant news today."
