...
class PricingInvestigationTester:
    ...
    def phase_1_data_sources_status(self):
        ...
        if not self.tradestation_authenticated and not self.unusual_whales_available:
            ...
            print(
                "   ⚠️  WARNING: Primary source (TradeStation) not authenticated, using fallback"
            )
        ...
        for symbol, expected in self.expected_prices.items():
            ...
            if success:
                ...
                if expected["min"] <= price <= expected["max"]:
                    ...
                    print(
                        f"   ⚠️  Price outside expected range but reasonable (${price:.2f} vs ${expected['min']}-${expected['max']})"
                    )
            ...
            if success:
                ...
                if symbol_data["basic_quote"]:
                    ...
                    if abs(price - basic_price) > 0.01:
                        print(
                            f"   ⚠️  Price difference between basic and enhanced: ${abs(price - basic_price):.2f}"
                        )
                        symbol_data["issues"].append(
            ...
            for source_name, source_data in test_results.items():
                ...
                if status == "success":
                    ...
                    if expected["min"] <= price <= expected["max"]:
                        ...
                        print(
                            f"      ⚠️  Price outside expected range (${expected['min']}-${expected['max']})"
                        )

                elif status == "not_authenticated":
                    print(
                        "      ⚠️  Not authenticated - expected for TradeStation if not logged in"
                    )
                elif status == "error":
