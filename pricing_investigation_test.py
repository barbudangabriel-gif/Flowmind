...
class PricingInvestigationTester:
 ...
 self.tests_passed += 1
 print(
 f" Passed - Status: {response.status_code}, Time: {response.elapsed.total_seconds():.2f}s"
 )
 try:
 ...
 print(
 f" - {symbol} ({info['name']}): Expected ~${info['min']}-${info['max']}"
 )
 ...
 if connection_test:
 print(
 f" Connection Test: {connection_test.get('status', 'Unknown')}"
 )
 if "accounts" in connection_test:
 print(
 f" Accounts Found: {len(connection_test.get('accounts', []))}"
 )
 if "environment" in connection_test:
 print(
 f" Environment: {connection_test.get('environment', 'Unknown')}"
 )
 ...
 print(
 f" TradeStation: {' Authenticated' if self.tradestation_authenticated else ' Not Authenticated'}"
 )
 print(
 f" Unusual Whales: {' Available' if self.unusual_whales_available else ' Not Available'}"
 )

 if not self.tradestation_authenticated and not self.unusual_whales_available:
 print(" 🚨 CRITICAL: No data sources are properly configured!")
 elif not self.tradestation_authenticated:
 print(
 " WARNING: Primary source (TradeStation) not authenticated, using fallback"
 )
 ...
 # Test 1: Basic Quote
 success, basic_data = self.run_test(
 f"Basic Quote ({symbol})", "GET", f"stocks/{symbol}", 200
 )
 if success:
 ...
 print(" CRITICAL: Price is $0.00 - no data retrieved")
 symbol_data["issues"].append("Zero price - no data")
 elif price < expected["min"] * 0.5:
 print(
 f" CRITICAL: Price significantly below expected range (${price:.2f} vs ${expected['min']}-${expected['max']})"
 )
 symbol_data["issues"].append(f"Price too low: ${price:.2f}")
 elif price > expected["max"] * 2:
 print(
 f" CRITICAL: Price significantly above expected range (${price:.2f} vs ${expected['min']}-${expected['max']})"
 )
 symbol_data["issues"].append(f"Price too high: ${price:.2f}")
 else:
 print(
 f" Price outside expected range but reasonable (${price:.2f} vs ${expected['min']}-${expected['max']})"
 )
 symbol_data["issues"].append(
 f"Price outside expected range: ${price:.2f}"
 )
 else:
 ...
 f"Enhanced Quote ({symbol})", "GET", f"stocks/{symbol}/enhanced", 200
 )
 if success:
 ...
 print(
 f" Price difference between basic and enhanced: ${abs(price - basic_price):.2f}"
 )
 symbol_data["issues"].append(
 f"Price inconsistency: Basic ${basic_price:.2f} vs Enhanced ${price:.2f}"
 )
 else:
 print(
 " Consistent pricing between basic and enhanced quotes"
 )
 else:
 ...
 success, comparison_data = self.run_test(
 f"Data Source Comparison ({symbol})",
 "GET",
 f"data-sources/test/{symbol}",
 200,
 )
 ...
 print(
 f" Price outside expected range (${expected['min']}-${expected['max']})"
 )

 elif status == "not_authenticated":
 print(
 " Not authenticated - expected for TradeStation if not logged in"
 )
 elif status == "error":
 ...
 print(" Price difference is reasonable")
 ...
 print(" Impact: Primary data source unavailable")
 print(" Symptoms: Falling back to secondary sources")
 print(" Affected: All symbols")
 ...
 expected = self.expected_prices[symbol]
 if basic_price > 0 and not (
 expected["min"] <= basic_price <= expected["max"]
 ):
 ...
 print(
 " Possible Causes: API failures, authentication issues, symbol not found"
 )
 self.root_causes.append(
 {
 "issue": "Zero Prices",
 ...
 print("\n ISSUE 3: Prices Outside Expected Ranges")
 print(f" Affected Symbols: {wrong_range_symbols}")
 print(
 " Possible Causes: Outdated data, wrong market, currency issues, cached data"
 )
 self.root_causes.append(
 {
 ...
 print("\n ISSUE 4: Price Inconsistencies")
 print(f" Affected Symbols: {inconsistent_symbols}")
 print(
 " Possible Causes: Different data sources, timing issues, caching problems"
 )
 self.root_causes.append(
 {
 ...
 print(f"\n ISSUE 5: {source.upper()} Data Source Problems")
 print(f" Failed Symbols: {failed_symbols}")
 print(" Impact: Reduced data reliability")
 self.root_causes.append(
 {
 ...
 print(" Possible Causes: Different markets, timing, data quality")
 self.root_causes.append(
 {
 ...
 critical_issues = [
 rc for rc in self.root_causes if rc["severity"] == "Critical"
 ]
 ...
 print(" Problem: Primary data source not authenticated")
 print(" Solution: Complete TradeStation OAuth authentication flow")
 print(" Steps:")
 print(" - Visit /api/auth/tradestation/login")
 print(" - Complete OAuth authorization")
 print(" - Verify authentication at /api/auth/tradestation/status")
 print(" Impact: Will provide most accurate real-time pricing")

 # Priority 2: Zero Price Issues
 zero_price_symbols = [
 s
 for s, d in self.symbol_results.items()
 ...
 print(" Solutions:")
 print(" - Check Unusual Whales API key validity")
 print(" - Verify symbol mappings in data sources")
 print(" - Implement better error handling and fallbacks")
 print(" - Add data validation before returning prices")
 ...
 print(" - Ensure Unusual Whales API key is valid and has sufficient quota")
 print(" - Implement health checks for all data sources")
 print(" - Add monitoring for data source failures")

 # Priority 4: Price Validation
 print("\n4. IMPLEMENT PRICE VALIDATION")
 print(" Problem: Prices outside expected ranges not caught")
 print(" Solutions:")
 print(" - Add price range validation for known symbols")
 print(" - Implement sanity checks (e.g., price > 0, reasonable ranges)")
 print(" - Add alerts for unusual price movements")
 print(" - Cache last known good prices as fallback")

 # Priority 5: Monitoring and Alerting
 print("\n5. ADD MONITORING AND ALERTING")
 print(" Implement:")
 print(" - Real-time price accuracy monitoring")
 print(" - Data source health dashboards")
 print(" - Automated alerts for pricing anomalies")
 print(" - Performance metrics for API response times")
 ...
 print(" 1. Re-run this test after TradeStation authentication")
 print(" 2. Monitor prices during market hours for real-time accuracy")
 print(" 3. Test with additional symbols to verify fixes")
 print(" 4. Implement automated price accuracy testing")

 print("\n SUCCESS CRITERIA:")
 print(" All symbols return non-zero prices")
 print(" Prices within expected ranges (±10% tolerance)")
 print(" Consistent prices across different endpoints")
 print(" TradeStation authenticated and working")
 ...
 print(
 f" {symbol}: ${current_price:.2f} (Expected: ${expected['min']}-${expected['max']}) {status}"
 )

 print("\n ACCURACY SUMMARY:")
 print(
 f" Accurate Prices: {accurate_count}/{len(self.expected_prices)} ({accurate_count/len(self.expected_prices)*100:.1f}%)"
 )
 ...
 print("\n🔧 NEXT STEPS:")
 if not self.tradestation_authenticated:
 print(" 1. URGENT: Authenticate TradeStation API for accurate pricing")
 if accurate_count < len(self.expected_prices):
 print(
 f" 2. Fix pricing issues for {len(self.expected_prices) - accurate_count} symbols"
 )
 print(" 3. Implement price validation and monitoring")
 print(" 4. Re-test after fixes are applied")
...
def main():
 ...
 print(
 f" Price Accuracy: {summary['accurate_prices']}/{summary['total_symbols']} symbols"
 )

 if (
 summary["critical_issues"] > 0
 or summary["accurate_prices"] < summary["total_symbols"]
 ):
 print(" 🚨 ACTION REQUIRED: Critical pricing issues detected")
 return False
 else:
 print(" All pricing appears to be working correctly")
 return True
