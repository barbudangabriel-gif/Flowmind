...
class ComprehensiveAPITester:
            ...
            if success:
                ...
                try:
                    ...
                    self.test_results.append(
                        {"name": name, "status": "PASSED", "data": response_data}
                    )
                    return True, response_data
                except:
                    self.test_results.append(
                        {"name": name, "status": "PASSED", "data": {}}
                    )
                    return True, {}
            else:
                print(
                ...
                try:
                    ...
                    self.test_results.append(
                        {"name": name, "status": "FAILED", "error": error_data}
                    )
                except:
                    print(f"   Error: {response.text}")
                    self.test_results.append(
                        {"name": name, "status": "FAILED", "error": response.text}
                    )
                return False, {}
    ...
    def test_portfolio_management_crud(self):
        ...
        success, portfolio_data = self.run_test(
            "GET /api/portfolio", "GET", "portfolio", 200
        )
        if success:
        ...
        success, aapl_response = self.run_test(
            "POST /api/portfolio (AAPL)", "POST", "portfolio", 200, data=aapl_position
        )
        aapl_id = None
        if success:
            ...
            print(
                f"   ✅ P&L: ${aapl_response.get('profit_loss', 0):.2f} ({aapl_response.get('profit_loss_percent', 0):.2f}%)"
            )
        ...
        success, msft_response = self.run_test(
            "POST /api/portfolio (MSFT)", "POST", "portfolio", 200, data=msft_position
        )
        msft_id = None
        if success:
            ...
            print(
                f"   ✅ P&L: ${msft_response.get('profit_loss', 0):.2f} ({msft_response.get('profit_loss_percent', 0):.2f}%)"
            )

        # Verify portfolio summary with P&L calculations
        success, updated_portfolio = self.run_test(
            "GET /api/portfolio (With Positions)", "GET", "portfolio", 200
        )
        if success:
            ...
            print(
                f"   ✅ Total P&L: ${updated_portfolio.get('total_profit_loss', 0):.2f} ({updated_portfolio.get('total_profit_loss_percent', 0):.2f}%)"
            )

        # Test DELETE /api/portfolio/{id} - remove positions
        if aapl_id:
            self.run_test(
                f"DELETE /api/portfolio/{aapl_id}",
                "DELETE",
                f"portfolio/{aapl_id}",
                200,
            )
        ...
        if msft_id:
            self.run_test(
                f"DELETE /api/portfolio/{msft_id}",
                "DELETE",
                f"portfolio/{msft_id}",
                200,
            )
    ...
    def test_market_data_apis(self):
        ...
        success, overview_data = self.run_test(
            "GET /api/market/overview", "GET", "market/overview", 200
        )
        ...
        success, movers_data = self.run_test(
            "GET /api/market/top-movers", "GET", "market/top-movers", 200
        )
        if success:
    ...
    def test_investment_scoring_system(self):
        ...
        success, aapl_score = self.run_test(
            "GET /api/investments/score/AAPL", "GET", "investments/score/AAPL", 200
        )
        if success:
            ...
            print(
                f"   ✅ Investment Horizon: {aapl_score.get('investment_horizon', 'N/A')}"
            )

        # Test GET /api/investments/score/{symbol} - individual stock scoring (MSFT)
        success, msft_score = self.run_test(
            "GET /api/investments/score/MSFT", "GET", "investments/score/MSFT", 200
        )
        if success:
        ...
        success, sector_leaders = self.run_test(
            "GET /api/investments/sector-leaders",
            "GET",
    ...
    def test_enhanced_stock_data(self):
        ...
        success, aapl_enhanced = self.run_test(
            "GET /api/stocks/AAPL/enhanced", "GET", "stocks/AAPL/enhanced", 200
        )
        if success:
        ...
        success, msft_enhanced = self.run_test(
            "GET /api/stocks/MSFT/enhanced", "GET", "stocks/MSFT/enhanced", 200
        )
        if success:
    ...
    def generate_summary(self):
        ...
        passed_tests = [
            test for test in self.test_results if test["status"] == "PASSED"
        ]
        failed_tests = [
            test
