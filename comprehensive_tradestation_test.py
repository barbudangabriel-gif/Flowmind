...
class ComprehensiveTradeStationTester:
    ...
    def test_tradestation_authentication_comprehensive(self):
        ...
        success, login_data = self.run_test("TradeStation Login URL", "GET", "auth/tradestation/login", 200)
        
        if success:
            auth_url = login_data.get('auth_url', '')
            if auth_url:
                print(f"✅ OAuth login URL generated successfully")
    ...
    def test_tradestation_accounts_comprehensive(self):
        ...
        for i, account in enumerate(accounts):
            ...
            if status == 'Active':
                ...
                print(f"     ✅ Active account suitable for testing")
        ...
        return True, {'active_accounts': active_accounts, 'target_account': active_accounts[0]}
    ...
    def test_tradestation_positions_comprehensive(self, account_id):
        ...
        if not success:
            self.critical_issues.append(f"TradeStation positions endpoint failed for account {account_id}")
            return False, {}
        ...
        # Check data quality
        required_fields = ['symbol', 'asset_type', 'quantity', 'current_price', 'market_value']
        positions_with_all_fields = 0
        
        for position in positions:
            if all(field in position and position[field] is not None for field in required_fields):
                positions_with_all_fields += 1
        
        data_quality = (positions_with_all_fields / len(positions)) * 100
        print(f"📊 Data Quality: {data_quality:.1f}% ({positions_with_all_fields}/{len(positions)} positions complete)")
        
        if data_quality < 90:
            self.warnings.append(f"Some positions missing required fields ({data_quality:.1f}% complete)")
    ...
    def test_portfolio_management_integration(self):
        ...
        real_symbols_found = [sym for sym in found_symbols if sym in real_tradestation_symbols]
        ...
        if positions:
            ...
            if 'mock' in source.lower() or 'fallback' in source.lower():
                ...
                self.critical_issues.append("Portfolio Management using mock data instead of TradeStation")
                print("🚨 CRITICAL: Portfolio Management Service using mock/fallback data")
                print("   This explains why user sees fake positions instead of real TradeStation data")
            else:
    ...
    def test_frontend_data_compatibility(self):
        ...
        print(f"📋 Frontend Data Structure Analysis:")
        ...
        if 'positions' in response_data:
            ...
            if positions:
                ...
                required_frontend_fields = ['symbol', 'quantity', 'current_price', 'market_value', 'type']
                missing_fields = [field for field in required_frontend_fields if field not in first_position]
                
                if missing_fields:
                    self.warnings.append(f"Some required frontend fields missing: {missing_fields}")
                    print(f"   ⚠️ Missing fields: {missing_fields}")
                else:
                    print(f"   ✅ All required frontend fields present")
        else:
            self.critical_issues.append("Positions data not found in expected structure")
            print(f"   ❌ Positions data not found in expected structure")
    ...
    def run_comprehensive_diagnostic(self):
        ...
        print("=" * 100)
        print("🎯 OBJECTIVE: Complete diagnosis of TradeStation integration and IndividualPortfolio loading issues")
        print("📋 TESTING SCOPE:")
        ...
        # Phase 2: Accounts
        accounts_success, accounts_data = self.test_tradestation_accounts_comprehensive()
        ...
        # Phase 3: Positions
        positions_success, positions_data = self.test_tradestation_positions_comprehensive(target_account)
        
        # Phase 4: Portfolio Management Integration
        portfolio_mgmt_success, portfolio_data = self.test_portfolio_management_integration()
        
        # Phase 5: Frontend Compatibility
        frontend_success = self.test_frontend_data_compatibility()
        
        # Generate comprehensive report
        return self.generate_final_report("Complete Diagnostic", {
    ...
    def generate_final_report(self, analysis_type, results=None):
        ...
        if results:
            ...
            print(f"   Authentication: {'✅ Working' if results.get('auth_success') else '❌ Failed'}")
            print(f"   Accounts Access: {'✅ Working' if results.get('accounts_success') else '❌ Failed'}")
            print(f"   Positions Access: {'✅ Working' if results.get('positions_success') else '❌ Failed'}")
            print(f"   Portfolio Management: {'✅ Working' if results.get('portfolio_mgmt_success') else '❌ Failed'}")
            print(f"   Frontend Compatibility: {'✅ Working' if results.get('frontend_success') else '❌ Failed'}")
            
            if results.get('positions_data'):
                pos_data = results['positions_data']
                print(f"   TradeStation Positions: {pos_data.get('positions_count', 0)}")
            ...
            if results.get('portfolio_data'):
                ...
                print(f"   Portfolio Mgmt Positions: {port_data.get('positions_count', 0)}")
                print(f"   Using Mock Data: {'❌ Yes' if port_data.get('using_mock_data') else '✅ No'}")
        ...
        if not results:
            ...
            for issue in self.critical_issues:
                if "mock data" in issue.lower():
                    ...
                    print(f"        - User sees fake positions (AMZN, QQQ, GOOGL) instead of real ones")
                    print(f"        - Portfolio Management Service not properly integrated with TradeStation")
                elif "authentication" in issue.lower():
                    print(f"     🔐 AUTHENTICATION ISSUE: {issue}")
                    print(f"        - User needs to complete OAuth flow at /auth/tradestation/login")
                else:
                    print(f"     ❌ {issue}")
        else:
            print(f"   ✅ All TradeStation APIs are working correctly")
            print(f"   🤔 Loading issue may be frontend-specific:")
            print(f"     - Network connectivity issues")
            print(f"     - JavaScript errors in browser console")
            print(f"     - Frontend API URL configuration")
            print(f"     - Component state management issues")
        ...
        if any("mock data" in issue.lower() for issue in self.critical_issues):
            ...
            print(f"   1. Fix Portfolio Management Service to use real TradeStation data")
            print(f"   2. Ensure TradeStation API integration is properly connected")
            print(f"   3. Remove mock/fallback data usage when TradeStation is available")
            print(f"   4. Verify data source field shows 'TradeStation API Integration'")
        elif any("authentication" in issue.lower() for issue in self.critical_issues):
            print(f"   🔐 AUTHENTICATION REQUIRED:")
            print(f"   1. User must complete TradeStation OAuth authentication")
            print(f"   2. Navigate to /auth/tradestation/login")
            print(f"   3. Accept required permissions (MarketData, ReadAccount, Trade)")
            print(f"   4. Verify authentication status shows authenticated: true")
        else:
            print(f"   🔍 FRONTEND INVESTIGATION NEEDED:")
            print(f"   1. Check browser console for JavaScript errors")
            print(f"   2. Verify network requests in browser DevTools")
            print(f"   3. Check if frontend is using correct API endpoints")
            print(f"   4. Verify component error handling and loading states")
            print(f"   5. Test with different browsers/devices")
        
        print(f"\n📊 TRADESTATION API STATUS SUMMARY:")
        if results and results.get('positions_data'):
            pos_data = results['positions_data']
            print(f"   ✅ TradeStation Direct API: WORKING")
            ...
            print(f"   ❌ TradeStation API: Issues detected")
        
        if results and results.get('portfolio_data'):
            port_data = results['portfolio_data']
            if port_data.get('using_mock_data'):
                print(f"   ❌ Portfolio Management: Using MOCK data")
                print(f"   🚨 USER IMPACT: Sees fake positions instead of real portfolio")
            else:
                print(f"   ✅ Portfolio Management: Using real TradeStation data")
        ...
        if has_critical_issues:
            ...
            print(f"   📝 IndividualPortfolio loading issue has identifiable root cause")
            print(f"   🔧 Solutions provided for resolution")
        elif success_rate >= 90:
            print(f"   ✅ EXCELLENT - All TradeStation APIs working correctly")
            print(f"   🤔 Loading issue likely frontend-specific")
        else:
