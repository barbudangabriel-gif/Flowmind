...
def test_detailed_unusual_whales():
    ...
    response = requests.get(
        f"{base_url}/unusual-whales/options/flow-alerts", timeout=30
    )
    response_time = time.time() - start_time

    print("1.1 Default Options Flow:")
    ...
    if response.status_code == 200:
        ...
        print(
            f"    📈 Bullish: {summary.get('bullish_count', 0)} | 📉 Bearish: {summary.get('bearish_count', 0)}"
        )
        ...
        if alerts:
            ...
            print(
                f"    🔍 Sample Alert: {first_alert.get('symbol')} {first_alert.get('strike_type')} - ${first_alert.get('premium', 0):,.0f}"
            )
            print(
                f"    📊 Sentiment: {first_alert.get('sentiment')} | Volume/OI: {first_alert.get('volume_oi_ratio', 0):.2f}"
            )
    ...
    start_time = time.time()
    response = requests.get(
        f"{base_url}/unusual-whales/options/flow-alerts", params=params, timeout=30
    )
    response_time = time.time() - start_time
    ...
    if response.status_code == 200:
        ...
        if "signals" in analysis:
            ...
            for signal in signals[:2]:
                print(
                    f"      - {signal.get('type', 'unknown')}: {signal.get('description', 'N/A')}"
                )
    ...
    print("2.1 Recent Dark Pool Activity:")
    print(f"    Status: {response.status_code} | Response Time: {response_time:.2f}s")
    ...
    if response.status_code == 200:
        ...
        if trades:
            ...
            print(
                f"    🔍 Sample Trade: {first_trade.get('ticker')} - {first_trade.get('dark_volume', 0):,} vol ({first_trade.get('dark_percentage', 0):.1f}% dark)"
            )
    ...
    start_time = time.time()
    response = requests.get(
        f"{base_url}/unusual-whales/dark-pool/recent", params=params, timeout=30
    )
    response_time = time.time() - start_time
    ...
    if response.status_code == 200:
        ...
        if "implications" in analysis:
            ...
            for implication in implications[:2]:
                print(
                    f"      - {implication.get('type', 'unknown')}: {implication.get('description', 'N/A')}"
                )
    ...
    start_time = time.time()
    response = requests.get(
        f"{base_url}/unusual-whales/congressional/trades", timeout=30
    )
    response_time = time.time() - start_time

    print("3.1 All Congressional Trades:")
    print(f"    Status: {response.status_code} | Response Time: {response_time:.2f}s")
    ...
    if response.status_code == 200:
        ...
        if "insights" in analysis:
            ...
            for insight in insights:
                print(
                    f"      - {insight.get('type', 'unknown')}: {insight.get('description', 'N/A')}"
                )
    ...
    start_time = time.time()
    response = requests.get(
        f"{base_url}/unusual-whales/congressional/trades", params=params, timeout=30
    )
    response_time = time.time() - start_time
    ...
    if response.status_code == 200:
        ...
        if trades:
            first_trade = trades[0]
            print(
                f"    🔍 Sample: {first_trade.get('representative')} - {first_trade.get('transaction_type')} {first_trade.get('ticker')} ${first_trade.get('transaction_amount', 0):,.0f}"
            )
    ...
    print("4.1 AI-Powered Trading Strategies:")
    print(f"    Status: {response.status_code} | Response Time: {response_time:.2f}s")
    ...
    if response.status_code == 200:
        ...
        print(
            f"    📈 Market Sentiment: {market_context.get('overall_sentiment', 'unknown')}"
        )
        ...
        if strategies:
            ...
            if tradestation:
                ...
                print(
                    f"        * Instrument: {tradestation.get('instrument_type', 'N/A')}"
                )
    ...
    start_time = time.time()
    response = requests.get(
        f"{base_url}/unusual-whales/analysis/comprehensive", timeout=30
    )
    response_time = time.time() - start_time

    print("5.1 Cross-Signal Analysis:")
    print(f"    Status: {response.status_code} | Response Time: {response_time:.2f}s")
    ...
    if response.status_code == 200:
        ...
        print(
            f"      - Congressional Trades: {data_summary.get('congressional_trades', 0)}"
        )

        print("    🔮 Market Outlook:")
        print(
            f"      - Overall Sentiment: {market_outlook.get('overall_sentiment', 'unknown')}"
        )
