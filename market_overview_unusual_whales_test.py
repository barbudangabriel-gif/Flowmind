...
class MarketOverviewUnusualWhalesTest:
    ...
    def test_market_overview_basic_functionality(self) -> bool:
        ...
        if not success:
            self.log_test(
                "Market Overview Basic Request",
                False,
                f"API request failed: {data.get('error', 'Unknown error')}",
                response_time,
            )
            return False
        ...
        self.log_test(
            "Market Overview Basic Request",
            True,
            f"Successfully retrieved market overview with {len(data.get('indices', []))} indices",
            response_time,
        )
        return True
    ...
    def test_unusual_whales_etf_data_integration(self) -> Tuple[bool, Dict]:
        ...
        if not success:
            self.log_test(
                "Unusual Whales ETF Data Request",
                False,
                f"API request failed: {data.get('error', 'Unknown error')}",
                response_time,
            )
        ...
        etf_symbols_found = [
            symbol for symbol in underlying_symbols if symbol in self.expected_etfs
        ]

        etf_integration_success = (
            len(etf_symbols_found) >= 3
        )  # At least 3 out of 4 ETFs

        self.log_test(
            "ETF Underlying Symbols",
            etf_integration_success,
            f"Found {len(etf_symbols_found)}/4 expected ETF symbols: {etf_symbols_found}",
            response_time,
        )
        ...
        futures_symbols_found = [
            ...
            if symbol in self.expected_futures_symbols
        ]

        futures_display_success = len(futures_symbols_found) >= 3

        self.log_test(
            "Futures Display Symbols",
            futures_display_success,
    ...
    def test_unusual_whales_data_source_verification(self, market_data: Dict) -> bool:
        ...
        self.log_test(
            "Unusual Whales Data Source", uw_data_source, f"Data source: {data_source}"
        )
        ...
        note_relevant = any(
            ...
            for keyword in ["etf", "unusual whales", "live data", "api"]
        )

        self.log_test(
            "Relevant Note Information", note_relevant, f"Note: {note[:100]}..."
        )
    ...
    def test_enhanced_fields_population(self, market_data: Dict) -> bool:
        ...
        for index in indices:
            ...
            print(
                f"    {symbol}: unusual_activity={unusual_activity}, options_flow_signal={options_flow_signal}"
            )
        ...
        unusual_activity_success = enhanced_fields_results[
            "indices_with_unusual_activity"
        ] >= (len(indices) * 0.75)
        options_flow_success = enhanced_fields_results[
            "indices_with_options_flow_signal"
        ] >= (len(indices) * 0.75)

        self.log_test(
            "Unusual Activity Fields",
            unusual_activity_success,
            f"{enhanced_fields_results['indices_with_unusual_activity']}/{len(indices)} indices have unusual_activity field",
        )

        self.log_test(
            "Options Flow Signal Fields",
            options_flow_success,
    ...
    def test_real_price_validation(self, market_data: Dict) -> bool:
        ...
        for index in indices:
            ...
            print(
                f"    {symbol} (via {underlying_symbol}): ${price:.2f} ({change:+.2f}, {change_percent:+.2f}%)"
            )
            ...
            if underlying_symbol in self.etf_price_ranges:
                ...
                if min_price <= price <= max_price:
                    ...
                    print(
                        f"      ✅ Price realistic for {underlying_symbol}: ${price:.2f} (range: ${min_price}-${max_price})"
                    )
                else:
                    price_validation_results["out_of_range_prices"] += 1
                    print(
                        f"      ⚠️  Price outside expected range for {underlying_symbol}: ${price:.2f} (expected: ${min_price}-${max_price})"
                    )
            else:
        ...
        realistic_price_success = price_validation_results["realistic_prices"] >= (
            len(indices) * 0.75
        )
    ...
    def test_performance_metrics(self) -> bool:
        ...
        for i in range(3):
            ...
            print(
                f"    Request {i+1}: {'✅' if success else '❌'} {response_time:.3f}s"
            )
    ...
    def test_fallback_behavior_analysis(self, market_data: Dict) -> bool:
        ...
        print("    Individual Index Sources:")
        for source, count in source_counts.items():
            print(f"      - {source}: {count} indices")

        # Test fallback behavior
        has_unusual_whales = any(
            "Unusual Whales" in source for source in individual_sources
        )
        has_fallback = any(
            "Yahoo Finance" in source or "Mock Data" in source
        ...
        display_symbols = [index.get("symbol", "") for index in indices]
        futures_maintained = all(
            symbol in self.expected_futures_symbols for symbol in display_symbols
        )

        self.log_test(
            "Fallback Data Source Analysis",
            fallback_success,
            f"Behavior: {fallback_behavior}",
        )

        self.log_test(
            "Futures Symbols Maintained in Fallback",
            futures_maintained,
    ...
    def run_comprehensive_test(self) -> Dict:
        ...
        print("=" * 80)
        print(
            "Testing optimized Market Overview endpoint with live ETF data from Unusual Whales API"
        )
        ...
        if test_1_success:
            test_2_success, market_data = (
                self.test_unusual_whales_etf_data_integration()
            )

            if test_2_success and market_data:
                test_3_success = self.test_unusual_whales_data_source_verification(
                    market_data
                )
                ...
                test_3_success = test_4_success = test_5_success = test_6_success = (
                    test_7_success
                ) = False
        else:
            test_2_success = test_3_success = test_4_success = test_5_success = (
                test_6_success
            ) = test_7_success = False
            market_data = {}
        ...
        if success_rate >= 90:
            ...
            verdict_details = (
                "All requirements met. Live ETF data integration successful."
            )
            ...
            verdict = (
                "⚠️  PARTIAL - Market Overview working but significant issues detected"
            )
            verdict_details = (
                "Basic functionality works but ETF integration has problems."
            )
        else:
            verdict = "❌ CRITICAL - Market Overview has major issues"
            verdict_details = (
                "Significant problems detected. Requires immediate attention."
            )
        ...
        if any(
            "ETF Underlying Symbols" in test["test_name"] and test["passed"]
            for test in self.test_results["test_details"]
        ):
            requirements_met.append(
                "✅ Live Unusual Whales Data: ETF data (SPY, QQQ, DIA, IWM) successfully fetched"
            )
        else:
            requirements_met.append(
                "❌ Live Unusual Whales Data: ETF data integration failed"
            )

        if any(
            "Unusual Whales Data Source" in test["test_name"] and test["passed"]
            for test in self.test_results["test_details"]
        ):
            requirements_met.append(
                "✅ Data Source Verification: Shows 'Unusual Whales API (Live ETF Data)'"
            )
        else:
            requirements_met.append(
                "❌ Data Source Verification: Does not show Unusual Whales as source"
            )

        if any(
            "Coverage Analysis Field" in test["test_name"] and test["passed"]
            for test in self.test_results["test_details"]
        ):
            requirements_met.append(
                "✅ Coverage Analysis: 'unusual_whales_coverage' field present"
            )
        else:
            requirements_met.append("❌ Coverage Analysis: Coverage field missing")

        if any(
            "Enhanced Fields" in test["test_name"] and test["passed"]
            for test in self.test_results["test_details"]
        ):
            requirements_met.append(
                "✅ Enhanced Fields: unusual_activity and options_flow_signal populated"
            )
        else:
            requirements_met.append(
                "❌ Enhanced Fields: Enhanced fields not properly populated"
            )

        if any(
            "Fallback" in test["test_name"] and test["passed"]
            for test in self.test_results["test_details"]
        ):
            requirements_met.append(
                "✅ Fallback Behavior: Proper fallback with clear source indication"
            )
        else:
            requirements_met.append(
                "❌ Fallback Behavior: Fallback behavior issues detected"
            )

        if any(
            "Performance" in test["test_name"] and test["passed"]
            for test in self.test_results["test_details"]
        ):
            requirements_met.append(
                "✅ Performance: Response times acceptable with optimized fetching"
            )
        else:
            requirements_met.append("❌ Performance: Response time issues detected")

        if any(
            "Price Validation" in test["test_name"] and test["passed"]
            for test in self.test_results["test_details"]
        ):
            requirements_met.append(
                "✅ Real Price Validation: Current and realistic ETF prices confirmed"
            )
        else:
