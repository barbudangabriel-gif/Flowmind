// Enhanced Mindfolio Component - Hybrid UI + Real TradeStation Data
const EnhancedMindfolio = () => {
  ...
  useEffect(() => {
    if (selectedAccount) {
      loadMindfolioData(selectedAccount);
    }
  }, [selectedAccount]);
  ...
  const loadMindfolioData = async (accountId) => {
    if (!accountId) return;
    ...
    try {
      ...
      if (data.data) {
        setMindfolioData(data.data);
        setError(null);
      } else {
        throw new Error('Invalid data structure received');
      }
    } catch (err) {
      setError(`Failed to load mindfolio data: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const generateChartData = () => {
    if (!mindfolioData?.positions) return [];
    
    const grouped = groupPositionsByTicker(filterPositionsByAsset(mindfolioData.positions));
  ...
  const renderPositionsTab = () => {
    if (!mindfolioData?.positions) return <div>No positions data</div>;
  ...
  const renderAllocationTab = () => (
    <div className="space-y-6">
      <div className="bg-white/80 backdrop-blur-sm p-6 rounded-xl shadow-lg">
        <h3 className="text-lg font-semibold mb-4">Mindfolio Allocation</h3>
        {chartData.length > 0 ? (
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-6 rounded-xl text-[rgb(252, 251, 255)]">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">Enhanced Mindfolio</h1>
          ...
          <div className="flex items-center space-x-4">
            ...
            {error && (
              <button
                onClick={() => selectedAccount && loadMindfolioData(selectedAccount)}
      ...
      {/* Mindfolio Metrics Cards */}
      {mindfolioData?.mindfolio_metrics && (
          ...
          <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 p-6 rounded-xl text-[rgb(252, 251, 255)] shadow-lg">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{formatCurrency(mindfolioData.mindfolio_metrics.total_market_value)}</p>
              </div>
          ...
          <div className="bg-gradient-to-br from-blue-500 to-blue-600 p-6 rounded-xl text-[rgb(252, 251, 255)] shadow-lg">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{mindfolioData.mindfolio_metrics.position_count || mindfolioData.positions?.length || 0}</p>
              </div>
              <Briefcase size={32} className="text-blue-200" />
            </div>
          </div>
          
          <div className={`bg-gradient-to-br ${mindfolioData.mindfolio_metrics.total_unrealized_pnl >= 0 ? 'from-green-500 to-green-600' : 'from-red-500 to-red-600'} p-6 rounded-xl text-[rgb(252, 251, 255)] shadow-lg`}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[rgb(252, 251, 255)]/80 text-sm">P&L</p>
                <p className="text-2xl font-bold">
                  {mindfolioData.mindfolio_metrics.total_unrealized_pnl > 0 ? '+' : ''}
                  {formatCurrency(mindfolioData.mindfolio_metrics.total_unrealized_pnl)}
                </p>
              </div>
          ...
          <div className="bg-gradient-to-br from-purple-500 to-purple-600 p-6 rounded-xl text-[rgb(252, 251, 255)] shadow-lg">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {mindfolioData.mindfolio_metrics.total_daily_pnl > 0 ? '+' : ''}
                  {formatCurrency(mindfolioData.mindfolio_metrics.total_daily_pnl)}
                </p>
              </div>
