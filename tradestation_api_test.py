...
class TradeStationAPITester:
 ...
 f"{name}: Status {response.status_code} - {error_data}"
 )
 except:
 print(f" Error: {response.text}")
 self.issues_found.append(
 f"{name}: Status {response.status_code} - {response.text}"
 )
 return False, {}
 ...
 print(
 " This could explain why IndividualPortfolio is stuck in loading state"
 )
 self.issues_found.append(
 "TradeStation not authenticated - portfolio data unavailable"
 )
 auth_working = False
 ...
 print(
 " REQUIREMENT: GET /api/tradestation/accounts should return account data"
 )
 ...
 print(" Will use this account for position testing")

 if target_account_id:
 print(
 f" TradeStation accounts accessible - Target Account: {target_account_id}"
 )
 ...
 print(
 f" Response Keys: {list(positions_data.keys()) if isinstance(positions_data, dict) else 'Not a dict'}"
 )
 ...
 print(
 f" Found positions in 'positions' field: {len(positions)} positions"
 )
 else:
 print(" CRITICAL: Cannot find positions data in response")
 ...
 print(
 f" WARNING: {missing_fields_count}/5 positions missing some required fields"
 )
 print(" This could cause frontend display issues")
 else:
 print(" All positions have required fields for frontend display")
 ...
 self.issues_found.append(
 "Data structure mismatch - frontend needs response.data access"
 )
 return False
 else:
 ...
 print("=" * 80)
 print(
 " OBJECTIVE: Test if Portfolio Management Service can access TradeStation data"
 )
 ...
 print(" WARNING: Portfolio Management using mock/fallback data")
 ...
 print("=" * 100)
 print(
 " OBJECTIVE: Diagnose why IndividualPortfolio component is stuck in loading state"
 )
 print(" FOCUS AREAS:")
 ...
 # Phase 3: Positions
 positions_working, positions_data = self.test_tradestation_positions(
 target_account_id
 )
 ...
 portfolio_mgmt_working, portfolio_data = (
 ...
 print("\n DETAILED RESULTS:")
 print(
 f" Authentication Working: {'' if results.get('auth_working') else ''}"
 )
 print(
 f" Accounts Accessible: {'' if results.get('accounts_working') else ''}"
 )
 print(
 f" Positions Accessible: {'' if results.get('positions_working') else ''}"
 )
 print(
 f" Data Structure Compatible: {'' if results.get('structure_compatible') else ''}"
 )
 print(
 f" Portfolio Management Working: {'' if results.get('portfolio_mgmt_working') else ''}"
 )
 ...
 print(" 🔐 Authentication Issue - User not logged into TradeStation")
 print(
 " SOLUTION: User needs to authenticate at /auth/tradestation/login"
 )
 elif not results.get("accounts_working"):
 print(" 🏛️ Accounts Access Issue - TradeStation accounts not accessible")
 print(" SOLUTION: Check TradeStation API integration and credentials")
 elif not results.get("positions_working"):
 print(" Positions Access Issue - Cannot fetch position data")
 print(" SOLUTION: Check TradeStation positions API endpoint")
 elif not results.get("structure_compatible"):
 print(" 🔧 Data Structure Issue - Frontend/Backend data format mismatch")
 print(
 " SOLUTION: Frontend needs to access response.data instead of response"
 )
 elif not results.get("portfolio_mgmt_working"):
 print(
 " 🔄 Integration Issue - Portfolio Management not using TradeStation data"
 )
 print(
 " SOLUTION: Fix Portfolio Management Service TradeStation integration"
 )
 else:
 print(" All TradeStation APIs working correctly")
 print(
 " 🤔 Loading issue may be frontend-specific (network, parsing, etc.)"
 )

 print("\n RECOMMENDATIONS FOR MAIN AGENT:")

 if not results or not results.get("auth_working"):
 print(" 1. 🔐 Ensure user completes TradeStation OAuth authentication")
 print(" 2. 🔗 Verify TradeStation API credentials are correct")
 print(" 3. Check TradeStation environment (LIVE vs PAPER)")
 elif not results.get("structure_compatible"):
 print(" 1. 🔧 Update frontend to access response.data for positions")
 print(" 2. Modify IndividualPortfolio component data parsing")
 print(" 3. 🧪 Test frontend with corrected data access pattern")
 elif results.get("positions_count", 0) > 0:
 print(" 1. TradeStation API is working correctly")
 print(" 2. Investigate frontend network requests and error handling")
 print(" 3. 🐛 Check browser console for JavaScript errors")
 print(" 4. Verify frontend API URL configuration")
 else:
 print(" 1. 🔄 Restart TradeStation services")
 print(" 2. Check TradeStation API rate limits")
 print(" 3. Contact TradeStation API support if issues persist")
