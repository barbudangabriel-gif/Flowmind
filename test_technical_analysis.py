...
def test_technical_analysis_expert_agent():
    ...
    print(
        "   2. ✅ Different Symbol Test - POST /api/agents/technical-analysis?symbol=AAPL"
    )
    print(
        "   3. ✅ Batch Technical Analysis - GET /api/agents/technical-analysis/batch?symbols=NVDA,AAPL,MSFT&include_smc=true"
    )
    print(
        "   4. ✅ Technical Analysis Methodology - GET /api/agents/technical-analysis/methodology"
    )
    ...
    if success:
        ...
        if "nvda_analysis" in test_results and test_results["nvda_analysis"]:
            ...
            if abs(nvda_score - aapl_score) >= 5:
                print(
                    f"     ✅ Different conditions detected (NVDA: {nvda_score}, AAPL: {aapl_score})"
                )
            else:
                print("     ⚠️  Similar scores")
    ...
    if success:
        ...
        print(
            f"     - Success Rate: {(successful_analyses/symbols_analyzed)*100:.1f}%"
    ...
    if success_rate >= 75:
        ...
        print("   All major endpoints functional with Smart Money Concepts.")
        print(
            "   Multi-timeframe analysis and professional recommendations operational."
        )
...
if __name__ == "__main__":
    print("🚀 Technical Analysis Expert Agent Testing")
