...
**Response:** Same as List (single portfolio object)
...
### 5. Add/Remove Funds
```http
POST /api/portfolios/{portfolio_id}/funds
```
...
```http
POST /api/portfolios/{portfolio_id}/allocate
```
...
### 8. Equity Curve (JSON)
```http
GET /api/portfolios/{portfolio_id}/analytics/equity
```
...
```http
GET /api/portfolios/{portfolio_id}/analytics/equity.csv
```
...
```http
GET /api/portfolios/{portfolio_id}/realized-pnl
```
...
```http
GET /api/portfolios/positions-ts
```
...
```http
GET /api/portfolios/ts/auth-url
```
...
```http
POST /api/portfolios/ts/callback?code={auth_code}&state={state}
```
...
```http
POST /api/portfolios/ts/subscribe
```
...
```http
POST /api/portfolios/ts/unsubscribe
```
...
### 21. Create Bucket
```http
POST /api/portfolios/{portfolio_id}/buckets
```
...
**Usage Flow:**
1. Create portfolio with `starting_balance`
...
**Backend:**
- `backend/portfolios.py` - Main router (1129 lines)
- `backend/portfolio_service.py` - Business logic
- `backend/portfolio_management_service.py` - Advanced features
- `backend/tradestation_client.py` - TS API integration

**Frontend:**
- `frontend/src/pages/Portfolio.jsx` - Main portfolio view
- `frontend/src/components/PortfolioGrid.jsx` - Positions grid
- `frontend/src/services/portfolioAPI.js` - API client

**Tests:**
- `tradestation_portfolio_test.py` - TS integration tests
- `backend/tests/test_portfolios.py` - Unit tests
