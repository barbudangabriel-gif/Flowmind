...
def test_different_leg_configurations():
 ...
 print(
 f" Response time {response_time:.3f}s exceeds 1s requirement"
 )
...
def test_options_overview_endpoint():
 ...
 print(" activeStrategies is 0 - may not be realistic")
 realistic_check = False
 if daily_volume <= 0:
 print(
 " dailyVolumeUsd is not positive - may not be realistic"
 )
 realistic_check = False
 ...
 print(
 " Symbol parameter filtering may not be working (same values for ALL vs TSLA)"
 )
...
def test_options_flow_summary_endpoint():
 ...
 print(
 f" Response time {response_time:.3f}s exceeds 5s requirement"
 )
 ...
 print(
 " Source shows error - graceful fallback working"
 )
 validation_results.append(True)
 else:
...
def test_options_analytics_integration():
 ...
 print("\n OPTIONS ANALYTICS INTEGRATION - ISSUES DETECTED")
