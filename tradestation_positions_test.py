...
class TradeStationPositionsTest:
    ...
    def make_request(self, method, endpoint, timeout=30):
        ...
        try:
            ...
            try:
                ...
                return response.status_code, response.text, f"JSON decode error: {str(e)}"
    ...
    def test_accounts_endpoint(self):
        ...
        status_code, response_data, error_msg = self.make_request('GET', 'tradestation/accounts')
        ...
        if status_code != 200:
            ...
            self.log_result("TradeStation Accounts", False, status_code, response_data, f"Non-200 status code")
            return False
        ...
        if isinstance(response_data, dict):
            ...
            if accounts and isinstance(accounts, list):
                ...
                if margin_accounts:
                    ...
                    self.log_result("TradeStation Accounts", True, status_code, {"accounts_count": len(accounts), "margin_account_id": self.margin_account_id})
                    return True
                else:
                    print(f"❌ NO MARGIN ACCOUNT FOUND")
                    print(f"   Available account types: {[acc.get('AccountType', 'Unknown') for acc in accounts]}")
                    self.log_result("TradeStation Accounts", False, status_code, response_data, "No margin account found")
                    return False
            else:
                print(f"❌ INVALID RESPONSE: No accounts array found")
                print(f"   Response keys: {list(response_data.keys()) if isinstance(response_data, dict) else 'Not a dict'}")
                self.log_result("TradeStation Accounts", False, status_code, response_data, "No accounts array in response")
                return False
        else:
            print(f"❌ INVALID RESPONSE: Expected dict, got {type(response_data)}")
            self.log_result("TradeStation Accounts", False, status_code, response_data, "Invalid response format")
            return False
    ...
    def test_positions_endpoint(self):
        ...
        if not self.margin_account_id:
            ...
            self.log_result("TradeStation Positions", False, None, None, "No margin account ID")
            return False
        
        print(f"📊 Testing with account ID: {self.margin_account_id}")
        
        status_code, response_data, error_msg = self.make_request('GET', f'tradestation/accounts/{self.margin_account_id}/positions')
        ...
        if status_code == 500:
            print(f"🚨 CRITICAL: 500 Internal Server Error persists!")
            print(f"   This indicates the session handling fix may not be working")
            print(f"   Response: {response_data}")
            self.log_result("TradeStation Positions", False, status_code, response_data, "500 Internal Server Error - session handling issue")
            return False
        
        if status_code != 200:
            print(f"❌ FAILED: Expected 200, got {status_code}")
            print(f"   Response: {response_data}")
            self.log_result("TradeStation Positions", False, status_code, response_data, f"Non-200 status code: {status_code}")
            return False
        
        print(f"✅ SUCCESS: Status 200 (no 500 error)")
        ...
        if isinstance(response_data, dict):
            ...
            if positions and isinstance(positions, list):
                ...
                if len(positions) > 0:
                    print(f"✅ At least one position found")
                    ...
                    key_fields = ['symbol', 'Symbol', 'quantity', 'Quantity', 'market_value', 'MarketValue', 
                                'unrealized_pnl', 'UnrealizedPnL', 'position_type', 'PositionType', 'asset_type', 'AssetType']
                    ...
                    self.log_result("TradeStation Positions", True, status_code, {
                    ...
                    print(f"⚠️  WARNING: No positions found in account")
                    self.log_result("TradeStation Positions", True, status_code, {"positions_count": 0}, "No positions in account")
                    return True
            else:
                print(f"❌ INVALID RESPONSE: No positions array found")
                print(f"   Response structure: {response_data}")
                self.log_result("TradeStation Positions", False, status_code, response_data, "No positions array in response")
                return False
        else:
            print(f"❌ INVALID RESPONSE: Expected dict, got {type(response_data)}")
            self.log_result("TradeStation Positions", False, status_code, response_data, "Invalid response format")
            return False
    ...
    def test_balances_endpoint(self):
        ...
        if not self.margin_account_id:
            print("❌ SKIPPED: No margin account ID available from previous test")
            self.log_result("TradeStation Balances", False, None, None, "No margin account ID")
            return False
        
        print(f"📊 Testing with account ID: {self.margin_account_id}")
        
        status_code, response_data, error_msg = self.make_request('GET', f'tradestation/accounts/{self.margin_account_id}/balances')
        ...
        if status_code != 200:
            print(f"❌ FAILED: Expected 200, got {status_code}")
            print(f"   Response: {response_data}")
            self.log_result("TradeStation Balances", False, status_code, response_data, f"Non-200 status code: {status_code}")
            return False
        ...
        if isinstance(response_data, dict):
            ...
            balance_fields = ['CashBalance', 'BuyingPower', 'Equity', 'MarketValue', 'balances', 'Balances']
            found_fields = []
            ...
            if found_fields:
                ...
                self.log_result("TradeStation Balances", True, status_code, {"balance_fields": found_fields})
                return True
            else:
                print(f"⚠️  WARNING: No recognized balance fields found")
                print(f"   Available fields: {list(response_data.keys())}")
                self.log_result("TradeStation Balances", True, status_code, response_data, "No recognized balance fields")
                return True
        else:
            print(f"❌ INVALID RESPONSE: Expected dict, got {type(response_data)}")
            self.log_result("TradeStation Balances", False, status_code, response_data, "Invalid response format")
            return False
    ...
    def run_all_tests(self):
        ...
        print("=" * 80)
        print("📋 REVIEW REQUEST: Re-test backend TradeStation positions route stability after session handling fix")
        print("🎯 FOCUS: Verify no 500 errors, proper JSON responses, positions data structure")
    ...
    def generate_final_report(self):
        ...
        has_500_error = any(result.get('status_code') == 500 for result in self.test_results)
        if has_500_error:
            print(f"   🚨 500 INTERNAL SERVER ERROR DETECTED - Session handling fix may not be working")
        else:
            print(f"   ✅ No 500 errors detected - Session handling appears stable")
        
        # Check positions endpoint specifically
        positions_test = next((r for r in self.test_results if 'Positions' in r['test_name']), None)
        if positions_test:
            if positions_test['success']:
                print(f"   ✅ Positions endpoint working - Returns JSON with positions[]")
                if positions_test['response_data'] and 'positions_count' in positions_test['response_data']:
                    count = positions_test['response_data']['positions_count']
                    print(f"   📊 Found {count} positions in account")
            else:
                print(f"   ❌ Positions endpoint failed - {positions_test.get('error_msg', 'Unknown error')}")
        
        # Account identification
        if self.margin_account_id:
            print(f"   ✅ Margin account identified: {self.margin_account_id}")
        else:
            print(f"   ❌ No margin account identified")
        
        # Sample payload information
        positions_test = next((r for r in self.test_results if 'Positions' in r['test_name'] and r['success']), None)
        ...
        print(f"\n💡 RECOMMENDATIONS:")
        if success_rate >= 100:
            print(f"   🎉 EXCELLENT: All TradeStation endpoints working perfectly after session handling fix")
            print(f"   ✅ No further action needed - positions route is stable")
        elif success_rate >= 66:
            print(f"   ✅ GOOD: Most endpoints working, minor issues detected")
            print(f"   🔧 Review failed tests and address specific issues")
        else:
            print(f"   ❌ CRITICAL: Multiple endpoint failures detected")
            print(f"   🚨 Session handling fix may not be complete")
            print(f"   🔧 Immediate investigation required")
        
        # Session handling assessment
        if has_500_error:
            print(f"   🚨 SESSION HANDLING: 500 errors indicate session management issues persist")
            print(f"   🔧 Review authentication token refresh logic")
            print(f"   🔧 Check TradeStation API session timeout handling")
        else:
            print(f"   ✅ SESSION HANDLING: No 500 errors suggest session handling fix is working")
