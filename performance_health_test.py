...
class HealthEndpointTester:
    ...
    def _assess_performance(self, endpoint: str, results: Dict[str, Any]):
        ...
        if p95 < 100:
            ...
            checks.append(("⚠️", "P95 Latency", f"{p95}ms (Target: <100ms, Acceptable: <200ms)"))
        else:
            checks.append(("❌", "P95 Latency", f"{p95}ms (Target: <100ms)"))
        ...
        if p99 < 200:
            ...
            checks.append(("⚠️", "P99 Latency", f"{p99}ms (Target: <200ms, Acceptable: <500ms)"))
        else:
            checks.append(("❌", "P99 Latency", f"{p99}ms (Target: <200ms)"))
    ...
    def generate_summary(self):
        ...
        for endpoint, data in self.results.items():
            ...
            if conc["success_rate"] < 99:
                all_good = False
                issues.append(f"  ⚠️ {endpoint}: Success rate below 99%")
            if conc["response_time"]["p95"] > 200:
                all_good = False
                issues.append(f"  ⚠️ {endpoint}: P95 latency above 200ms")
