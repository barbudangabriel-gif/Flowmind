...
class InvestmentScoringPriceInvestigator:
    def __init__(self, base_url="https://trade-insights-27.preview.emergentagent.com"):
    ...
    def test_scanner_top_stocks_price_structure(self):
        ...
        success, data = self.run_test("Scanner Top Stocks", "GET", "scanner/top-stocks", 200, params={"limit": 10})
        ...
        for i, stock in enumerate(top_stocks[:10]):  # Check first 10
            ...
            if 'price' not in stock:
                ...
                print(f"     ❌ MISSING: No 'price' field")
                self.log_issue("MISSING_PRICE_FIELD", f"Stock {ticker} missing price field", stock)
            elif price == 'N/A' or price == "N/A":
                stocks_with_na_prices += 1
                print(f"     ❌ N/A: Price shows 'N/A'")
                self.log_issue("NA_PRICE", f"Stock {ticker} has N/A price", stock)
            elif price == 0 or price == 0.0:
                stocks_with_zero_prices += 1
                print(f"     ❌ ZERO: Price is zero")
                ...
                print(f"     ⚠️  UNKNOWN: Unexpected price format")
                self.log_issue("INVALID_PRICE_FORMAT", f"Stock {ticker} has invalid price format", {"price": price, "type": type(price)})
        ...
        for i, stock in enumerate(top_5_stocks):
            ...
            if 'price' in stock:
                ...
                if isinstance(price, (int, float)) and price > 0:
                    ...
                    self.log_issue("INVALID_TOP5_PRICE", f"Top 5 stock {ticker} has invalid price", stock)
            else:
                print(f"     ⚠️  No price field in top_5_stocks")
    ...
    def test_individual_stock_scoring_price(self):
        ...
        print("="*80)
        print("🎯 OBJECTIVE: Test calculate_investment_score for AAPL and check price data")
        
        # Test investment scoring for AAPL
        success, data = self.run_test("Investment Score AAPL", "GET", "investments/score/AAPL", 200)
        ...
        if not found_price_fields:
            print(f"   ⚠️  No direct price fields found in investment score response")
            print(f"   📝 Note: Price data might be fetched separately by the scoring system")
        
        # Check if we can get enhanced stock data for AAPL
        success2, stock_data = self.run_test("Enhanced Stock Data AAPL", "GET", "stocks/AAPL/enhanced", 200)
        
        if success2:
            aapl_price = stock_data.get('price', 'N/A')
            print(f"\n📊 AAPL Enhanced Stock Data:")
            print(f"   - Price from enhanced endpoint: ${aapl_price:.2f}" if isinstance(aapl_price, (int, float)) else f"   - Price: {aapl_price}")
            
            if isinstance(aapl_price, (int, float)) and aapl_price > 0:
                print(f"   ✅ Enhanced stock data has valid price")
                return True
            else:
                print(f"   ❌ Enhanced stock data has invalid price")
                self.log_issue("INVALID_ENHANCED_PRICE", "AAPL enhanced stock data has invalid price", stock_data)
                return False
        else:
            self.log_issue("API_FAILURE", "Enhanced stock data endpoint failed for AAPL")
            return False
    ...
    def test_mongodb_data_structure(self):
        ...
        print("="*80)
        print("🎯 OBJECTIVE: Analyze scanner data to understand what's saved in MongoDB")
        
        # Get scanner status to understand data structure
        success, status_data = self.run_test("Scanner Status for MongoDB Analysis", "GET", "scanner/status", 200)
        
        if not success:
            return False
        
        # Get top stocks to see full data structure
        success2, top_stocks_data = self.run_test("Top Stocks for MongoDB Analysis", "GET", "scanner/top-stocks", 200, params={"limit": 5})
        ...
        if top_stocks:
            ...
            if 'stock_data' in sample_stock and isinstance(sample_stock['stock_data'], dict):
            ...
            if not stock_data_price_fields:
                ...
                self.log_issue("NO_PRICE_IN_STOCK_DATA", "No price fields found in stock_data", sample_stock)
                ...
                for field in price_fields:
                    if field in sample_stock:
                        ...
                        print(f"   💰 PRICE FOUND at root level.{field}: {sample_stock[field]}")
                
                if not root_price_fields:
                    print(f"   ❌ NO PRICE FIELDS FOUND at root level either")
                    self.log_issue("NO_PRICE_ANYWHERE", "No price fields found anywhere in document", sample_stock)
                    return False
            
            return True
        else:
            print(f"   ❌ No sample stock data available for analysis")
            return False
    ...
    def test_new_scan_price_saving(self):
        ...
        # Start a new scan
        success, scan_data = self.run_test("Start New Scan", "POST", "scanner/start-scan", 200)
        ...
        success2, status_data = self.run_test("Scanner Status After New Scan", "GET", "scanner/status", 200)
        ...
        if total_scanned > 0:
            ...
            success3, fresh_data = self.run_test("Fresh Top Stocks After Scan", "GET", "scanner/top-stocks", 200, params={"limit": 3})
            ...
            if success3:
                ...
                for i, stock in enumerate(fresh_stocks[:3]):
                    ...
                    if 'stock_data' in stock:
                        ...
                        if isinstance(stock_data_price, (int, float)) and stock_data_price > 0:
                            print(f"     ✅ Fresh scan has valid price in stock_data")
                        else:
                            print(f"     ❌ Fresh scan has invalid price in stock_data")
                            self.log_issue("FRESH_SCAN_INVALID_PRICE", f"Fresh scan stock {ticker} has invalid price", stock)
                
                return True
            else:
                return False
        else:
            print(f"   ⚠️  No stocks processed yet, scan might still be running")
            return False
    ...
    def test_enhanced_ticker_manager_prices(self):
        ...
        for symbol in test_symbols:
            success, stock_data = self.run_test(f"Enhanced Stock Data {symbol}", "GET", f"stocks/{symbol}/enhanced", 200)
            ...
            if success:
                ...
                if isinstance(price, (int, float)) and price > 0:
                    print(f"   ✅ {symbol} has valid price from enhanced ticker manager")
                else:
                    print(f"   ❌ {symbol} has invalid price from enhanced ticker manager")
                    self.log_issue("ENHANCED_TICKER_INVALID_PRICE", f"{symbol} enhanced ticker data has invalid price", stock_data)
            else:
                self.log_issue("API_FAILURE", f"Enhanced stock data failed for {symbol}")
    ...
    def run_comprehensive_investigation(self):
        ...
        print("="*100)
        print("🎯 INVESTIGATING: Missing prices issue - all prices show 'N/A' in frontend table")
        print("📋 TESTING PLAN:")
        ...
        test_results.append(("Scanner Top Stocks Price Structure", self.test_scanner_top_stocks_price_structure()))
        test_results.append(("Scanner Status Top 5 Prices", self.test_scanner_status_top5_prices()))
        test_results.append(("Individual Stock Scoring Price", self.test_individual_stock_scoring_price()))
        test_results.append(("MongoDB Data Structure", self.test_mongodb_data_structure()))
        test_results.append(("New Scan Price Saving", self.test_new_scan_price_saving()))
        test_results.append(("Enhanced Ticker Manager Prices", self.test_enhanced_ticker_manager_prices()))
        ...
        if any("NA_PRICE" in issue['type'] for issue in self.issues_found):
            print(f"   🎯 PRIMARY ISSUE: Prices are being set to 'N/A' in the data pipeline")
            print(f"   🔧 LIKELY CAUSE: enhanced_ticker_manager.get_real_time_quote() returning invalid prices")
            print(f"   💡 SOLUTION: Check enhanced_ticker_data.py price fetching logic")
        
        elif any("MISSING_PRICE_FIELD" in issue['type'] for issue in self.issues_found):
            print(f"   🎯 PRIMARY ISSUE: Price field is missing from scanner results")
            print(f"   🔧 LIKELY CAUSE: investment_scoring.py not including price in response structure")
            print(f"   💡 SOLUTION: Modify scanner response to include price field")
        
        elif any("NO_PRICE_IN_STOCK_DATA" in issue['type'] for issue in self.issues_found):
            print(f"   🎯 PRIMARY ISSUE: Prices not being saved in stock_data during scanning")
            print(f"   🔧 LIKELY CAUSE: StockScanner.scan_all_stocks() not preserving price data")
            print(f"   💡 SOLUTION: Ensure stock_data includes price when saving to MongoDB")
        
        else:
            print(f"   🤔 UNCLEAR: Multiple potential issues detected")
            print(f"   🔧 RECOMMENDATION: Check the entire data pipeline from enhanced_ticker_manager to frontend")
        
        # Specific Recommendations
        print(f"\n💡 SPECIFIC RECOMMENDATIONS:")
        print(f"   1. Check enhanced_ticker_data.py get_real_time_quote() method")
        print(f"   2. Verify investment_scoring.py includes price in scanner response")
        print(f"   3. Ensure MongoDB documents include stock_data.price field")
        print(f"   4. Test the complete pipeline: enhanced_ticker_manager → investment_scorer → MongoDB → API response")
