...
const TradeStationMainMindfolio = () => {
 const navigate = useNavigate();
 const [mindfolioData, setMindfolioData] = useState(null);
 ...
 // Fetch real TradeStation data directly from TradeStation API (not Mindfolio Management Service)
 ...
 // Set top-level mindfolio summary
 setMindfolioData({
 account: mainAccount,
 totalMarketValue,
 totalUnrealizedPnL,
 ...
 const acctValue = (mindfolioData?.totalMarketValue || 0) + (cashBalance || 0);
 ...
 return groups;
 }, [positionsNormalized, mindfolioData?.totalMarketValue, cashBalance]);
 ...
 <span className="text-xl text-gray-300">Loading TradeStation Mindfolio...</span>
 ...
 if (error && !mindfolioData) {
 return (
 <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center">
 <div className="text-center max-w-md">
 <AlertCircle className="mx-auto h-16 w-16 text-red-500 mb-4" />
 <h2 className="text-xl font-medium text-[rgb(252, 251, 255)] mb-2">Error Loading Mindfolio</h2>
 <p className="text-gray-400 mb-6">{error}</p>
 <button onClick={fetchTradeStationData} className="bg-blue-600 text-[rgb(252, 251, 255)] px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors">Retry</button>
 </div>
 </div>
 );
 }

 const ChangeIcon = getChangeIcon(mindfolioData?.totalUnrealizedPnL);
 ...
 <div>
 <h1 className="text-3xl font-medium text-[rgb(252, 251, 255)] flex items-center">
 <Activity className="mr-3" size={32} />
 TradeStation Main Mindfolio
 </h1>
 <p className="text-blue-100 mt-2">Account: {mindfolioData?.account?.AccountID} • Live Trading Account</p>
 </div>
 <button onClick={fetchTradeStationData} className="bg-white/20 text-[rgb(252, 251, 255)] px-4 py-2 rounded-lg hover:bg-white/30 transition-colors flex items-center space-x-2" disabled={loading}>
 ...
 <Eye className="text-[rgb(252, 251, 255)]" size={20} />
 <span className="text-3xl font-medium text-[rgb(252, 251, 255)]">{formatCurrency(accountValue, 0)}</span>
 <span className="text-blue-100">(Market Value + Cash)</span>
 </div>
 <div className="mt-2 flex items-center gap-4 text-xl text-blue-100">
 <span>Market Value: <strong>{formatCurrency(mindfolioData?.totalMarketValue || 0, 0)}</strong></span>
 <span>Cash: <strong>{formatCurrency(cashBalance || 0, 0)}</strong></span>
 </div>
 <div className={`flex items-center mt-2 space-x-2 ${getChangeColor(mindfolioData?.totalUnrealizedPnL)}`}>
 <ChangeIcon size={24} />
 <span className="text-2xl font-medium">{mindfolioData?.totalUnrealizedPnL >= 0 ? '+' : ''}{formatCurrency(Math.abs(mindfolioData?.totalUnrealizedPnL || 0), 0)}</span>
 <span className="text-3xl">({mindfolioData?.percentChange >= 0 ? '+' : ''}{(mindfolioData?.percentChange || 0).toFixed(2)}%)</span>
 </div>
 <p className="text-blue-100 text-xl mt-1">Last updated: {lastUpdated.toLocaleString()}</p>
 </div>
 </div>
 </div>
 ...
 <p className="text-gray-400 text-xl">Account Value</p>
 <p className="text-3xl font-medium text-[rgb(252, 251, 255)]">{formatCurrency(accountValue, 0)}</p>
 </div>
 <div className="bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
 <p className="text-gray-400 text-xl">Cash</p>
 <p className="text-3xl font-medium text-[rgb(252, 251, 255)]">{formatCurrency(cashBalance || 0, 0)}</p>
 </div>
 <div className="bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
 <p className="text-gray-400 text-xl">Stocks</p>
 <p className="text-3xl font-medium text-[rgb(252, 251, 255)]">{formatCurrency(stocksValue, 0)}</p>
 </div>
 <div className="bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
 <p className="text-gray-400 text-xl">Options</p>
 <p className="text-3xl font-medium text-[rgb(252, 251, 255)]">{formatCurrency(optionsValue, 0)}</p>
 </div>
 </div>
 </div>
 ...
 <div className="text-xl text-gray-300">Positions: <span className="font-medium text-[rgb(252, 251, 255)]">{mindfolioData?.totalPositions || 0}</span></div>
 ...
 {positionsNormalized.length > 0 && (
 <table className="w-full text-xl bg-gray-800 rounded-lg">
 <thead>
 ...
 {summary.hasStock && (<span className="px-1 py-0.5 rounded text-lg bg-blue-600 text-[rgb(252, 251, 255)]">S</span>)}
 {summary.hasOptions && (<span className="px-1 py-0.5 rounded text-lg bg-purple-600 text-[rgb(252, 251, 255)]">O</span>)}
 </div>
 </div>
 </td>
 <td className="text-left py-4 px-2 text-gray-200">-</td>
 <td className="text-left py-4 px-2 text-gray-200">-</td>
 <td className={`text-right py-4 px-2 font-medium ${summary.totalPnL >= 0 ? 'text-green-400' : 'text-red-400'}`}>{summary.totalPnL >= 0 ? '+' : ''}{formatCurrency(Math.abs(summary.totalPnL), 0)}</td>
 <td className="text-right py-4 px-2 text-gray-400">-</td>
 <td className="text-right py-4 px-2 text-gray-400">-</td>
 <td className="text-right py-4 px-2 text-gray-400">-</td>
 <td className="text-right py-4 px-2 text-gray-400">-</td>
 <td className="text-right py-4 px-2 text-gray-400">-</td>
 <td className="text-right py-4 px-2 font-medium text-[rgb(252, 251, 255)]">{formatCurrency(summary.totalValue, 0)}</td>
 <td className="text-right py-4 px-2 font-medium text-gray-200">{summary.totalQuantity.toLocaleString()}</td>
 ...
 <p className={`text-3xl font-medium ${mindfolioData?.totalUnrealizedPnL >= 0 ? 'text-green-400' : 'text-red-400'}`}>{mindfolioData?.totalUnrealizedPnL >= 0 ? '+' : ''}{formatCurrency(Math.abs(mindfolioData?.totalUnrealizedPnL || 0), 0)}</p>
 </div>
 ...
 <div className="flex items-center justify-between">
 <div>
 <p className="text-gray-400 text-xl">Cash Balance</p>
 <p className="text-3xl font-medium text-[rgb(252, 251, 255)]">{formatCurrency(cashBalance || 0, 0)}</p>
 </div>
...
export default TradeStationMainMindfolio;