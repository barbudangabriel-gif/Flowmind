...
const LiveFlowFeed = ({ 
 ...
 return sentiment === 'bullish' ? 'text-green-400 font-medium' : 'text-red-400 font-medium';
 }
 ...
 return <span className="text-lg px-2 py-0.5 rounded bg-green-500/20 text-green-400 font-medium">🐂 BULL</span>;
 }
 return <span className="text-lg px-2 py-0.5 rounded bg-red-500/20 text-red-400 font-medium">🐻 BEAR</span>;
 };
 ...
 <h3 className="text-3xl font-medium text-[rgb(252, 251, 255)]">📡 Live Flow</h3>
 ...
 {/* Stats Bar */}
 <div className="flex items-center gap-6 px-4 py-2 bg-slate-800/50 border-b border-slate-700/30 text-lg">
 <div className="flex items-center gap-2">
 <span className="text-gray-400">Total:</span>
 <span className="text-[rgb(252, 251, 255)] font-medium">{stats.total}</span>
 </div>
 <div className="flex items-center gap-2">
 ...
 <span className="text-blue-400 font-medium">{formatPremium(stats.avgPremium)}</span>
 </div>
 </div>
 ...
 <div className="text-lg mt-1">Trades will appear here in real-time</div>
 </div>
 </div>
 ) : (
 ...
 >
 {/* Time */}
 <div className="text-lg text-gray-400 w-20 flex-shrink-0">
 ...
 className="text-xl font-medium text-blue-400 hover:text-blue-300 hover:underline w-16 text-left flex-shrink-0"
 >
 {item.symbol}
 </button>
 ...
 <span className="font-medium">${item.strike}</span>
 <span className="text-gray-500 mx-1">•</span>
 <span>{item.expiry}</span>
 </div>

 {/* Type */}
 <div className={`text-lg font-medium px-2 py-0.5 rounded flex-shrink-0 ${
 item.kind === 'CALL' 
 ...
 {/* Premium */}
 <div className={`text-xl font-medium ml-auto flex-shrink-0 ${getPremiumColor(item.sentiment, item.premium)}`}>
 {formatPremium(item.premium)}
 </div>

 {/* Sweep indicator */}
 {item.is_sweep && (
 <div className="text-lg px-2 py-0.5 rounded bg-yellow-500/20 text-yellow-400 font-medium flex-shrink-0">
 SWEEP
 </div>
 )}
 </div>
 ...
 <div className="text-[rgb(252, 251, 255)] font-medium mb-2">Feed Paused</div>
 <div className="text-xl text-gray-400 mb-4">
 {pausedMessagesRef.current.length} new trades queued
 </div>
 <button
 onClick={handleResume}
 className="px-4 py-2 bg-green-500 hover:bg-green-600 text-[rgb(252, 251, 255)] font-medium rounded transition-colors"
 >
