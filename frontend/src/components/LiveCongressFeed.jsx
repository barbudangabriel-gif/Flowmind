...
const LiveCongressFeed = ({ maxItems = 50, className = '' }) => {
  ...
  const getPartyBadge = (party) => {
    ...
    return (
      <span className={`text-xs px-2 py-0.5 rounded font-semibold ${colors[party] || colors['I']}`}>
        {party}
      </span>
    );
  };
    ...
    <div className={`flex flex-col h-full bg-slate-900 rounded-lg border border-slate-700 ${className}`}>
      ...
      <div className="flex items-center justify-between p-4 border-b border-slate-700/50">
        <div className="flex items-center gap-3">
          <h3 className="text-lg font-bold text-white">🏛️ Congress Trades</h3>
        ...
        {filteredTrades.length === 0 ? (
          <div className="flex items-center justify-center h-full text-gray-400">
            <div className="text-center">
              ...
              <div className="text-xs mt-1">Disclosures will appear here in real-time</div>
            </div>
          </div>
        ) : (
            ...
            >
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className="text-sm font-bold text-blue-400 hover:text-blue-300 hover:underline flex-shrink-0"
                >
                  {trade.symbol}
                </button>
                ...
                {(() => {
                  ...
                  if (lagDays > 45) {
                    return (
                      <div className="text-xs px-2 py-0.5 rounded bg-orange-500/20 text-orange-400 font-semibold flex-shrink-0">
                        ⚠️ LATE ({lagDays}d)
                      </div>
                    );
                  }
