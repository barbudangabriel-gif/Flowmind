...
const OptionTradesFeed = ({ defaultTicker = 'TSLA' }) => {
 ...
 <div>
 <label className="block text-lg text-gray-400 mb-1">Min Premium</label>
 <input
 type="number"
 value={filters.minPremium}
 onChange={(e) => setFilters(prev => ({ ...prev, minPremium: Number(e.target.value) }))}
 placeholder="0"
 className="w-full bg-gray-800 text-[rgb(252, 251, 255)] border border-gray-700 rounded px-3 py-1 text-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
 />
 </div>
 
 <div>
 <label className="block text-lg text-gray-400 mb-1">Type</label>
 <select
 value={filters.type}
 ...
 <div>
 <label className="block text-lg text-gray-400 mb-1">Side</label>
 <select
 value={filters.side}
 ...
 <div className="text-lg text-gray-400">Total Premium</div>
 <div className="text-3xl font-medium text-[rgb(252, 251, 255)]">{formatPremium(stats.totalPremium)}</div>
 </div>
 <div className="bg-gray-800 rounded p-3">
 <div className="text-lg text-gray-400">Calls</div>
 <div className="text-3xl font-medium text-green-400">{stats.callCount}</div>
 </div>
 <div className="bg-gray-800 rounded p-3">
 <div className="text-lg text-gray-400">Puts</div>
 <div className="text-3xl font-medium text-red-400">{stats.putCount}</div>
 </div>
 <div className="bg-gray-800 rounded p-3">
 <div className="text-lg text-gray-400">Buys</div>
 <div className="text-3xl font-medium text-green-400">{stats.buyCount}</div>
 </div>
 <div className="bg-gray-800 rounded p-3">
 <div className="text-lg text-gray-400">Sells</div>
 <div className="text-3xl font-medium text-red-400">{stats.sellCount}</div>
 </div>
 </div>
 )}
 ...
 <tr>
 <th className="px-3 py-2 text-left text-lg font-medium text-gray-300">Time</th>
 <th className="px-3 py-2 text-left text-lg font-medium text-gray-300">Strike</th>
 <th className="px-3 py-2 text-left text-lg font-medium text-gray-300">Expiry</th>
 <th className="px-3 py-2 text-center text-lg font-medium text-gray-300">Type</th>
 <th className="px-3 py-2 text-center text-lg font-medium text-gray-300">Side</th>
 <th className="px-3 py-2 text-right text-lg font-medium text-gray-300">Price</th>
 <th className="px-3 py-2 text-right text-lg font-medium text-gray-300">Qty</th>
 <th className="px-3 py-2 text-right text-lg font-medium text-gray-300">Premium</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-gray-700">
 {trades.map((trade) => (
 <tr key={trade.id} className="hover:bg-gray-750 transition-colors">
 <td className="px-3 py-2 text-gray-400 font-mono text-lg">
 {formatTime(trade.receivedAt)}
 </td>
 <td className="px-3 py-2 text-[rgb(252, 251, 255)] font-medium">
 ${trade.strike}
 </td>
 <td className="px-3 py-2 text-gray-300 text-lg">
 {trade.expiry}
 </td>
 <td className="px-3 py-2 text-center">
 <span className={`px-2 py-0.5 rounded text-lg font-medium ${
 trade.type === 'CALL' 
 ...
 <td className="px-3 py-2 text-right text-[rgb(252, 251, 255)] font-mono">
 ${trade.price?.toFixed(2)}
 </td>
 <td className="px-3 py-2 text-right text-gray-300">
 {trade.quantity}
 </td>
 <td className="px-3 py-2 text-right text-[rgb(252, 251, 255)] font-medium">
 ...
 {/* Footer */}
 <div className="mt-4 pt-3 border-t border-gray-800 flex items-center justify-between text-lg text-gray-500">
 <span>Showing last {trades.length} trades (max 50)</span>
 <button
 onClick={() => setTrades([])}
 className="text-gray-400 hover:text-[rgb(252, 251, 255)] transition-colors"
 >
 Clear All
 </button>
 </div>
