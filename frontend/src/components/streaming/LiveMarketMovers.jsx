 ...
 * 
 * ⚠️ EXPERIMENTAL CHANNEL: market_movers
 * 
  ...
  const renderMover = (mover, idx, type) => (
    ...
    >
      <div className="flex-1">
        <div className="font-semibold text-white">{mover.ticker || mover.symbol}</div>
      ...
      {error && (
        <div className="bg-yellow-500/10 border border-yellow-500/30 rounded p-3 mb-4">
          <p className="text-yellow-400 text-xs">
            ⚠️ WebSocket unavailable. Using REST API fallback (updates every 30s)
          </p>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-gray-800 rounded-lg p-4">
            <h3 className="text-sm font-semibold text-green-400 mb-3 flex items-center gap-2">
              📈 Top Gainers
              <span className="text-xs text-gray-500">({moversData.gainers?.length || 0})</span>
            </h3>
          ...
          <div className="bg-gray-800 rounded-lg p-4">
            <h3 className="text-sm font-semibold text-red-400 mb-3 flex items-center gap-2">
              📉 Top Losers
              <span className="text-xs text-gray-500">({moversData.losers?.length || 0})</span>
            </h3>
            <div className="space-y-1">
