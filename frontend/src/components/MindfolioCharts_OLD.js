...
const MindfolioCharts = () => {
 ...
 // Fetch mindfolio charts data
 const fetchMindfolioChartsData = async () => {
 setLoading(true);
 setError(null);
 try {
 const params = {
 ...
 const performanceResponse = await axios.get(`${API}/mindfolio/${mindfolioId}/performance`, { params });
 setPerformanceData(performanceResponse.data?.performance_data || []);

 // Fetch allocation data 
 const allocationResponse = await axios.get(`${API}/mindfolio/${mindfolioId}/allocation`, { params });
 setAllocationData(allocationResponse.data?.allocation_data || []);

 // Set mindfolio summary
 setMindfolioData({
 total_value: performanceResponse.data?.mindfolio_summary?.total_value || currentMindfolio.value,
 total_pnl: performanceResponse.data?.mindfolio_summary?.total_pnl || 0,
 cash_balance: performanceResponse.data?.mindfolio_summary?.cash_balance || currentMindfolio.cash
 });

 } catch (err) {
 console.error('Error fetching mindfolio charts data:', err);
 setError('Failed to load mindfolio charts data');
 ...
 const mindfolioCash = currentMindfolio.cash || 100000;
 const totalMindfolioValue = currentMindfolio.value;
 const calculatedMarginUsed = Math.max(0, totalMindfolioValue - mindfolioCash);
 const calculatedMarginAvailable = mindfolioCash > 0 ? 0 : 25000; // Only available if no cash
 ...
 if (mindfolioCash > 0) {
 allocationData.push({ name: 'Cash', value: mindfolioCash, type: 'cash', count: 1 });
 }
 ...
 setMindfolioData({
 total_value: currentMindfolio.value,
 total_pnl: 2500.75,
 cash_balance: mindfolioCash,
 margin_used: calculatedMarginUsed,
 margin_available: calculatedMarginAvailable,
 buying_power: mindfolioCash + (mindfolioCash > 0 ? 0 : 25000)
 });
 };

 useEffect(() => {
 fetchMindfolioChartsData();
 }, [mindfolioId, activeFilter, assetFilter, timeFilter, customDateRange]);
 ...
 <p className="text-gray-600">Loading mindfolio charts...</p>
 </div>
 </div>
 );
 }
 ...
 <button
 onClick={() => navigate(`/mindfolios/${mindfolioId}`)}
 ...
 {currentMindfolio.name} Charts
 </h1>
 <p className="text-gray-600">Mindfolio performance and allocation analysis</p>
 </div>
 </div>
 
 <div className="flex items-center space-x-3">
 <button
 onClick={fetchMindfolioChartsData}
 className="flex items-center space-x-2 px-3 py-2 bg-blue-600 text-[rgb(252, 251, 255)] rounded-lg hover:bg-blue-700 transition-colors"
 >
 <RefreshCw size={16} />
 <span>Refresh</span>
 </button>
 ...
 <div>
 <h3 className="text-3xl font-medium text-gray-900">Mindfolio Allocation</h3>
 ...
 ${(mindfolioData.cash_balance / 1000).toFixed(0)}k
 </span>
 </div>
 ...
 {(mindfolioData?.margin_used > 0 || mindfolioData?.margin_available > 0) && (
 <div className="bg-orange-50 p-3 rounded-lg">
 <div className="flex items-center justify-between">
 <span className="text-xl font-medium text-orange-700">
 {mindfolioData?.margin_used > 0 ? 'Margin Used' : 'Margin Available'}
 </span>
 <span className="text-3xl font-medium text-orange-900">
 ${((mindfolioData?.margin_used || mindfolioData?.margin_available || 0) / 1000).toFixed(0)}k
 </span>
 </div>
 <p className="text-lg text-orange-600 mt-1">
 {mindfolioData?.margin_used > 0 ? 'Currently leveraged amount' : 'Available when cash depleted'}
 </p>
 </div>
 )}
 ...
 ${((mindfolioData?.buying_power || 0) / 1000).toFixed(0)}k
 </span>
 </div>
 ...
 {/* Mindfolio Summary Panel */}
 <div className="bg-white rounded-xl shadow-sm">
 <div className="px-6 py-4 border-b border-gray-200">
 <h3 className="text-3xl font-medium text-gray-900">Mindfolio Summary</h3>
 </div>
 ...
 ${mindfolioData?.total_value?.toLocaleString() || '0.00'}
 </div>
 <div className="text-xl text-gray-600">Total Mindfolio Value</div>
 <div className="text-lg text-gray-500 mt-1">
 Cash excluded from charts: ${mindfolioData?.cash_balance?.toLocaleString() || '0.00'}
 </div>
 </div>
 
 <div className="text-center">
 <div className={`text-2xl font-medium ${
 (mindfolioData?.total_pnl || 0) >= 0 ? 'text-green-600' : 'text-red-600'
 }`}>
 {(mindfolioData?.total_pnl || 0) >= 0 ? '+' : ''}${mindfolioData?.total_pnl?.toFixed(2) || '0.00'}
 </div>
...
export default MindfolioCharts;
