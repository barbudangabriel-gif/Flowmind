...
const VirtualizedStockTable = ({ stocks, itemHeight = 60, containerHeight = 400, sortConfig, onSort }) => {
    ...
    return (
      ...
      >
        ...
        <div className="w-20 flex-shrink-0">
          <span className="font-mono font-bold text-blue-600 text-sm">{stock.symbol}</span>
        </div>

        {/* Company Name */}
        <div className="w-48 flex-shrink-0 px-2">
          <div className="truncate">
            <div className="font-medium text-sm text-gray-900 truncate">{stock.name}</div>
        ...
        <div className="w-32 flex-shrink-0 px-2">
          <span className="text-sm text-gray-600 truncate">{stock.sector}</span>
        </div>
        ...
        {/* Volume */}
        <div className="w-24 flex-shrink-0 px-2 text-right">
          <span className="text-sm">{formatNumber(stock.volume)}</span>
        </div>

        {/* Market Cap */}
        <div className="w-28 flex-shrink-0 px-2 text-right">
          <span className="text-sm">${formatNumber(stock.market_cap)}</span>
        </div>

        {/* P/E Ratio */}
        <div className="w-20 flex-shrink-0 px-2 text-right">
          <span className="text-sm">{stock.pe_ratio ? stock.pe_ratio.toFixed(2) : 'N/A'}</span>
        </div>
  ...
  const SortableHeader = ({ column, children, width, className = "" }) => (
    <div 
      className={`${width} flex-shrink-0 px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase cursor-pointer hover:bg-gray-100 transition-colors ${className}`}
