    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-[1000px] max-w-[96vw] max-h-[90vh] overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="p-4 border-b flex items-center gap-3">
          <div className="text-lg font-semibold">Strategy Builder — Catalog</div>
          <input 
            value={q} 
        ...
        <div className="px-4 pt-3 pb-2 text-sm text-slate-600 flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <input 
              ...
              className="w-40"
            />
            <span className="font-semibold w-6 text-right">{width}</span>
          </div>
          <div className="flex items-center gap-2">
            Distance (steps)
            <input 
              ...
              className="w-40"
            />
            <span className="font-semibold w-6 text-right">{dist}</span>
          </div>
          <div className="ml-auto text-slate-500">
            Symbol <span className="font-semibold">{symbol}</span>
            {expiry ? <> · Exp <span className="font-semibold">{expiry.slice(0, 10)}</span></> : null}
          </div>
        </div>
          ...
          {list.map(def => (
            <div key={def.id} className="border rounded-xl p-3 hover:shadow transition">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-700">{def.category}</span>
                ...
                <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-700">{def.bias}</span>
              </div>
              <div className="text-sm text-slate-600 mt-1">{def.explain}</div>
              {def.notes && <div className="text-xs text-slate-500 mt-1">{def.notes}</div>}
