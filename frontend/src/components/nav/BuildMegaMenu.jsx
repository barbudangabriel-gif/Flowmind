...
export default function BuildMegaMenu({ symbol = "TSLA" }) {
 ...
 {levels.map(level => (
 <div key={level}>
 <h4 className={`mb-3 text-xl font-medium ${levelColors[level]} flex items-center gap-2`}>
 {titles[level]}
 <span className="text-lg px-1.5 py-0.5 rounded bg-slate-800 text-slate-400">
 ...
 <h3 className="text-3xl font-medium text-[rgb(252, 251, 255)]">Build Strategy</h3>
 <button
 onClick={() => setOpen(false)}
 ...
 className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-[rgb(252, 251, 255)] placeholder-slate-400 focus:outline-none focus:border-emerald-500"
 />
 </div>
 ...
 href={deepLinkFromStrategy(strategy, { symbol })}
 onClick={(e) => handleStrategyClick(strategy, e)}
 className="block text-xl text-sky-300 hover:text-emerald-300 hover:underline transition-colors py-2 px-3 -mx-3 rounded-lg hover:bg-slate-800/50"
 title={`${strategy.name} - ${strategy.stance} strategy`}
 >
 {strategy.name}
 </a>
