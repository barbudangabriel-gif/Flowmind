  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-gray-900">Trade List</h2>
            ...
            {trades.map((trade) => (
              <tr key={trade.id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="py-3 px-4">
                  <span className={`inline-block text-xs px-2 py-1 rounded ${getTypeColor(trade.typeColor)}`}>
                    {trade.type}
                  </span>
                </td>
                <td className="py-3 px-4">
                  <span className={`inline-block text-xs px-2 py-1 rounded ${getStatusColor(trade.statusColor)}`}>
                    {trade.status}
                  </span>
                </td>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-red-50 p-4 rounded-lg">
          <div className="text-sm text-red-700 font-medium">Total Loss</div>
        ...
        <div className="bg-blue-50 p-4 rounded-lg">
          <div className="text-sm text-blue-700 font-medium">Net P&L</div>
          <div className={`text-lg font-bold ${trades.reduce((sum, t) => sum + t.pnl, 0) >= 0 ? 'text-green-900' : 'text-red-900'}`}>
