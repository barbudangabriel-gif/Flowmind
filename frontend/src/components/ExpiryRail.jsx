...
export default function ExpiryRail({ 
 ...
 <div className="flex justify-between text-lg text-slate-400 mb-1">
 ...
 <div className="text-lg text-slate-400 mb-2 font-medium">{month}</div>
...
function ExpiryBtn({ iso, active, onClick, showOpexMarker }) {
 ...
 onClick={() => onClick(iso)}
 className={[
 "relative px-3 py-1 rounded-md text-xl transition",
 active ? "bg-sky-500 text-[rgb(252, 251, 255)]" : "bg-slate-700/60 hover:bg-slate-600 text-slate-200"
 ].join(" ")}
 title={getTooltip()}
