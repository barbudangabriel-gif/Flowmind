    ...
    if (portfolioId === 'tradestation-main') {
      ...
      const loadRealTradeStationData = async () => {
        try {
          ...
          // Test data pentru a verifica collapse functionality
          const testPositions = [
            {
              id: 'aapl-stock',
              symbol: 'AAPL',
                    ...
                    <React.Fragment key={symbol}>
                      ...
                      {isExpanded && symbolPositions.map((position, index) => {
                        ...
                        return (
                          <tr 
                            key={position.id}
                            className="border-b border-slate-700 hover:bg-slate-700 cursor-context-menu bg-slate-900 border-l-4 border-blue-400"
