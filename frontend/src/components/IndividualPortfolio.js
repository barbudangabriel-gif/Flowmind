...
const IndividualPortfolio = () => {
 ...
 <h1 className="text-2xl font-medium text-[rgb(252, 251, 255)] flex items-center">Portfolio {displayPortfolio.name}</h1>
 </div>
 <div className="flex items-center mt-2">
 <Eye className="text-[rgb(252, 251, 255)] mr-2" size={16} />
 <span className="text-3xl font-medium text-[rgb(252, 251, 255)]">${((displayPortfolio?.total_value || 0) + cashBalance).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
 <span className="text-xl text-blue-200 ml-2">(Total Account Value)</span>
 <span className={`ml-4 text-3xl font-medium ${displayPortfolio.total_pnl >= 0 ? 'text-green-300' : 'text-red-300'}`}>{displayPortfolio.total_pnl >= 0 ? '+' : ''}${Math.abs(displayPortfolio.total_pnl).toFixed(2)} ({changePercent >= 0 ? '+' : ''}{changePercent.toFixed(2)}%)</span>
 </div>
 <div className="flex items-center mt-4 space-x-3">
 <button onClick={() => navigate(`/portfolios/${portfolioId}/charts`)} className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-[rgb(252, 251, 255)] rounded-lg hover:bg-blue-700 transition-colors">
 <PieChartIcon size={16} />
 <span>Charts</span>
 </button>
 <button onClick={() => navigate(`/portfolios/${portfolioId}/rebalancing`)} className="flex items-center space-x-2 px-4 py-2 bg-purple-600 text-[rgb(252, 251, 255)] rounded-lg hover:bg-purple-700 transition-colors">
 <Brain size={16} />
 ...
 <td className="py-3 px-2 font-medium text-slate-200">TOTAL PORTFOLIO</td>
 ...
 <td className={`text-right py-3 px-2 font-medium ${getChangeColor(displayPortfolio.total_pnl || 0)}`}>{displayPortfolio?.total_value > 0 ? `${(((displayPortfolio.total_pnl || 0) / (displayPortfolio.total_value - (displayPortfolio.total_pnl || 0))) * 100).toFixed(2)}%` : '0.00%'}</td>
