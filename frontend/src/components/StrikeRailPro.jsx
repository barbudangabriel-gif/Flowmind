...
export function StrikeRailPro({
 ...
 <div className="text-rose-500 mb-1"> Chain Error</div>
 <div className="text-xl opacity-70">{error}</div>
 </div>
 </div>
 );
 }
 ...
 <div className="text-xl opacity-70">No options chain available</div>
 </div>
 </div>
 );
 }
 ...
 <div className="flex items-center justify-between mb-2 text-lg text-slate-300">
 ...
 className={"relative rounded-lg px-2 py-2 text-lg text-slate-200 bg-white/5 hover:bg-white/10 transition border "+(active?"border-emerald-400 shadow-[0_0_0_2px_rgba(16,185,129,0.2)]":"border-transparent")}
 >
 {/* header strike */}
 <div className="flex items-center justify-between mb-1">
 <div className="font-medium">{c.strike.toFixed(0)}</div>
 ...
 {legsHere.map((l,i)=> (
 <span key={i} className={"px-1.5 py-0.5 rounded text-[10px] font-medium "+badgeCls(l)}>{l.side==='BUY'?'B':'S'}{l.type==='CALL'?'C':'P'}</span>
 ))}
 </div>
 )}
 </button>
 ...
 {hover && (
 <div className="mt-2 text-lg rounded-lg border border-[#12233f] bg-[#0b1c33] p-3 shadow-xl">
 <div className="flex gap-6">
 <div>
 <div className="text-slate-400">Strike</div>
 <div className="font-medium">{hover.strike.toFixed(0)}</div>
 </div>
 <div>
...
function badgeCls(l){
 const base = 'shadow text-[rgb(252, 251, 255)]'
