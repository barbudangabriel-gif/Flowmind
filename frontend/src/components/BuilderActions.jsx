...
export function TradeButton({ bias, onClick }) {
 ...
 ? 'bg-emerald-600 hover:bg-emerald-700 text-[rgb(252, 251, 255)]'
 : bias === 'Bearish'
 ? 'bg-rose-600 hover:bg-rose-700 text-[rgb(252, 251, 255)]'
 : 'bg-amber-500 hover:bg-amber-600 text-slate-900';
 
 return (
 <button onClick={onClick} className={clsx('px-4 py-1.5 rounded font-medium shadow-sm', colors)}>
 TRADE
 </button>
 );
}
...
export function AddMenu({ onAdd }) {
 ...
 <button onClick={() => setOpen(v => !v)} className="px-3 py-1.5 rounded bg-sky-600 text-[rgb(252, 251, 255)] text-xl">
 Add +
 </button>
 {open && (
 <div className="absolute right-0 mt-2 w-44 bg-white border rounded-lg shadow z-20 text-xl">
...
export function SaveTradeModal({ open, onClose, defaultName, onSave }) {
 ...
 <div className="bg-[#0b2347] text-[rgb(252, 251, 255)] rounded-2xl p-5 w-[680px] max-w-[92vw]" onClick={e => e.stopPropagation()}>
 <div className="text-2xl font-medium mb-4">Save & Share this Trade</div>
 <div className="space-y-3">
 <div>
 <div className="text-xl opacity-80 mb-1">Trade Name (optional)</div>
 <input 
 value={name} 
 ...
 <div>
 <div className="text-xl opacity-80 mb-1">Trade Description (optional)</div>
 <textarea 
 rows={4} 
 value={notes} 
 ...
 <div>
 <div className="text-xl opacity-80 mb-1">Group</div>
 <select 
 value={group} 
 ...
 className="px-3 py-1.5 rounded bg-sky-600 text-[rgb(252, 251, 255)]"
 >
 Save trade
 </button>
 </div>
...
export function PositionsModal({ 
 ...
 <div className="text-2xl font-medium mb-3">Positions</div>
 <div className="flex gap-2 mb-2">
 <button className="px-3 py-1.5 rounded bg-sky-600 text-[rgb(252, 251, 255)] text-xl">
 Open ({positions.length})
 </button>
 <button className="px-3 py-1.5 rounded bg-slate-100 text-slate-700 text-xl">
 Closed (0)
 </button>
 </div>
 <div className="text-xl mb-3">Total unrealized gain: {fmtUSD(unrealized)} (0%)</div>
 <div className="border rounded p-2 mb-3 text-xl">
 {positions.map((p, i) => (
 <div key={i} className="py-1">{p.label}</div>
 ))}
 </div>
 <div className="grid grid-cols-2 gap-2 mb-3 text-xl">
 <button onClick={onFlip} className="px-3 py-2 rounded bg-slate-900 text-[rgb(252, 251, 255)]">
 Flip Positions
 </button>
 ...
 <div className="mb-3">
 <div className="text-xl mb-1">Pricing Mode</div>
 <div className="flex gap-2">
 <button 
 onClick={() => setPricingMode('Midpoint')} 
 className={clsx('px-3 py-1.5 rounded text-xl', pricingMode === 'Midpoint' ? 'bg-sky-600 text-[rgb(252, 251, 255)]' : 'bg-slate-100')}
 >
 Midpoint
 </button>
 <button 
 onClick={() => setPricingMode('BidAsk')} 
 className={clsx('px-3 py-1.5 rounded text-xl', pricingMode === 'BidAsk' ? 'bg-sky-600 text-[rgb(252, 251, 255)]' : 'bg-slate-100')}
 >
 Bid / Ask
 </button>
 </div>
 ...
 <label className="mt-3 inline-flex items-center gap-2 text-xl">
 <input 
 type="checkbox" 
...
export function HistoricalChartModal({ open, onClose, data }) {
 ...
 <div className="bg-[#0a2a4f] rounded-2xl text-[rgb(252, 251, 255)] p-4 w-[1000px] max-w-[96vw]" onClick={e => e.stopPropagation()}>
 <div className="text-2xl font-medium mb-2">Price History</div>
...
export function ActionsBar({
 ...
 <AddMenu onAdd={onAdd} />
 <button onClick={onOpenPositions} className="px-3 py-1.5 rounded bg-slate-100 text-slate-700 text-xl">
 Positions
 </button>
 <button onClick={onOpenSave} className="px-3 py-1.5 rounded bg-indigo-600 text-[rgb(252, 251, 255)] text-xl">
 Save Trade
 </button>
 <button onClick={onOpenHistorical} className="px-3 py-1.5 rounded bg-slate-900 text-[rgb(252, 251, 255)] text-xl">
 Historical Chart
 </button>
