...
export function LivePriceTicker({ 
 ...
 <div className="px-2.5 py-1 rounded-lg bg-slate-900 text-[rgb(252, 251, 255)] font-medium text-xl tracking-wide">
 {symbol}
 </div>
 <div className="text-xl font-medium tabular-nums">
 {price != null ? price.toLocaleString(undefined, {maximumFractionDigits: 2}) : '—'}
 </div>
 <div className={clsx('text-xl font-medium tabular-nums', color)}>
 ...
 <button 
 onClick={() => setOpen(v => !v)} 
 className="ml-1 px-2 py-1 rounded-lg bg-slate-100 text-slate-700 text-lg border flex items-center gap-1" 
 title="Realtime info"
 >
 <span>Real‑time</span>
 ...
 {open && (
 <div className="absolute z-20 mt-2 w-80 p-3 rounded-xl bg-white border shadow text-xl text-slate-700">
 ...
 className="px-3 py-1.5 rounded bg-slate-900 text-[rgb(252, 251, 255)] text-lg"
 >
...
export function StrategyTitle({ info }) {
 ...
 <div className="text-2xl font-medium mb-2">{info.name}</div>
 <div className="text-xl leading-5 opacity-95">{info.explain}</div>
 {info.badges?.length ? (
 <div className="mt-2 flex flex-wrap gap-2">
 {info.badges.map(b => (
 <span 
 key={b} 
 className="px-2 py-0.5 rounded-full bg-emerald-600/20 border border-emerald-400/40 text-emerald-200 text-lg"
 >
 {b}
 </span>
 ))}
...
export function ExpirationPicker({ nowISO, expirations, selected, onSelect }) {
 ...
 onClick={() => setMonth(ym)} 
 className={clsx(
 'px-3 py-1.5 rounded-lg border text-xl', 
 month === ym ? 'bg-slate-900 text-[rgb(252, 251, 255)] border-slate-900' : 'bg-slate-100 text-slate-700 border-slate-200'
 )}
 >
 ...
 className={clsx(
 'w-10 h-8 rounded-lg border text-xl', 
 sel ? 'bg-sky-600 text-[rgb(252, 251, 255)] border-sky-600' : 'bg-white text-slate-700 border-slate-200 hover:border-slate-300'
 )}
 >
 {day}
 </button>
...
export function BuilderHeader({
 ...
 <button 
 onClick={onClickAdd} 
 className="px-3 py-1.5 rounded bg-sky-600 text-[rgb(252, 251, 255)] text-xl"
 >
 Add +
 </button>
 <button 
 onClick={onClickPositions} 
 className="px-3 py-1.5 rounded bg-slate-100 text-slate-700 text-xl border"
 >
 Positions
 </button>
 <button 
 onClick={onClickSave} 
 className="px-3 py-1.5 rounded bg-indigo-600 text-[rgb(252, 251, 255)] text-xl"
 >
 Save Trade
 </button>
 <button 
