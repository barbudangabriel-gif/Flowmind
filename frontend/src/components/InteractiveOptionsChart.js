...
const InteractiveOptionsChart = ({ 
 ...
 <h5 className="text-[rgb(252, 251, 255)] font-medium">{strategyName} - Profit & Loss</h5>
 ...
 {/* Price Labels */}
 <div className="absolute bottom-2 left-2 text-lg text-gray-400 font-mono">
 ${Math.min(...chartData.x).toFixed(0)}
 </div>
 <div className="absolute bottom-2 right-2 text-lg text-gray-400 font-mono">
 ${Math.max(...chartData.x).toFixed(0)}
 </div>
 {showCurrentPrice && stockPrice && (
 <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 text-lg text-orange-400 font-medium bg-gray-800 px-1 rounded">
 ${stockPrice.toFixed(0)}
 </div>
 )}
 
 {/* P&L Labels */}
 <div className="absolute top-2 left-2 text-lg text-green-400 font-medium bg-gray-800 px-1 rounded">
 ${maxProfit.toFixed(0)}
 </div>
 <div className="absolute top-2 right-2 text-lg text-red-400 font-medium bg-gray-800 px-1 rounded">
 ${Math.abs(maxLoss).toFixed(0)}
 </div>
 </div>
 
 {/* Chart Legend */}
 <div className="mt-3 flex items-center justify-center space-x-6 text-lg">
