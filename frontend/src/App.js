...
const AdvancedOptionsModal = ({ strategy, isOpen, onClose, isDarkMode }) => {
  ...
  const generateMockOptionsChain = () => {
    ...
    const expirations = [
      { date: 'Aug 15', dte: 2, type: 'Standard', expectedRange: '30.12 - 33.34', movePercent: '+/- 5.07%', iv: '66.6%', maxPain: '30 strike' },
      { date: 'Aug 22', dte: 9, type: 'Weeklys', expectedRange: '29.07 - 34.39', movePercent: '+/- 8.37%', iv: '52.9%', maxPain: '31.5 strike' },
      { date: 'Aug 29', dte: 16, type: 'EoM', expectedRange: '28.30 - 35.16', movePercent: '+/- 10.81%', iv: '51.5%', maxPain: '30 strike' },
      { date: 'Sep 5', dte: 23, type: 'Weeklys', expectedRange: '27.86 - 35.60', movePercent: '+/- 12.2%', iv: '48.5%', maxPain: '30 strike' },
      { date: 'Sep 12', dte: 30, type: 'Weeklys', expectedRange: '27.46 - 36', movePercent: '+/- 13.46%', iv: '46.9%', maxPain: '30 strike' }
    ];

    setOptionsChain(expirations);
  };
  ...
  const handleTradeClick = (expiration) => {
    alert(`Opening trade interface for ${strategy.ticker} ${expiration.date} expiration`);
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className={`${isDarkMode ? 'bg-slate-800 text-white' : 'bg-white text-gray-900'} rounded-lg shadow-xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col`}>
          ...
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2">
              <div className="bg-blue-100 text-blue-800 px-2 py-1 rounded text-sm font-semibold">
                {underlyingData?.symbol || strategy.ticker}
              </div>
              <span className="text-sm text-gray-500">{underlyingData?.company || getCompanyName(strategy.ticker)}</span>
              <span className="font-semibold">${(underlyingData?.price || strategy.entry_logic?.underlying_price || 100).toFixed(2)}</span>
              <span className="text-green-500 text-sm">+{underlyingData?.change || 0.35} {(underlyingData?.changePercent || 1.12).toFixed(2)}%</span>
            </div>
            <div className="text-sm text-gray-500">
              Earnings in {underlyingData?.earnings || '63d'}
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Strategy Info Bar */}
        <div className={`px-6 py-3 ${isDarkMode ? 'bg-slate-700' : 'bg-blue-50'} border-b`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <span className="font-semibold text-blue-600">Recommended Strategy:</span>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-2/3 p-6 border-r">
            <div className="mb-4 flex space-x-2 text-sm">
              <button className="bg-blue-500 text-white px-3 py-1 rounded">1M</button>
              <button className="text-gray-600 px-3 py-1">3M</button>
              <button className="text-gray-600 px-3 py-1">6M</button>
              <button className="text-gray-600 px-3 py-1">1Y</button>
              <button className="text-gray-600 px-3 py-1">3Y</button>
              <div className="ml-auto flex items-center space-x-4 text-xs">
                <span className="flex items-center"><div className="w-3 h-3 bg-green-400 rounded mr-1"></div>Expected Range</span>
                <span className="flex items-center"><div className="w-3 h-3 bg-orange-400 rounded mr-1"></div>Max Pain</span>
              </div>
            </div>
            
            {/* Mock Chart Area */}
            <div className={`${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'} rounded p-4 h-64 flex items-center justify-center`}>
              <div className="text-center text-gray-500">
                <BarChart3 className="w-12 h-12 mx-auto mb-2" />
                <p>Interactive Price Chart</p>
                <p className="text-sm">Expected Range & Max Pain Analysis</p>
              </div>
            </div>
            ...
            <div className="mt-6">
              <div className="border-b border-gray-200">
                <nav className="-mb-px flex space-x-8">
                  {['Expirations', 'Stats', 'Technicals'].map((tab) => (
                    <button
                      key={tab}
              ...
              {activeTab === 'expirations' && (
                <div className="mt-4">
                  <div className={`${isDarkMode ? 'bg-slate-700' : 'bg-white'} rounded-lg border overflow-hidden`}>
                    <table className="w-full">
                      <thead className={`${isDarkMode ? 'bg-slate-600' : 'bg-gray-50'}`}>
                        <tr className="text-left text-xs text-gray-500 uppercase tracking-wider">
                          ...
                          <th className="px-4 py-3">Max Pain</th>
                          <th className="px-4 py-3"></th>
                        </tr>
                      </thead>
                      <tbody>
                        {optionsChain.map((exp, index) => (
                          <tr key={index} className={`${isDarkMode ? 'border-slate-600' : 'border-gray-200'} border-b hover:bg-opacity-50 hover:bg-gray-50`}>
                            <td className="px-4 py-3 font-medium">{exp.date}</td>
                            <td className="px-4 py-3">{exp.dte}d</td>
                            <td className="px-4 py-3">{exp.type}</td>
                            <td className="px-4 py-3">{exp.expectedRange} <span className="text-gray-500">{exp.movePercent}</span></td>
                            <td className="px-4 py-3">{exp.iv}</td>
                            <td className="px-4 py-3">{exp.maxPain}</td>
                            <td className="px-4 py-3">
                              <button
                                onClick={() => handleTradeClick(exp)}
