...
function FlowScroller() {
 ...
 <div className="text-xl font-medium text-slate-100">{it.label}</div>
 <div className="text-lg text-slate-400 mt-1">{it.desc}</div>
 </button>
 ))}
 </div>
 </div>
...
export default function OptionsWorkbench() {
 ...
 {showBuild && (
 <Suspense fallback={<div className="absolute left-0 top-full mt-1 px-3 py-2 text-lg bg-slate-900/90 border border-slate-800 rounded">Loading…</div>}>
 <BuildHoverMegaMenu
 symbol={symbol}
