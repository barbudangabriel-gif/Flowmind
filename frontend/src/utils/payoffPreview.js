  ...
  if (!strategy) {
    return (
      <div 
        ...
        style={{ width, height }}
      >
        <div className="text-xs text-slate-500">No preview</div>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg bg-slate-900/60 border border-slate-700 overflow-hidden">
      <svg width={width} height={height} className="w-full">
        ...
        {/* Labels */}
        <text x={pad.l} y={height - 5} fontSize="10" fill="#94a3b8" className="text-xs">
          P&L
        </text>
        <text x={width - pad.r - 30} y={height - 5} fontSize="10" fill="#94a3b8" className="text-xs">
          Price
        </text>
      </svg>
    </div>
