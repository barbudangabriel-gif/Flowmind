...
export default function ScreenerV2() {
 ...
 <div className="flex flex-col">
 <label className="text-xl">Mult</label>
 ...
 <div className="flex flex-col">
 <label className="text-xl">Limit</label>
 ...
 <span className="text-xl">Hide Errors</span>
 </label>
 <button onClick={fetchBatch} disabled={loading} className="px-4 py-2 rounded bg-blue-600 text-[rgb(252, 251, 255)] disabled:opacity-50">
 ...
 <h3 className="font-medium">Order Payload</h3>
 <button onClick={() => setShowPayload(false)} className="px-2 py-1">✕</button>
 </div>
 <pre className="bg-gray-100 rounded p-3 max-h-[60vh] overflow-auto text-lg">{payloadJson}</pre>
 <div className="flex items-center justify-between gap-2">
 <div className="text-lg text-gray-500">TS endpoint: {TS_SEND_URL}</div>
 <div className="flex gap-2">
 <button onClick={() => navigator.clipboard?.writeText(payloadJson)} className="px-3 py-1 rounded border">Copy</button>
 <button onClick={sendToTS} className="px-3 py-1 rounded bg-black text-[rgb(252, 251, 255)]">Send to TS (SIM)</button>
 <button onClick={() => setShowPayload(false)} className="px-3 py-1 rounded">Close</button>
 </div>
 </div>
 {sendMsg && (
 <div className={`text-xl ${sendMsg.startsWith('OK') ? 'text-green-700' : 'text-red-600'}`}>{sendMsg}</div>
 )}
 </div>
 </div>
 )}
